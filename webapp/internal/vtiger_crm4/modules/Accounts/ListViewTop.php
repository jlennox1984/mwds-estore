...
$list_query = 'select custbranch.accountid, custbranch.brname, sum(potential.amount) as amount from potential inner join crmentity on (potential.potentialid=crmentity.crmid) inner join custbranch on (potential.accountid=custbranch.accountid) where crmentity.smownerid="'.$current_user->id.'" and potential.sales_stage <> "'.$app_strings['LBL_CLOSE_WON'].'" and potential.sales_stage <> "'.$app_strings['LBL_CLOSE_LOST'].'" group by custbranch.brname order by 3 desc;';
...
for($i=0;$i<$noofrows;$i++) 
{
  ...
  $open_accounts_list[] = Array('accountid' => $adb->query_result($list_result,$i,'accountid'),
                                     'brname' => $adb->query_result($list_result,$i,'brname'),
