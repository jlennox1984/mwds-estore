...
if(isset($_REQUEST['query']) && $_REQUEST['query'] == 'true')
{
	...
	if (isset($_REQUEST['brname'])) $name = $_REQUEST['brname'];
...
//upto this added for Custom Field
	...
	if(isset($name) && $name != "")
	{
		array_push($where_clauses, "custbranch.brname like ".PearDatabase::quote($name."%"));
		$url_string .= "&brname=".$name;
	}
	if(isset($website) && $website != "")
	{
		array_push($where_clauses, "custbranch.website like ".PearDatabase::quote("%".$website."%"));
		$url_string .= "&website=".$website;
	}
	if(isset($phone) && $phone != "")
	{
		array_push($where_clauses, "(custbranch.phoneno like ".PearDatabase::quote("%".$phone."%")." OR custbranch.otherphone like ".PearDatabase::quote("%".$phone."%")." OR custbranch.faxno like ".PearDatabase::quote("%".$phone."%").")");
		$url_string .= "&phone=".$phone;
	}
	if(isset($annual_revenue) && $annual_revenue != "")
	{
		array_push($where_clauses, "custbranch.annualrevenue like ".PearDatabase::quote($annual_revenue."%"));
		$url_string .= "&annual_revenue=".$annual_revenue;
	}
	if(isset($employees) && $employees != "")
	{
		array_push($where_clauses, "custbranch.employees like ".PearDatabase::quote($employees."%"));
		$url_string .= "&employees=".$employees;
	}
	if(isset($address_street) && $address_street != "")
	{
		array_push($where_clauses, "(custbranch.braddress1 like ".PearDatabase::quote($address_street."%")." OR custbranch.brpostaddr1  like ".PearDatabase::quote($address_street."%").")");
		$url_string .= "&address_street=".$address_street;
	}
	if(isset($address_city) && $address_city != "")
	{
		array_push($where_clauses, "(custbranch.braddress3 like ".PearDatabase::quote($address_city."%")." OR custbranch.brpostaddr3 like ".PearDatabase::quote($address_city."%").")");
		$url_string .= "&bill_city=".$address_city;
	}
	if(isset($address_state) && $address_state != "")
	{
		array_push($where_clauses, "(custbranch.braddress4 like ".PearDatabase::quote($address_state."%")." OR custbranch.brpostaddr4 like ".PearDatabase::quote($address_state."%").")");
		$url_string .= "&bill_state=".$address_state;
	}
	if(isset($address_postalcode) && $address_postalcode != "")
	{
		array_push($where_clauses, "(custbranch.braddress6 like ".PearDatabase::quote($address_postalcode."%")." OR custbranch.brpostaddr6 like ".PearDatabase::quote($address_postalcode."%").")");
		$url_string .= "&bill_code=".$address_postalcode;
	}
	if(isset($address_country) && $address_country != "")
	{
		array_push($where_clauses, "(custbranch.braddress5 like ".PearDatabase::quote($address_country."%")." OR custbranch.brpostaddr5 like ".PearDatabase::quote($address_country."%").")");
		$url_string .= "&bill_country=".$address_country;
	}
	if(isset($email) && $email != "")
	{
		array_push($where_clauses, "(custbranch.email like ".PearDatabase::quote($email."%")." OR custbranch.email2 like ".PearDatabase::quote($email."%").")");
		$url_string .= "&email=".$email;
	}
	if(isset($industry) && $industry != "")
	{
		array_push($where_clauses, "custbranch.industry = ".PearDatabase::quote($industry));
		$url_string .= "&industry=".$industry;
	}
	if(isset($ownership) && $ownership != "")
	{
	 	array_push($where_clauses, "custbranch.ownership like ".PearDatabase::quote($ownership."%"));
		$url_string .= "&ownership=".$ownership;
	}
	if(isset($rating) && $rating != "") array_push($where_clauses, "custbranch.rating like ".PearDatabase::quote($rating."%"));
	if(isset($sic_code) && $sic_code != "")
	{
		array_push($where_clauses, "custbranch.siccode like ".PearDatabase::quote($sic_code."%"));
		$url_string .= "&siccode=".$sic_code;
	}
	if(isset($ticker_symbol) && $ticker_symbol != "")
	{
		array_push($where_clauses, "custbranch.tickersymbol like ".PearDatabase::quote($ticker_symbol."%"));
		$url_string .= "&tickersymbol=".$ticker_symbol;
	}
	if(isset($account_type) && $account_type != "")
	{
		array_push($where_clauses, "custbranch.account_type = ".PearDatabase::quote($account_type));
...
global $vtlog;
	$vtlog->logthis("in viewname ".$_REQUEST['viewname'],'info');  
	$vtlog->logthis("in viewid ".$viewid,'info');  
...
if (!isset($_REQUEST['search_form']) || $_REQUEST['search_form'] != 'false') {
	...
	$search_form->assign("ALPHABETICAL",AlphabeticalSearch('Accounts','index','brname','true','advanced',"","","","",$viewid));
...
//upto this added for Custom Field
	...
	global $vtlog;
	$vtlog->logthis("in viewname1 ".$_REQUEST['viewname'],'info');  
	$vtlog->logthis("in viewid1 ".$viewid,'info');  

		$search_form->assign("ALPHABETICAL",AlphabeticalSearch('Accounts','index','brname','true','basic',"","","","",$viewid));
...
$ListView->setQuery($where, "", "brname", "ACCOUNT");
