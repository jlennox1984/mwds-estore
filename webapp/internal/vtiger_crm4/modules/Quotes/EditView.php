...
{
	if(isset($_REQUEST['convertmode']) &&  $_REQUEST['convertmode'] == 'potentoinvoice')
    {
	    $focus->mode = '';		
    }
	else {
...
if(isset($_REQUEST['account_id']) && $_REQUEST['account_id']!='' && ($_REQUEST['record']=='' || $_REQUEST['convertmode'] == "potentoinvoice")){
	...
	$focus->column_fields['bill_street2']=$acct_focus->column_fields['bill_street2'];
	$focus->column_fields['ship_street2']=$acct_focus->column_fields['ship_street2'];
	...
	if(isset($_REQUEST['opportunity_id']) && $_REQUEST['opportunity_id'] !='' && ($_REQUEST['record']=='' || $_REQUEST['convertmode'] == "potentoinvoice"))
{
	$potfocus = new Potential();
        $potfocus->column_fields['potential_id'] = $_REQUEST['opportunity_id'];
		$potfocus->retrieve_entity_info($_REQUEST['opportunity_id'],"Potentials");
	
		$temp = $potfocus->column_fields['potential_id'];
		$temp1 = $potfocus->column_fields['potentialname'];

	$focus->column_fields['potential_id']=$temp;
	$focus->column_fields['potential_name']=$temp1;
...
if($focus->mode == 'edit')
{
	...
	$xtpl->assign("DISCOUNTVALUE", $focus->column_fields['hdnDiscountTotal']);
	$xtpl->assign("ADJUSTMENTVALUE", $focus->column_fields['freightcost']);
	$xtpl->assign("FREIGHTTAXVALUE", $focus->column_fields['freighttax']);
...
elseif((isset($_REQUEST['potential_id']) && $_REQUEST['potential_id'] != '') || (isset($_REQUEST['product_id']) && $_REQUEST['product_id'] != '')) {
	...
	$InvTotal = getInventoryTotal($_REQUEST['return_module'],$_REQUEST['return_id']);
        ...
        $xtpl->assign("DISCOUNTVALUE", "0.000");
        $xtpl->assign("ADJUSTMENTVALUE", "0.000");
        $xtpl->assign("FREIGHTTAXVALUE", "0.000");
...
else
{
	...
	$xtpl->assign("DISCOUNTVALUE", '0');
	$xtpl->assign("ADJUSTMENTVALUE", '0');
	$xtpl->assign("FREIGHTTAXVALUE", '0');
	//Setting the first row
    	$vtlog->logthis("Setting the first product row when the mode is create",'debug');		
	$output ='';
	$output .= '<tr id="row1" class="oddListRow">';
        $output .= '<td height="25" style="padding:3px;" nowrap><input id="txtProduct1" name="txtProduct1" type="text" size="30" readonly> <img src="'.$image_path.'search.gif" onClick=\'productPickList(this)\' align="absmiddle" style=\'cursor:hand;cursor:pointer\'></td>';
        $output .= '<td WIDTH="1" class="blackLine"><IMG SRC="'.$image_path.'blank.gif"></td>';
        $output .= '<td style="padding:3px;"><div id="qtyInStock1"></div>&nbsp;</td>';
			$output .= '<td WIDTH="1" class="blackLine"><IMG SRC="'.$image_path.'blank.gif"></td>';
			$output .= '<td style="padding:3px;"><div id="qtyInDemand1"></div>&nbsp;</td>';
			$output .= '<td WIDTH="1" class="blackLine"><IMG SRC="'.$image_path.'blank.gif"></td>';
        ...
        $output .= '<td WIDTH="1" class="blackLine"><IMG SRC="'.$image_path.'blank.gif"></td>';
        $output .= '<td style="padding:3px;"><input type=text id="txtDiscount1" name="txtDiscount1" value="'.$discount.'" size="8" onBlur="calcTotal2(this)">%</td>';
		$output .= '<td WIDTH="1" class="blackLine"><IMG SRC="'.$image_path.'blank.gif"></td>';
        ...
        $output .= '<input type="hidden" id="hdnSOId1" name="hdnSOId1">';
        $output .= '<input type="hidden" id="hdnStockId1" name="hdnStockId1">';
        $output .= '<input type="hidden" id="hdnProductId1" name="hdnProductId1">';
        $output .= '<input type="hidden" id="hdnRowStatus1" name="hdnRowStatus1">';
        $output .= '<input type="hidden" id="hdnOrderlineno1" name="hdnOrderlineno1" value="0">';
        $output .= '<input type="hidden" id="hdnItemTax1" name="hdnItemTax1">';
        $output .= '<input type="hidden" id="hdnItemTaxTotal1" name="hdnItemTaxTotal1">';
        $output .= '<input type="hidden" id="hdnTotal1" name="hdnTotal1">';
        $output .= '<input type="hidden" id="hdnDiscount1" name="hdnDiscount1">';
        $output .= '</td></tr>';
