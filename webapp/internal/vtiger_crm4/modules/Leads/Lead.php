...
class Lead extends CRMEntity {
  ...
  var $accountid;
	...
	//var $tab_name = Array('crmentity','custbranch','accountbillads','accountshipads');
...
//          renderRelatedNotes($query,$id);
  ...
  {
	//	$query = 'select users.user_name, users.id, leaddetails.leadid,leaddetails.lastname, stockmaster.productid, stockmaster.description, seticketsrel.*, troubletickets.ticketid, troubletickets.title, troubletickets.status, crmentity.crmid, crmentity.smownerid, crmentity.modifiedtime from leaddetails inner join seproductsrel on seproductsrel.crmid = leaddetails.leadid inner join stockmaster on seproductsrel.productid = stockmaster.productid inner join users on users.id=crmentity.smownerid inner join seticketsrel on seticketsrel.crmid = leaddetails.leadid left join troubletickets on troubletickets.ticketid = seticketsrel.ticketid inner join crmentity on crmentity.crmid = leaddetails.leadid where leaddetails.leadid='.$id;
	...
	{
		$query = 'select stockmaster.productid, stockmaster.description, stockmaster.stockid, stockmaster.commissionrate, stockmaster.qty_per_unit, stockmaster.unit_price, crmentity.crmid, crmentity.smownerid from stockmaster inner join seproductsrel on stockmaster.productid = seproductsrel.productid inner join crmentity on crmentity.crmid = stockmaster.productid inner join leaddetails on leaddetails.leadid = seproductsrel.crmid  where leaddetails.leadid = '.$id.' and crmentity.deleted = 0';
