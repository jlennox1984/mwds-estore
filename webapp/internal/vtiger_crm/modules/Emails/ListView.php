...
if(isset($_REQUEST['query']) && $_REQUEST['query'] == 'true')
{
	...
	if(isset($name) && $name != '')
	{
		array_push($where_clauses, "activity.subject like ".$adb->quote($name.'%')."");
		$url_string .= "&subject=".$name;

	}
	if(isset($contactname) && $contactname != '')
	{
		array_push($where_clauses, "(contactdetails.firstname like ".$adb->quote($contactname.'%')." OR contactdetails.lastname like ".$adb->quote($contactname.'%').")");
		$url_string .= "&contactname=".$contactname;

	}
	if(isset($date_start) && $date_start != '')
	{
		array_push($where_clauses, "events.eventdatestart like ".$adb->quote($date_start.'%')."");
	}
