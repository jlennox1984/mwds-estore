...
class Account extends CRMEntity {
	...
	var $table_name = "custbranch";
	var $tab_name = Array('crmentity','custbranch','accountscf');
	var $tab_name_index = Array('crmentity'=>'crmid','custbranch'=>'accountid','accountscf'=>'accountid');
...
//	var $billadr_table = "accountbillads";
	...
	var $sortby_fields = Array('brname','braddress3','website','phoneno');		
	...
	var $list_fields = Array(
				'Account Name'=>Array('custbranch'=>'brname'),
				'City'=>Array('custbranch'=>'braddress3'), 
				'Website'=>Array('custbranch'=>'website'),
				'Phone'=>Array('custbranch'=> 'phoneno'),
	...
	var $search_fields1 = Array(
				'Account Name'=>Array('custbranch'=>'debtorno'),
//				'City'=>Array('custbranch'=>'address3'), 
				);
	
	var $search_fields_name1 = Array(
				        'Account Name'=>'debtorno',
//				        'City'=>'address3',
				      );
	...
	function get_member_accounts()
	{
		// First, get the list of IDs.
          //$query = "SELECT a1.id from accounts as a1, accounts as a2 where a2.id=a1.parent_id AND a2.id='$this->id' AND a1.deleted=0";
             	$query = "SELECT a1.id from custbranch a1, custbranch  a2 where a2.id=a1.parent_id AND a2.id='$this->id' AND a1.deleted=0";
  ...
  {
//	$query = "select stockmaster.productid, stockmaster.description, stockmaster.stockid, potential.potentialid, potential.accountid, potential.potentialname, potential.amount, potential.closingdate, potential.potentialtype, crmentity.crmid from stockmaster,potential inner join seproductsrel on seproductsrel.crmid = potential.accountid and seproductsrel.productid=stockmaster.productid inner join crmentity on crmentity.crmid=potential.potentialid";
...
//          $query="select activity.subject,emails.description,emails.filename from emails inner join activity on activity.activityid=emails.emailid inner join seactivityrel on seactivityrel.activityid=emails.emailid inner join crmentity on crmentity.crmid=seactivityrel.crmid and crmentity.deleted=0 where crmentity.crmid=".$id."";
	...
	{
		$query = "select crmentity.*, quotes.*,potential.potentialname,custbranch.brname from quotes inner join crmentity on crmentity.crmid=quotes.quoteid left outer join custbranch on custbranch.accountid=quotes.accountid left outer join potential on potential.potentialid=quotes.potentialid where crmentity.deleted=0 and custbranch.accountid=".$id;
		renderRelatedQuotes($query,$id);
	}
	function get_invoices($id)
	{
		$query = "select crmentity.*, invoice.*, custbranch.brname, salesorders.subject as salessubject from invoice inner join crmentity on crmentity.crmid=invoice.invoiceid left outer join custbranch on custbranch.accountid=invoice.accountid left outer join salesorders on salesorders.salesorderid=invoice.salesorderid where crmentity.deleted=0 and custbranch.accountid=".$id;
		renderRelatedInvoices($query,$id);
	}
	function get_salesorder($id)
	{
		$query = "select crmentity.*, salesorders.*, quotes.subject as quotename, custbranch.brname from salesorders inner join crmentity on crmentity.crmid=salesorders.salesorderid left outer join quotes on quotes.quoteid=salesorders.quoteid left outer join custbranch on custbranch.accountid=salesorders.accountid where crmentity.deleted=0 and salesorders.accountid = ".$id;
		renderRelatedOrders($query,$id);	
	}
	function get_tickets($id)
	{
		$query = "select users.user_name, users.id, troubletickets.title, troubletickets.ticketid as crmid, troubletickets.status, troubletickets.priority, troubletickets.parent_id, crmentity.smownerid, crmentity.modifiedtime from troubletickets inner join crmentity on crmentity.crmid = troubletickets.ticketid left join custbranch on custbranch.accountid=troubletickets.parent_id left join users on users.id=crmentity.smownerid where custbranch.accountid =".$id;
		$query .= " union all ";
		$query .= "select users.user_name, users.id, troubletickets.title, troubletickets.ticketid as crmid, troubletickets.status, troubletickets.priority, troubletickets.parent_id, crmentity.smownerid, crmentity.modifiedtime from troubletickets inner join crmentity on crmentity.crmid = troubletickets.ticketid left join contactdetails on contactdetails.contactid = troubletickets.parent_id left join custbranch on custbranch.accountid=contactdetails.accountid left join users on users.id=crmentity.smownerid where custbranch.accountid =".$id;
		renderRelatedTickets($query,$id);
	}
	function get_products($id)
	{
		$query = 'select stockmaster.productid, stockmaster.description, stockmaster.stockid, stockmaster.commissionrate, stockmaster.qty_per_unit, stockmaster.unit_price, crmentity.crmid, crmentity.smownerid from stockmaster inner join seproductsrel on stockmaster.productid = seproductsrel.productid inner join crmentity on crmentity.crmid = stockmaster.productid inner join custbranch on custbranch.accountid = seproductsrel.crmid  where custbranch.accountid = '.$id.' and crmentity.deleted = 0';
	...
	{
		$query = "update custbranch set parent_id='$account_id' where id='$member_id' and deleted=0";
	...
	{
		$query = "update custbranch set parent_id='' where parent_id='$account_id' and deleted=0";
	...
	{
		$query = "update custbranch set parent_id='' where id='$account_id' and deleted=0";
	...
	function fill_in_additional_detail_fields()
	{
		...
		//$query = "SELECT a1.name from custbranch as a1, accounts as a2 where a1.id = a2.parent_id and a2.id = '$this->id' and a1.deleted=0";
                $query = "SELECT a1.name from custbranch a1, custbranch a2 where a1.id = a2.parent_id and a2.id = '$this->id' and a1.deleted=0";
	...
	*/
	...
	array_push($where_clauses, "brname like '$the_query_string%'");
	if (is_numeric($the_query_string)) {
		array_push($where_clauses, "otherphone like '%$the_query_string%'");
		array_push($where_clauses, "faxno like '%$the_query_string%'");
		array_push($where_clauses, "phoneno like '%$the_query_string%'");
	}
...
return $exists;
  ...
  $query = $this->constructCustomQueryAddendum() . ", 
			custbranch.*, ".$this->entity_table.".*, custbranch.braddress3 billing_city, custbranch.braddress5  billing_country, custbranch.braddress6  billing_code, custbranch.braddress4  billing_state, custbranch.braddress1  billing_street, custbranch.braddress2  billing_street2, custbranch.brpostaddr3  shipping_city, custbranch.brpostaddr5  shipping_country, custbranch.brpostaddr6  shipping_code, custbranch.brpostaddr4  shipping_state,  custbranch.brpostaddr1  shipping_street,  custbranch.brpostaddr2  shipping_street2,
                        users.user_name, users.status  user_status
                        FROM ".$this->entity_table."
                        INNER JOIN custbranch
                        ON crmentity.crmid=custbranch.accountid
                        LEFT JOIN accountscf 
                        ON accountscf.accountid=custbranch.accountid
                        LEFT JOIN users
