...
echo '<A HREF="'. $rootpath . '/PO_SelectOSPurchOrder2.php?' . SID . '">' . _('Back to Purchase Orders'). '</A><BR>';

if ($_GET['PONumber']<=0 AND !isset($_SESSION['PO'])) {
	/* This page can only be called with a purchase order number for invoicing*/
	echo '<CENTER><A HREF="' . $rootpath . '/PO_SelectOSPurchOrder2.php?' . SID . '">'.
...
/*first the debit using the GLCode in the PO detail record entry*/
	...
	echo "<A HREF='$rootpath/PO_SelectOSPurchOrder2.php?" . SID . "'>" . _('Select a different purchase order for receiving goods against'). '</A>';
