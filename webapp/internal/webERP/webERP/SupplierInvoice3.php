...
if (isset($_POST['ExRate'])){
	...
	$_SESSION['SuppTrans']->InvoiceType = $_POST['InvoiceType'];
...
if the link is not active then OvAmount must be entered manually. */
		...
		if (count($_SESSION['SuppTrans']->ProdRuns) > 0){
			foreach ( $_SESSION['SuppTrans']->ProdRuns as $ProdRunsLine){
				$_SESSION['SuppTrans']->OvAmount = $_SESSION['SuppTrans']->OvAmount + $ProdRunsLine->Amount;
			}
		}
...
if (!isset($_POST['PostInvoice'])){
	...
	if (count( $_SESSION['SuppTrans']->ProdRuns) > 0){   /*if there are any Shipment charges on the invoice*/
	...
	if ( $_SESSION['SuppTrans']->GLLink_Creditors == 1){
		...
		if (count($_SESSION['SuppTrans']->GLCodes) > 0){
			...
			foreach ($_SESSION['SuppTrans']->GLCodes as $EnteredGLCode){
				...
				echo '<TR><TD>' . $EnteredGLCode->GLCode . '</TD><TD>' . $EnteredGLCode->GLActName .
					...
					'</TD><TD>' . $EnteredGLCode->ShiptRef . '</TD><TD>' .$EnteredGLCode->ProdRunsRef .
		...
		$_SESSION['SuppTrans']->OvAmount = $TotalGRNValue + $TotalGLValue + $TotalShiptValue + $TotalProdRunsValue;
	...
	echo "<TR><TD COLSPAN=2></TD><TD><SELECT NAME='InvoiceType'>";
	if (!isset($_SESSION['SuppTrans']->InvoiceType) || ($_SESSION['SuppTrans']->InvoiceType=="")){
	...
	elseif ($_SESSION['SuppTrans']->InvoiceType=="Brokerage"){
	...
	elseif ($_SESSION['SuppTrans']->InvoiceType=="Fair Trade Premium"){
	...
	elseif ($_SESSION['SuppTrans']->InvoiceType=="Shipping"){
	...
	elseif ($_SESSION['SuppTrans']->InvoiceType=="Production Run Fees"){
...
/*Need to recalc the taxtotal */
	...
	}elseif ( $_SESSION['SuppTrans']->OvAmount < round($TotalShiptValue + $TotalProdRunsValue + $TotalGLValue + $TotalGRNValue,2)){
	...
	/*Start an SQL transaction */
		...
		$InvoiceType1 = $_SESSION['SuppTrans']->InvoiceType;
		...
		/*Loop through the GL Entries and create a debit posting for each of the accounts entered */
			...
			*/
			...
			the credit is to creditors control act  done later for the total invoice value + tax*/
				...
				$SQL = 'INSERT INTO gltrans (type, 
						...
						VALUES (20, ' .
							...
							'" . $EnteredGLCode->JobRef . "', '" . $InvoiceType1 . "')";
			...
			these entries are reversed from the GRN suspense when the shipment is closed*/
				...
				$SQL = 'INSERT INTO gltrans (type, 
							...
							VALUES (20, ' .
								...
								" . $ShiptChg->Amount/ $_SESSION['SuppTrans']->ExRate . ', '" . $InvoiceType1 . "')';
			...
			foreach ($_SESSION['SuppTrans']->ProdRuns as $ProdRunsChg){
			...
			these entries are reversed from the GRN suspense when the shipment is closed*/
				...
				$SQL = 'INSERT INTO gltrans (type, 
							...
							VALUES (20, ' .
								...
								" . $PeriodNo . ', 
								' . $_SESSION['SuppTrans']->GRNAct . ", 
								'" . $_SESSION['SuppTrans']->SupplierID . ' ' . _('Production Run charge against') . ' ' . $ProdRunsChg->ProdRunsRef . "', 
								" . $ProdRunsChg->Amount/ $_SESSION['SuppTrans']->ExRate . ', '" . $InvoiceType1 . "')';
				...
				$LocalTotal += $ProdRunsChg->Amount/ $_SESSION['SuppTrans']->ExRate;
//prnMsg(_('ShiptRef ') . ' ' . $_SESSION['SuppTrans']->ShiptRef . ' ' . _(' STAGE 1'),'success');
//prnMsg(_('ProdRunsRef ') . ' ' . $_SESSION['SuppTrans']->ProdRunsRef . ' ' . _(' STAGE 1'),'success');
				  ...
				  enter the GL entry to reverse the GRN suspense entry created on delivery at standard cost used on delivery */
						...
						$SQL = 'INSERT INTO gltrans (type, 
								...
								VALUES (20, ' . $InvoiceNo . ", 
								 ...
								 _('std cost of') . ' ' . $EnteredGRN->StdCostUnit  . "', 
								 	" . $EnteredGRN->StdCostUnit * $EnteredGRN->This_QuantityInv . ', '" . $InvoiceType1 . "')';
								...
								*/
								...
								if ($EnteredGRN->This_QuantityInv > $TotalQuantityOnHand){
									...
									* (($EnteredGRN->ChgPrice  / $_SESSION['SuppTrans']->ExRate) - $EnteredGRN->StdCostUnit);
									...
									VALUES (20, ' .
									 ...
									 "', " . $WriteOffToVariances . ', '" . $InvoiceType1 . "')';
								...
								$SQL = 'INSERT INTO gltrans (type, 
									...
									VALUES (20, ' .
									 ...
									 number_format(($EnteredGRN->ChgPrice  / $_SESSION['SuppTrans']->ExRate) - $EnteredGRN->StdCostUnit,2)  .
									 "', " . ($PurchPriceVar - $WriteOffToVariances) . ', '" . $InvoiceType1 . "')';
					...
					$SQL = 'INSERT INTO gltrans (type, 
							...
							VALUES (20, ' .
							 ...
							 round(($EnteredGRN->ChgPrice * $EnteredGRN->This_QuantityInv) / $_SESSION['SuppTrans']->ExRate,2) . ', '" . $InvoiceType1 . "')';
			...
			foreach ($_SESSION['SuppTrans']->Taxes as $Tax){
				/* Now the TAX account */
						...
						VALUES (20, ' .
						 ...
						 "',
						 	" . round( $Tax->TaxOvAmount/ $_SESSION['SuppTrans']->ExRate,2) . ', '" . $InvoiceType1 . "')';
			...
			$SQL = 'INSERT INTO gltrans (type,
					...
					VALUES (20, ' .
					 ...
					 -round(($LocalTotal + ( $TaxTotal / $_SESSION['SuppTrans']->ExRate)),2) . ', '" . $InvoiceType1 . "')';
	...
	/*Now insert the invoice into the SuppTrans table*/
		...
		$SQL = 'INSERT INTO supptrans (transno, 
			...
			VALUES ('. $InvoiceNo . ",
				...
				' . round($TaxTotal,2) . ', 
				' .  $_SESSION['SuppTrans']->ExRate . ", 
				'" . $_SESSION['SuppTrans']->Comments . "', '" . $InvoiceType1 . "')";
...
// prnMsg( _('ProdRunRef sql ') . $SQL,'info');
		...
		foreach ($_SESSION['SuppTrans']->Shipts as $ShiptChg){
			...
			$DbgMsg = _('The following SQL to insert the Shipment charge record was used');
// prnMsg(_('Shipment ') . ' ' . $_SESSION['SuppTrans']->Shipts . ' ' . _('has been processed'),'success');
		prnMsg(_('Prod Run ') . ' ' . $SQL . ' ' . _('has been processed'),'success');
	...
	// print_r($_SESSION['SuppTrans']->ProdRuns);
...
{$EnteredGRN->PRNo=0;}
			...
			$SQL = 'INSERT INTO productionruncostingcharges (prref, 
					...
					VALUES (' . $ProdRunsChg->ProdRunsRef . ', 
						20, 
						' . $InvoiceNo . ', 
						' . $ProdRunsChg->Amount/ $_SESSION['SuppTrans']->ExRate . ',
						' . $EnteredGRN->PRNo . ')';
			...
			$DbgMsg = _('The following SQL to insert the Shipment charge record was used');
//prnMsg(_('Prod Run ') . ' ' . $_SESSION['SuppTrans']->ProdRuns . ' ' . _('has been processed'),'success');
		// prnMsg(_('Prod Run ') . ' ' . $SQL . ' ' . _('has been processed'),'success');
		...
		unset( $_SESSION['SuppTrans']->ProdRuns);
		unset( $_SESSION['SuppTrans']->GLCodes);
		unset( $_SESSION['SuppTrans']);
