		...
		// Test above to see if the line is completed or not
			...
			if ($MBFlag=="B" OR $MBFlag=="M") {
				...
				$SQL = "SELECT bom.component,
						bom.quantity,
						stockmaster.materialcost+stockmaster.overheadcost AS standard
					FROM bom,
						stockmaster
...
/*first the cost of sales entry*/
				...
				$SQL ="UPDATE custbranch set salesman='S99' WHERE branchcode=  '" . $_SESSION['Items']->Branch . "' ";
...
$orderID = $_SESSION['ProcessingOrder'];
$crmid = getSOID($orderID);

if ($delivered==1){
$SQL="UPDATE salesorders
	SET sostatus='Delivered'
	WHERE salesorderid= " . $crmid;
	} else {
$SQL="UPDATE salesorders
	SET sostatus='Partial Delivery'
	WHERE salesorderid= " .  $crmid;
	}
$ErrMsg = _(' ERROR') . '! ' . _('NOT CRITICAL, BUT SEEK ASSISTANCE') . ': ' . _('Sales Order status could not be updated ');
				$DbgMsg = _('The following SQL to insert the salesorders record was used');
				$Result = DB_query($SQL,$db,$ErrMsg,$DbgMsg,true);	
	
$SQL = "UPDATE crmentity SET description = CONCAT(description,' Invoice ','" . $InvoiceNo . "') WHERE crmid= " . $crmid;
$ErrMsg = _(' ERROR') . '! ' . _('NOT CRITICAL, BUT SEEK ASSISTANCE') . ': ' . _('Sales Order description could not be updated ');
				$DbgMsg = _('The following SQL to insert the crmentity record was used');
				$Result = DB_query($SQL,$db,$ErrMsg,$DbgMsg,true);	
	...
	$Result = DB_query($SQL,$db);
       /*Virtue Mart*/
       $SQL = "UPDATE jos_vm_orders SET order_status='C' where order_id=" . $orderID ;
        $Result =DB_query($SQL,$db);
...
Function GetSOID ($orderID){

/*Gets the Product ID for VTiger */
	$Cresult = mysql_query('SELECT salesorderid FROM salesorders WHERE orderno = "'.$orderID.'" ');
if (!$Cresult) {
   die('Could not query:' . mysql_error());
}

$crmid = mysql_result($Cresult, 0); // outputs current crmid
