...
$SQL = "SELECT grnbatch,
		...
		purchorderdetails.prref,
		purchorderdetails.prno,
...
/*	if ($myrow['closed']==1){ //Shipment is closed so pre-empt problems later by warning the user - need to modify the order first
		...
		echo "<INPUT TYPE=HIDDEN NAME='ProdRunsRef' Value='" . $GRNTmp->ProdRunsRef . "'>";
	echo "<INPUT TYPE=HIDDEN NAME='PRNo' Value='" . $GRNTmp->PRNo . "'>";
