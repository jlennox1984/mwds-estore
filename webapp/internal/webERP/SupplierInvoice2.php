...
if (!isset($_POST['PostInvoice'])){
	...
	if ($_POST['GRNS'] == _('Enter Against Goods Recd')){
		...
		echo "<META HTTP-EQUIV='Refresh' CONTENT='0; URL=" . $rootpath . "/SuppInvGRNs2.php?" . SID . "'>";
		echo '<P>' . _('You should automatically be forwarded to the entry of invoices against goods received page') .
			'. ' . _('If this does not happen') .' (' . _('if the browser does not support META Refresh') . ') ' .
			"<A HREF='" . $rootpath . "/SuppInvGRNs2.php?" . SID . "'>" . _('click here') . '</a> ' . _('to continue') . '.<BR>';
		exit;
	}
	...
	if (count( $_SESSION['SuppTrans']->GRNs)>0){   /*if there are any GRNs selected for invoicing then */
		...
		$TotalGRNValue = 0;
		prnMsg(_('TEST0') . '<BR>' . $TotalGRNValue,'warn');
	...
	prnMsg(_('TEST') . '<BR>' . $TotalGRNValue . ' -- ' . $EnteredGRN->ChgPrice . ' -- ' . $EnteredGRN->This_QuantityInv,'warn');
			$i++;
			if ($i > 15){
				$i = 0;
	...
	prnMsg(_('TEST0a') . '<BR>' . $TotalGRNValue,'warn');
		if (!isset($TotalGRNValue)){
			$TotalGRNValue = 0;
			prnMsg(_('TEST1') . '<BR>' . $TotalGRNValue,'warn');
		}
		...
		prnMsg(_('TEST2') . '<BR>' . $TotalGRNValue . ' -- ' . $_SESSION['SuppTrans']->OvAmount,'warn');
...
/*Need to recalc the taxtotal */
	...
	}elseif ( $_SESSION['SuppTrans']->OvAmount < $TotalGRNValue){
	prnMsg(_('TEST3') . '<BR>' . $TotalGRNValue . ' -- ' . $_SESSION['SuppTrans']->OvAmount,'warn');
		
		prnMsg( _('The invoice total as entered is less than the sum of the shipment charges, the general ledger entires (if any) and the charges for goods received') . '. ' .  $_SESSION['SuppTrans']->OvAmount . '--' . $TotalGRNValue . _('There must be a mistake somewhere, the invoice as entered will not be processed'),'error');
		$InputError = True;
...
//prnMsg(_('Prod Run ') . ' ' . $_SESSION['SuppTrans']->ProdRuns . ' ' . _('has been processed'),'success');
		...
		$SQL="COMMIT";
		 prnMsg(_('SUCCESS '). _('has been processed'),'success');
