...
if (isset($_POST['UpdateRates'])){
  ...
  *
  */
		//TODO: put in loop control
		
		$id= $_POST['TaxAuthority'];
//GET STATE AND CONUTRY VARABLES
		              $locationresult=DB_query('select state_abr,country_abr from taxauthorities where taxid='. $id ,$db );
		              $myrow3 = mysql_fetch_array($locationresult); 
		              $_POST['state']= $myrow3['state_abr'];
		              $_POST['country']= $myrow3['conutry_abr'];
		               $state=$myrow3['state_abr'];
		               $country=$myrow3['country_abr'];
//ok we have defined those varables
		
	$SQL1= 'UPDATE jos_vm_tax_rate SET tax_rate='. ($_POST[$myrow['dispatchtaxprovince'] . '_' . $myrow['taxcatid']]/100)  .' where  tax_state="' . $state .'" AND tax_country=" '. $country. '"';
	DB_query($SQL1,$db);
	
	}
	prnMsg(_('All rates updated successfully'),'info');
prnMsg( _('State = ' .$state. "));
	prnMsg( _('country = ' .$checkno."));
}
