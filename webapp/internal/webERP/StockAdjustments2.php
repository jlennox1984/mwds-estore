...
include('includes/DefineStockAdjustment2.php');
...
if (isset($_GET['StockID'])){
	...
	$_SESSION['Adjustment']->AdjDate = $_POST['AdjDate'];
...
if ($NewAdjustment){
	...
	$sql ="SELECT description,
				units,
				mbflag,
				materialcost+overheadcost as standardcost,
				controlled,
				serialised,
				decimalplaces
			FROM stockmaster
...
/*All inputs must be sensible so make the stock movement records and update the locations stocks */
		...
		$AdjustmentDate=$_SESSION['Adjustment']->AdjDate;
		$PeriodNo = GetPeriod (Date($AdjustmentDate), $db);
		$SQLAdjustmentDate = FormatDateForSQL(Date($AdjustmentDate));
...
echo '<TR><TD>'. _('Date of Adjustment (dd/mm/yyyy)').':</TD>
	<TD><input type=text name="AdjDate" size=32 maxlength=30 value="' . $_SESSION['Adjustment']->AdjDate . '"></TD></TR>';
...
if ($_SESSION['Adjustment']->Controlled==1){
		if ($_SESSION['Adjustment']->StockLocation != ''){
			echo '<INPUT TYPE="HIDDEN" NAME="Quantity" Value="' . $_SESSION['Adjustment']->Quantity . '">
				'.$_SESSION['Adjustment']->Quantity.' &nbsp; &nbsp; &nbsp; &nbsp;
				[<A HREF="'.$rootpath.'/StockAdjustmentsControlled2.php?AdjType=REMOVE&' . SID . '">'._('Remove').'</A>]
				[<A HREF="'.$rootpath.'/StockAdjustmentsControlled2.php?AdjType=ADD&' . SID . '">'._('Add').'</A>]';
		} else {
