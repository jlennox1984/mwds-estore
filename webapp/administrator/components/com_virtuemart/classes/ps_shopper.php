...
//ERP CODE BEGINS - CREATE CRM ENTRIES FOR CONTACT AND ACCOUNT 

		$db->query( 'UPDATE crmentity_seq SET id = id+1 ' );
		
		$db->query( 'select id from crmentity_seq ' );
		
		$accountid = $db->loadResult();
		$debtorno = $accountid;		

		$db->query( "INSERT INTO crmentity (crmid,smcreatorid,smownerid,setype,description,createdtime,modifiedtime) VALUES ( $accountid,'1','1','Accounts','',(CURRENT_TIMESTAMP),(CURRENT_TIMESTAMP) ) " );
		...
		$db->query( "INSERT INTO crmentity (crmid,smcreatorid,smownerid,setype,description,createdtime,modifiedtime) VALUES ( $contactid,'1','1','Contacts','',(CURRENT_TIMESTAMP),(CURRENT_TIMESTAMP) ) " );
		...
		if (!empty($d['company'])) {
			...
			$company =$d["last_name"] . ', ' . $d["first_name"]; 
		}


 
		if ($d["country"]=="CAN" && $d["state"]=="ON")
		{$taxgroup=1;}

		elseif ($d["country"]=="CAN" && $d["state"]=="QC")
		{$taxgroup=2;}

		elseif ($d["country"]=="CAN" && $d["state"]=="NS")
		{$taxgroup=4;}

		elseif ($d["country"]!= "CAN")
		{$taxgroup=3;}
		...
		$db->query("INSERT INTO accountscf (accountid,cf_374) VALUES ('" .$accountid . "', 'English')");	
	
		

		$db->query("INSERT INTO  contactdetails (contactid, accountid, firstname, lastname, email, phone, currency) VALUES  ( '" . $contactid . "','" . $accountid . "', '" . $d["first_name"] . "', '" . $d["last_name"] . "', '" .$d["email"] . "', '" .$d["phone_1"] . "', 'Dollars')");

 	

		$db->query("INSERT INTO  contactscf (contactid) VALUES  ('" . $contactid . "')");


 	

		$db->query("INSERT INTO  contactsubdetails (contactsubscriptionid, homephone) VALUES  ( '" . $contactid . "', '" .$d["phone_1"] . "')");




		$db->query("INSERT INTO contactaddress   ( contactaddressid, mailingcity, mailingstate, mailingstreet, mailingstreet2, mailingcountry, mailingzip) VALUES ('" . $contactid . "',  '" .$d["city"] . "', '" .$d["state"] . "', '" .$d["address_1"] . "', '" .$d["address_2"] . "', '" .$d["country"] . "', '" .$d["zip"] . "')"); 

 	//important 
            $db->query("INSERT INTO user_rel (userid,debtorid,accountid,contactid) VALUES( '". $uid ."', '". $debtorid . "', '". $accountid ."' , '". $contactid . "')");  //Account Relationship Table J Moncrieff 03/28/2007
