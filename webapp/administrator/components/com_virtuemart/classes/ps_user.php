  ...
  function validate_add(&$d) {
	...
	**************************************************************************/
	function update(&$d) {
		...
		// get user relation ship infomation  J Moncrieff 03/28/2007
                $contactid = $db->query("SELECT contactid FROM user_rel where userid= .$uid. ");

              $accountid = $db->query("SELECT accountid FROM user_rel where userid= .$uid.");
 		$db->query("UPDATE debtorsmaster SET name=". $d["company"] .",address1=" . $d["address_1"] ." , address2= ". $d["address_2"] .", address3=". $d["city"] ." , address4=" .$d["state"] ."  ,address5=". $d["country"] ." address6= " .$d["zip"] ." WHERE debtorno=’WWW .$contactid.? "); //debtor master update
		$db->query("UPDATE custnbranch SET brname=".$d["company"].", braddress1=".$d["address_1"]." , braddress2=".$d["address_2"].", braddress3=". $d["city"] .", braddress4=" .$d["state"]." , braddress5=".$d["country"].",braddress6=".$d["zip"]." ,phoneno=" .$d["phone_1"]. " WHERE branchcode=WWW .$contactid .");//custbranch
	       $db->query("UPDATE contactsubdetails SET homephone=".$d["phone_1"]. ", email=".$d["email"] ." WHERE contactsubscription=.$contatid.");
		$db->query("UPDATE contactaddress SET mailingcity=".$d["city"]. " ,mailingstreet1=".$d["address_1"].",mailingstreet2=" .$d[address_2] .", mailingstate=.$d[state] . , mailingzip=".$d[zip]." WHERE addresscontactid=.$contactid.");
		$db->query($q);
             	}
 		if( $perm->check("admin")) {
			$vendor_id = $d['vendor_id'];
