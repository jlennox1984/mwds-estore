...
if(isset($_REQUEST['query']))
{
	...
	if(isset($name) && $name != '')
	{
		array_push($where_clauses, "tasks.name like '".$adb->quote($name)."%'");
	}
	if(isset($contact_name) && $contact_name != '')
	{
		$contact_names = explode(" ", $contact_name);
		foreach ($contact_names as $name) {
			array_push($where_clauses, "(contacts.first_name like '".$adb->quote($name)."%' OR contacts.last_name like '".$adb->quote($name)."%')");
		}
	}
	if(isset($date_due) && $date_due != '')
	{
		array_push($where_clauses, "tasks.date_due like '".$adb->quote($date_due)."%'");
	}
	if(isset($status) && $status != '')
	{
		...
		do {
			$the_where_clause .= "status = '".$adb->quote($val)."'";
