...
function add_create_account()
		...
		//$query = "select * from {$focus->table_name} WHERE brname='{$acc_name}' left join crmentity on crmentity.crmid =custbranch.accountid";
		$acc_name = addslashes($acc_name);
		$query = "select crmentity.deleted, custbranch.* from custbranch,crmentity WHERE brname='{$acc_name}' and crmentity.crmid =custbranch.accountid";
		...
		// if we didnt find the account, so create it
			...
			$adb->println("Createing new account");
                        $focus->column_fields['brname'] = $acc_name;
