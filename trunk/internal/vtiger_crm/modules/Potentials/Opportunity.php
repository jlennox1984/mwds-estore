...
class Potential extends CRMEntity {
  ...
  var $deleted;
	...
	var $additional_column_fields = Array('assigned_user_name', 'assigned_user_id', 'brname', 'accountid', 'description', 'productid', 'contactid', 'taskid', 'notesid', 'meetingid', 'callid', 'emailid');
	...
	function create_list_query($order_by, $where)
	{
		...
		if($account_required)
		{
			$query = "SELECT potential.potentialid,  potential.potentialname, potential.dateclosed FROM potential, custbranch ";
			$where_auto = "custbranch.accountid = potential.accountid AND crmentity.deleted=0 ";
		}
		else
		{
...
return $exists;
 ...
 $query = $this->constructCustomQueryAddendum() .",                                potential.*,
                                custbranch.brname account_name,
                                users.user_name assigned_user_name
                                FROM potential
                                INNER JOIN crmentity
                                ON crmentity.crmid=potential.potentialid
                                LEFT JOIN custbranch on potential.accountid=custbranch.accountid
		...
		else
		{
                  	$query = "SELECT
                                ...
                                LEFT JOIN custbranch on potential.accountid=custbranch.accountid  LEFT JOIN potentialscf on potentialscf.potentialid=potential.potentialid where crmentity.deleted=0 ";
		}	
	...
	function fill_in_additional_detail_fields()
	{
		...
		$query = "SELECT acc.accountid, acc.brname from custbranch acc, potential pt where acc.accountid = pt.accountid and pt.potentialid = '$this->potentialid' and pt.deleted=0";
		$result =& $this->db->query($query, true,"Error filling in additional detail fields: ");
		...
		if($row != null)
		{
			$this->brname = stripslashes($row['brname']);
		...
		$query = "SELECT pr.productid, pr.description from stockmaster pr, potential pt where pr.productid = pt.productid and pt.potentialid = '$this->potentialid' and pr.deleted=0 and pt.deleted=0";
		$result =& $this->db->query($query, true,"Error filling in additional detail fields: ");
		...
		if($row != null)
		{
			$this->productname = stripslashes($row['description']);
	...
	*/
	...
	{
		//$query = 'select potential.potentialid, potential.potentialname, stockmaster.productid, stockmaster.description, stockmaster.qty_per_unit, stockmaster.unit_price, stockmaster.purchase_date from potential inner join crmentity on crmentity.crmid = potential.potentialid inner join seproductsrel on seproductsrel.productid = stockmaster.productid inner join stockmaster on seproductsrel.productid = stockmaster.productid where potential.potentialid='.$id;
		$query = 'select stockmaster.productid, stockmaster.description, stockmaster.stockid, stockmaster.commissionrate, stockmaster.qty_per_unit, stockmaster.unit_price, crmentity.crmid, crmentity.smownerid from stockmaster inner join seproductsrel on stockmaster.productid = seproductsrel.productid inner join crmentity on crmentity.crmid = stockmaster.productid inner join potential on potential.potentialid = seproductsrel.crmid  where potential.potentialid = '.$id.' and crmentity.deleted = 0';
	...
	function get_quotes($id)
	{
	 ...
	 {
	 	$query = "select crmentity.*, salesorders.*, quotes.subject as quotename, custbranch.brname, potential.potentialname from salesorders inner join crmentity on crmentity.crmid=salesorders.salesorderid left outer join quotes on quotes.quoteid=salesorders.quoteid left outer join custbranch on custbranch.accountid=salesorders.accountid left outer join potential on potential.potentialid=salesorders.potentialid where crmentity.deleted=0 and potential.potentialid = ".$id;
