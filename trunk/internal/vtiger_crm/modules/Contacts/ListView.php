...
//upto this added for Custom Field
	...
	if(isset($accountname) && $accountname != "")	{
			array_push($where_clauses, "custbranch.brname like ".$adb->quote($accountname.'%')."");
	...
	if(isset($phone) && $phone != "") {
			array_push($where_clauses, "(contactdetails.phone like ".$adb->quote('%'.$phone.'%')." OR contactdetails.mobile like ".$adb->quote('%'.$phone.'%')." OR contactdetails.fax like ".$adb->quote('%'.$phone.'%').")");
			$url_string .= "&phone=".$phone;
	}
	if(isset($email) && $email != "") {
			array_push($where_clauses, "(contactdetails.email like ".$adb->quote($email.'%').")");
			$url_string .= "&email=".$email;
	}
	if(isset($yahooid) && $yahooid != "") {
			array_push($where_clauses, "contactdetails.yahooid like ".$adb->quote($yahooid.'%')."");
			$url_string .= "&yahooid=".$yahooid;
	}
	if(isset($assistant) && $assistant != "") {
			array_push($where_clauses, "contactsubdetails.assistant like ".$adb->quote($assistant.'%')."");
			$url_string .= "&yahooid=".$yahooid;
	}
	if(isset($mailingstreet) && $mailingstreet != "") {
			array_push($where_clauses, "(contactaddress.mailingstreet like ".$adb->quote($mailingstreet.'%')." OR contactaddress.otherstreet like ".$adb->quote($mailingstreet.'%').")");
			$url_string .= "&mailingstreet=".$mailingstreet;
	}
	if(isset($mailingcity) && $mailingcity != "") {
			array_push($where_clauses, "(contactaddress.mailingcity like ".$adb->quote($mailingcity.'%')." OR contactaddress.othercity like ".$adb->quote($mailingcity.'%').")");
			$url_string .= "&mailingcity=".$mailingcity;
	}
	if(isset($mailingstate) && $mailingstate != "") {
			array_push($where_clauses, "(contactaddress.mailingstate like ".$adb->quote($mailingstate.'%')." OR contactaddress.otherstate like ".$adb->quote($mailingstate.'%').")");
			$url_string .= "&mailingstate=".$mailingstate;
	}
	if(isset($mailingzip) && $mailingzip != "") {
			array_push($where_clauses, "(contactaddress.mailingzip like ".$adb->quote($mailingzip.'%')." OR contactaddress.otherzip like ".$adb->quote($mailingzip.'%').")");
			$url_string .= "&mailingzip=".$mailingzip;
	}
	if(isset($mailingcountry) && $mailingcountry != "") {
			array_push($where_clauses, "(contactaddress.mailingcountry like ".$adb->quote($mailingcountry.'%')." OR contactaddress.othercountry like ".$adb->quote($mailingcountry.'%').")");
