...
function getOutputHtml($uitype, $fieldname, $fieldlabel, $maxlength, $col_fields,$generatedtype)
{
	...
	{	
		...
		if($uitype == 5)
			$custfld .= '<input type="hidden" name="deliverydate" value="'.$disp_value.'">';
	...
	#$vtlog->logthis("uitype selected is  ".$uitype,'info');  
		...
		$custfld .= $mod_strings[$fieldlabel].':</td>';
		$pick_query="select shipper_id, shippername from shippers";
		$pickListResult = $adb->query($pick_query);
		$noofpickrows = $adb->num_rows($pickListResult);
		$custfld .= '<td width="30%"><select name="'.$fieldname.'">';
		for($j = 0; $j < $noofpickrows; $j++)
		{
			$pickListValue=$adb->query_result($pickListResult,$j,shipper_id);
			$pickListDisplay=$adb->query_result($pickListResult,$j,shippername);
			...
			$custfld .= '<OPTION value="'.$pickListValue.'" '.$chk_val.'>'.$pickListDisplay.'</OPTION>';
		}
		$custfld .= '</td>';
	}
	elseif($uitype == 32)
	...
	#$vtlog->logthis("uitype selected is  ".$uitype,'info');  
		...
		$custfld .= $mod_strings[$fieldlabel].':</td>';
		$pick_query="SELECT locationname,loccode FROM locations";
		$pickListResult = $adb->query($pick_query);
		$noofpickrows = $adb->num_rows($pickListResult);
		$custfld .= '<td width="30%"><select name="'.$fieldname.'">';
		for($j = 0; $j < $noofpickrows; $j++)
		{
			$pickListValue=$adb->query_result($pickListResult,$j,loccode);
			$pickListDisplay=$adb->query_result($pickListResult,$j,locationname);

			if(($value == '') || (!isset($value)))
			{if ($pickListValue == 'MILL')
				{$chk_val = "selected";}
			 else
				{$chk_val = '';}
			}
	...
	#$vtlog->logthis("uitype selected is  ".$uitype,'info');  
		...
		$custfld .= '<font color="red">*</font>';
		$custfld .= $mod_strings[$fieldlabel].':</td>';
		$custfld .= '<td width="30%"><select name="deliverblind">';
		$custfld .= '<option value="1" selected="selected">Show Company Details/Logo</option><option value="2">Hide Company Details/Logo</option></select>';
		$custfld .= '</td>';
	}
	elseif($uitype == 34)
	{
	#$vtlog->logthis("uitype selected is  ".$uitype,'info');  
		$custfld .= '<td width="20%" class="dataLabel"></td>';
		$custfld .= '<td width="30%"><input name="quotation" type="hidden" value="0">';
		$custfld .= '</td>';
	}
	elseif($uitype == 35)
	{
	$custfld .= '<input name="ordertype" type="hidden" value="DE">';
		
	}
	elseif($uitype == 37)
	{
	#$vtlog->logthis("uitype selected is  ".$uitype,'info');  
		$custfld .= '<td width="20%" class="dataLabel"><input name="description" type="hidden" value="'.$value.'">'.$mod_strings[$fieldlabel].':</td>';
		$custfld .= '<td width="30%"><b>'.$value.'</b>';
		$custfld .= '</td>';
	}
	elseif($uitype == 38)
	{
	#$vtlog->logthis("uitype selected is  ".$uitype,'info');  
		$custfld .= '<td width="20%" class="dataLabel"><input name="stockid" type="hidden" value="'.$value.'">'.$mod_strings[$fieldlabel].':</td>';
		$custfld .= '<td width="30%"><b>'.$value.'</b>';
		$custfld .= '</td>';
	}
	elseif($uitype == 39)
	{
	#$vtlog->logthis("uitype selected is  ".$uitype,'info');  
		$custfld .= '<td width="20%" class="dataLabel"><input name="units" type="hidden" value="'.$value.'">'.$mod_strings[$fieldlabel].':</td>';
		$custfld .= '<td width="30%"><b>'.$value.'</b>';
		$custfld .= '</td>';
	}
	elseif($uitype == 40)
	{
	#$vtlog->logthis("uitype selected is  ".$uitype,'info');  
		$custfld .= '<td width="20%" class="dataLabel"><input name="lowestlevel" type="hidden" value="'.$value.'">'.$mod_strings[$fieldlabel].':</td>';
		$custfld .= '<td width="30%"><b>'.$value.'</b>';
		$custfld .= '</td>';
	}
	elseif($uitype == 41)
	...
	#$vtlog->logthis("uitype selected is  ".$uitype,'info'); 
		...
		$custfld .= '<td width="20%" class="dataLabel"><input name="categoryid" type="hidden" value="'.$value.'">'.$mod_strings[$fieldlabel].':</td>';
		$custfld .= '<td width="30%"><b>'.$category.'</b>';
		$custfld .= '</td>';
	}
	elseif($uitype == 42)
	{
	#$vtlog->logthis("uitype selected is  ".$uitype,'info');  
		...
		$custfld .= $mod_strings[$fieldlabel].':</td>';
		$pick_query="SELECT * FROM areas";
		$pickListResult = $adb->query($pick_query);
		$noofpickrows = $adb->num_rows($pickListResult);
		$custfld .= '<td width="30%"><select name="'.$fieldname.'">';
		for($j = 0; $j < $noofpickrows; $j++)
		{
			$pickListValue=$adb->query_result($pickListResult,$j,areacode);
			$pickListDisplay=$adb->query_result($pickListResult,$j,areadescription);
	...
	global $current_user;
		$assigned_user_id1 = $current_user->id;
//		$userid1 = $_REQUEST['assigned_user_id'];
	$salesperson = getSalesperson($assigned_user_id1);
	$salesmancode = getSalespersonCode($assigned_user_id1);
	$custfld .= '<td width="20%" class="dataLabel"><input name="salesman" type="hidden" value="'.$salesmancode.'" >'.$mod_strings[$fieldlabel].':</td>';
		$custfld .= '<td width="30%"><b>'.$salesperson.'</b>';
		$custfld .= '</td>';
	}
	elseif($uitype == 44)
	{
	#$vtlog->logthis("uitype selected is  ".$uitype,'info');  
		...
		for($j = 0; $j < $noofpickrows; $j++)
		{
			...
			$custfld .= '<OPTION size="10" value="'.$pickListValue.'" '.$chk_val.'>'.$pickListDisplay.'</OPTION>';
		}
		$custfld .= '</td>';
	}
	elseif($uitype == 45)
	...
	#$vtlog->logthis("uitype selected is  ".$uitype,'info');  
		...
		$custfld .= $mod_strings[$fieldlabel].':</td>';
		$pick_query="SELECT * FROM taxgroups";
		$pickListResult = $adb->query($pick_query);
		$noofpickrows = $adb->num_rows($pickListResult);
		$custfld .= '<td width="30%"><select name="'.$fieldname.'">';
		for($j = 0; $j < $noofpickrows; $j++)
		{
			$pickListValue=$adb->query_result($pickListResult,$j,taxgroupid);
			$pickListDisplay=$adb->query_result($pickListResult,$j,taxgroupdescription);
	...
	#$vtlog->logthis("uitype selected is  ".$uitype,'info');  
		...
		$custfld .= $mod_strings[$fieldlabel].':</td>';
		$pick_query="SELECT * FROM shippers";
		$pickListResult = $adb->query($pick_query);
		$noofpickrows = $adb->num_rows($pickListResult);
		$custfld .= '<td width="30%"><select name="'.$fieldname.'">';
		for($j = 0; $j < $noofpickrows; $j++)
	...
	#$vtlog->logthis("uitype selected is  ".$uitype,'info');  
		...
		$custfld .= '<input name="deliverto" type="hidden" value="'.$value.'" >';
	}
	elseif($uitype == 48)
	{
	#$vtlog->logthis("uitype selected is  ".$uitype,'info');  
		...
		$custfld .= $mod_strings[$fieldlabel].':</td>';
		$pick_query="SELECT * FROM currencies";
		$pickListResult = $adb->query($pick_query);
		$noofpickrows = $adb->num_rows($pickListResult);
		$custfld .= '<td width="30%"><select name="'.$fieldname.'">';
		for($j = 0; $j < $noofpickrows; $j++)
		{
			$pickListValue=$adb->query_result($pickListResult,$j,currabrev);
			$pickListDisplay=$adb->query_result($pickListResult,$j,currency);

			if(($value == '') || (!isset($value)))
			{if ($pickListValue == 'CDN')
				{$chk_val = "selected";}
			 else
				{$chk_val = '';}
			}
	...
	#$vtlog->logthis("uitype selected is  ".$uitype,'info');  
		...
		$custfld .= $mod_strings[$fieldlabel].':</td>';
		//$pick_query="select * from `stockcategory` ";
		$pick_query="select * from `stockcategory` ";
		$pickListResult = $adb->query($pick_query);
		$noofpickrows = $adb->num_rows($pickListResult);
		$custfld .= '<td width="30%"><select name="'.$fieldname.'">';
		...
		{
			$pickListValue=$adb->query_result($pickListResult,$j,strtolower($fieldname));
			$pickListValueDisplay=$adb->query_result($pickListResult,$j,'categorydescription');
			...
			$custfld .= '<OPTION value="'.$pickListValue.'" '.$chk_val.'>'.$pickListValueDisplay.'</OPTION>';
		}
		$custfld .= '</td>';
	}
	elseif($uitype == 14)
	...
	#$vtlog->logthis("uitype selected is  ".$uitype,'info');  
		...
		$custfld .= $mod_strings[$fieldlabel].':</td>';
		//$pick_query="select * from `taxcategories` ";
		$pick_query="select * from `taxcategories` ";
		$pickListResult = $adb->query($pick_query);
		$noofpickrows = $adb->num_rows($pickListResult);
		$custfld .= '<td width="30%"><select name="'.$fieldname.'">';
		...
		{
			$pickListValue=$adb->query_result($pickListResult,$j,strtolower($fieldname));
			$pickListValueDisplay=$adb->query_result($pickListResult,$j,'taxcatname');
	...
	elseif($uitype == 28)
	{
		$stock_id = $col_fields['stockid'];
       $stockValue = getStock($stock_id);
       $demandValue = getDemand($stock_id);
       $availValue = $stockValue - $demandValue;
         
		$custfld .= '<td width="20%" class="dataLabel" valign="top">'.$mod_strings[$fieldlabel].':</td>';
		$custfld .= '<td> '.$availValue.'</td>';
	}
	elseif($uitype == 128)
	{
		  
		$custfld .= '<td width="20%" class="dataLabel" valign="top"></td>';
		$custfld .= '<td></td>';
	}
	elseif($uitype == 29)
	{
	...
	elseif($uitype == 21 || $uitype == 24)
	{
		...
		$custfld .= $mod_strings[$fieldlabel].':</td>';
        	$custfld .= '<td><input name="'.$fieldname.'" type="text" value="'.$value.'" size="25" maxlength="200"></td>';
	}
	elseif($uitype == 25)
	{
		$custfld .= '<td width="20%" class="dataLabel" valign="top">';
	...
	elseif($uitype == 26)
	{
		$custfld .= '<td width="20%" class="dataLabel" valign="top">';
        $custfld .= '</td>';
        	$custfld .= '<td><input type="text" name="'.$fieldname.'" value="'.$value.'" size="25" maxlength="200"></td>';
	}
	elseif($uitype == 22)
	{
...
//          $GROUP_SELECT_OPTION .='<option value=none>'.$app_strings['LBL_NONE_NO_LINE'].'</option>';
	...
	elseif($uitype == 51 || $uitype == 50 || $uitype == 73)
	{
		...
		if(isset($_REQUEST['record'])|| $_REQUEST['record'] != '')
		{
                	if(isset($_REQUEST['account_id']) && $_REQUEST['account_id'] != '')
                        	$value = $_REQUEST['account_id'];	
							$record1 = $col_fields["record_id"];
				
							$account_name = getAccountName($value);	
							$branchcode = getCustBranch($value);
							$debtor_no = getDebtorNo($branchcode);
							$holdreason = getHoldReason($debtor_no);
							$CreditAvailable = getCreditAvailable($debtor_no);

							$order_no_spec = getOrderNoSpec($record1);
		}

		else
		{	$debug4 = getDebug($_REQUEST['record'],$_REQUEST['account_id'],$value);
			$branchcode = '';
			$debtor_no = '';
			$order_no_spec = '';
			...
			$custfld .= '<td width="30%" valign="top"  class="dataField"><input readonly name="account_name" type="text" value="'.$account_name.'"><input name="account_id" type="hidden" value="'.$value.'"><input name="holdreason" type="hidden" value="'.$holdreason.'"><input name="availablecredit" type="hidden" value="'.$CreditAvailable.'"><input name="orderno" type="hidden" value="'.$order_no_spec.'"><input name="debtorno" type="hidden" value="'.$debtor_no.'"><input name="branchcode" type="hidden" value="'.$branchcode.'"><input name="deliverto" type="hidden" value="'.$account_name.'"><input name="ordertype" type="hidden" value="DE">&nbsp;<img src="'.$image_path.'select.gif" alt="Select" title="Select" LANGUAGE=javascript onclick=\'return window.open("index.php?module=Accounts&action=Popup&popuptype=specific_account_address&form=TasksEditView&form_submit=false","test","width=600,height=400,resizable=1,scrollbars=1");\' align="absmiddle" style=\'cursor:hand;cursor:pointer\'></td>';
		...
		$custfld .= '<td width="30%" valign="top"  class="dataField"><input readonly name="account_name" type="text" value="'.$account_name.'"><input name="account_id" type="hidden" value="'.$value.'"><input name="holdreason" type="hidden" value="'.$holdreason.'"><input name="availablecredit" type="hidden" value="'.$AvailableCredit.'"><input name="debtorno" type="hidden" value="'.$debtor_no.'"><input name="branchcode" type="hidden" value="'.$branchcode.'">&nbsp;<img src="'.$image_path.'select.gif" alt="Select" title="Select" LANGUAGE=javascript onclick=\'return window.open("index.php?module=Accounts&action=Popup&popuptype=specific&form=TasksEditView&form_submit=false","test","width=600,height=400,resizable=1,scrollbars=1");\' align="absmiddle" style=\'cursor:hand;cursor:pointer\'>&nbsp;<input type="image" src="'.$image_path.'clear_field.gif" alt="Clear" title="Clear" LANGUAGE=javascript onClick="this.form.account_id.value=\'\';this.form.account_name.value=\'\';return false;" align="absmiddle" style=\'cursor:hand;cursor:pointer\'></td>';
		}	
	}
	...
	{
		$custfld .= '<td width="20%" class="dataLabel">'.$mod_strings[$fieldlabel].':</td>';
		if ($mod_strings[$fieldlabel] == "Product Active")
		{
			if($value == 0)
			{
				$display_val = 'yes';
			...
			{
				$display_val = 'discontinued';
			}
			$custfld .= '<td width="30%" valign="top" class="dataField"><input type="hidden" value="'.$value.'"><b>'.$display_val.'</b></td>';
		}
		elseif($value == 1)
		{
...
'"/><input type="hidden" name="filename" value=""/><input type="hidden" name="id" value=""/>'.$filename.'</td>';
		...
		if($value != '')
		{
			...
			{
				$sql = "select * from custbranch where accountid=".$value;
				$result = $adb->query($sql);
				$parent_name = $adb->query_result($result,0,"brname");
			...
			{
				$sql = "select * from  stockmaster where productid=".$value;
				$result = $adb->query($sql);
				$parent_name= $adb->query_result($result,0,"description");
			...
			elseif($parent_module == "Orders")
                        ...
                        {
                                $sql = "select * from  salesorders where salesorderid=".$value;
                                $result = $adb->query($sql);
                                $parent_name= $adb->query_result($result,0,"subject");
	...
	elseif($uitype == 27)
	{
		...
		if($value != '')
               {
                       $debtor_name = getDebtorName($value);
			$vtlog->logthis("value2 selected is  ".$value2 ,'info');  


$vtlog->logthis("record_id selected is  ".$col_fields["record_id"] ,'info');  


$vtlog->logthis("branchcode selected is  ".$col_fields["branchcode"] ,'info');  


			$value2 = $_REQUEST['account_id'];	
			$record2 = $col_fields["record_id"];
			$branchcode = $col_fields["branchcode"];
               }
	       elseif(isset($_REQUEST['debtor_no']) && $_REQUEST['debtor_no'] != '')
	       {
			$value = $_REQUEST['debtor_no'];
			$debtor_name = getDebtorName($value);
			$record2 = $col_fields["record_id"];
			$branchcode = $col_fields["branchcode"];
		...
		$crmseqid = getCrmSeq();
		$crmseqid1 = $crmseqid + 1;
		$custfld .= '<td width="20%" valign="center" class="dataLabel">';
		$pop_type = 'specific';
		$custfld .= '<font color="red">*</font>';
		$custfld .= $mod_strings[$fieldlabel].'</td>';
		$custfld .= '<td width="30%"><input name="crmseq" type="hidden" value="'.$crmseqid1.'"><input name="branchcode" type="hidden" value="' .$branchcode . '"><input name="debtorname" style="border-style: none;" readonly="readonly" type="text" value="'.$debtor_name.'"><input name="debtorno" type="hidden" readonly="readonly" value="'.$value.'">&nbsp;<img src="'.$image_path.'select.gif" alt="Select" title="Select" LANGUAGE=javascript onclick=\'return window.open("index.php?module=Accounts&action=Popup1&popuptype=specific1&form=TasksEditView&add=1&form_submit=false","test","width=600,height=400,resizable=1,scrollbars=1");\' align="absmiddle" style=\'cursor:hand;cursor:pointer\'>';
$custfld .='&nbsp;<input type="image" src="'.$image_path.'clear_field.gif" alt="Clear" title="Clear" LANGUAGE=javascript onClick="this.form.debtorno.value=\'\';this.form.debtorname.value=\'\';this.form.self.checked=0;return false;" align="absmiddle" style=\'cursor:hand;cursor:pointer\'><input name="self" type="checkbox" onClick="return emptyCheck1(\'brname\',\'Account Name\', \'text\', \'debtorno\', \'debtorname\', \'branchcode\', \'self\', \'crmseq\');"> Self</td>';
...
function getDetailViewOutputHtml($uitype, $fieldname, $fieldlabel, $col_fields,$generatedtype)
{
	...
	elseif($uitype == 20 || $uitype == 21 || $uitype == 22 || $uitype == 24 || $uitype == 25) // Armando LC<scher 11.08.2005 -> B'descriptionSpan -> Desc: removed $uitype == 19 and made an aditional elseif above
	{
		$col_fields[$fieldname]=nl2br($col_fields[$fieldname]);
		$custfld .= '<td width="20%" class="dataLabel" valign="top">'.$mod_strings[$fieldlabel].':</td>';
	...
	elseif($uitype == 28)
	{
       ...
       $availValue = $stockValue - $demandValue;
		...
		$custfld .= '<td width="20%" class="dataLabel" valign="top">'.$mod_strings[$fieldlabel].':</td>';
		$custfld .= '<td class="dataField"> '.$availValue.'</td>';
	}
	elseif($uitype == 128)
	{
		    
		$custfld .= '<td width="20%" class="dataLabel" valign="top">'.$mod_strings[$fieldlabel].':</td>';
		$custfld .= '<td class="dataField"> '.$col_fields[$fieldname].'</td>';
	}
	elseif($uitype == 29)
	{
		$stock_id = $col_fields['stockid'];
       $demandValue = getDemand($stock_id);
		...
		$custfld .= '<td width="20%" class="dataLabel" valign="top">'.$mod_strings[$fieldlabel].':</td>';
		$custfld .= '<td class="dataField"> '.$demandValue.'</td>';
	}
	elseif($uitype == 31)
	{
	#$vtlog->logthis("uitype selected is  ".$uitype,'info');  
		...
		{
		$pick_query="select salesorders.shipvia, shippers.shipper_id, shippers.shippername from salesorders inner join shippers on shippers.shipper_id=salesorders.shipvia where salesorders.orderno=$col_fields[orderno]";
		}
		else 
		{
		$ref_id = $_REQUEST['record'];
      
		$pick_query="select quotes.shipvia, shippers.shipper_id, shippers.shippername from quotes inner join shippers on shippers.shipper_id=quotes.shipvia where quotes.quoteid=$ref_id";
		}
		$pickListResult = $adb->query($pick_query);
		$noofpickrows = $adb->num_rows($pickListResult);
		$custfld .= '<td class="dataField">';
	...
	#$vtlog->logthis("uitype selected is  ".$uitype,'info');  
		...
		$pick_query="SELECT locations.locationname,locations.loccode,custbranch.defaultlocation FROM custbranch INNER JOIN locations on locations.loccode=custbranch.defaultlocation where custbranch.defaultlocation='$col_fields[$fieldname]'";
		$pickListResult = $adb->query($pick_query);
		$noofpickrows = $adb->num_rows($pickListResult);
		$custfld .= '<td class="dataField">';
		$pickListValue=$adb->query_result($pickListResult,$j,loccode);
	...
	#$vtlog->logthis("uitype selected is  ".$uitype,'info');  
		...
		$pick_query="select deliverblind from salesorders where salesorders.orderno=$col_fields[orderno]";
		$pickListResult = $adb->query($pick_query);
		$noofpickrows = $adb->num_rows($pickListResult);
		$custfld .= '<td class="dataField">';
		for($j = 0; $j < $noofpickrows; $j++)
		{
			...
			$pickListValue=$adb->query_result($pickListResult,$j,deliverblind);
			$deliverblind = $pickListValue;
			if ($deliverblind = 1)
			{$deliverblindvalue = "Show Company Details and Logo";}
			elseif ($deliverblind = 2)
			{$deliverblindvalue = "Hide Company Details and Logo";}

			

			$custfld .= ' '.$deliverblindvalue.' ';
		}
		$custfld .= '</td>';
	}
	elseif($uitype == 34 || $uitype == 36)
	{
	#$vtlog->logthis("uitype selected is  ".$uitype,'info');  
	...
	elseif($uitype == 35)
	{
	$custfld .= '<input type="hidden" name="ordertype" value="'.$col_fields[$fieldname].'">';
	...
	elseif($uitype == 41)
	{
	$catid = $col_fields[$fieldname];
	$category = getCategory($catid);
	$custfld .= '<td width="20%" class="dataLabel" valign="top">'.$mod_strings[$fieldlabel].':</td>';
	$custfld .= '<td class="dataField">'.$category.'</td>';
	...
	#$vtlog->logthis("uitype selected is  ".$uitype,'info');  
		...
		$pick_query="SELECT areas.areacode, areas.areadescription, custbranch.area FROM custbranch INNER JOIN areas on areas.areacode=custbranch.area where custbranch.area='$col_fields[area]'";
		$pickListResult = $adb->query($pick_query);
		$noofpickrows = $adb->num_rows($pickListResult);
		$custfld .= '<td class="dataField">';
		$pickListValue=$adb->query_result($pickListResult,0,area);
		$pickListDisplay=$adb->query_result($pickListResult,0,areadescription);
		$custfld .= ' '.$pickListDisplay.' ';
...
//		$userid1 = $_REQUEST['assigned_user_id'];
	...
	$salesmancode = getSalespersonCode($assigned_user_id1);
$custfld .= '<td width="20%" class="dataLabel" valign="top">'.$mod_strings[$fieldlabel].':</td>';
	$custfld .= '<td class="dataField">'.$salesperson.'</td>';
	...
	#$vtlog->logthis("uitype selected is  ".$uitype,'info');  
		...
		$pick_query="SELECT taxgroups.taxgroupid, taxgroups.taxgroupdescription, custbranch.taxgroupid FROM custbranch INNER JOIN taxgroups on taxgroups.taxgroupid=custbranch.taxgroupid where custbranch.taxgroupid='$col_fields[$fieldname]'";
		$pickListResult = $adb->query($pick_query);
		$noofpickrows = $adb->num_rows($pickListResult);
		$custfld .= '<td class="dataField">';
		$pickListValue=$adb->query_result($pickListResult,0,taxgroupid);
		$pickListDisplay=$adb->query_result($pickListResult,0,taxgroupdescription);
	...
	#$vtlog->logthis("uitype selected is  ".$uitype,'info');  
		...
		$custfld .= '<input type="hidden" name="deliverto" value="'.$value.'">';
	...
	#$vtlog->logthis("uitype selected is  ".$uitype,'info');  
		...
		$pick_query="SELECT shippers.shipper_id, shippers.shippername, custbranch.defaultshipvia FROM custbranch INNER JOIN shippers on shippers.shipper_id=custbranch.defaultshipvia where custbranch.defaultshipvia='$col_fields[$fieldname]'";
		$pickListResult = $adb->query($pick_query);
		$noofpickrows = $adb->num_rows($pickListResult);
		$custfld .= '<td class="dataField">';
		$pickListValue=$adb->query_result($pickListResult,0,defaultshipvia);
		$pickListDisplay=$adb->query_result($pickListResult,0,shippername);
	...
	#$vtlog->logthis("uitype selected is  ".$uitype,'info');  
		...
		$pick_query="SELECT currencies.currabrev, currencies.currency, custbranch.currcode FROM custbranch INNER JOIN currencies on currencies.currabrev=custbranch.currcode where custbranch.currcode='$col_fields[$fieldname]'";
		$pickListResult = $adb->query($pick_query);
		$noofpickrows = $adb->num_rows($pickListResult);
		$custfld .= '<td class="dataField">';
		$pickListValue=$adb->query_result($pickListResult,0,currcode);
		$pickListDisplay=$adb->query_result($pickListResult,0,currency);
	...
	{
		$custfld .= '<td width="20%" class="dataLabel"></td>';
		$custfld .= '<td width="30%" valign="top" class="dataField">'.$col_fields[$fieldname].'</a></td>';
	...
	elseif($uitype == 15)
	{
		$custfld .= '<td width="20%" class="dataLabel">'.$mod_strings[$fieldlabel].':</td>';
		$value = $col_fields[$fieldname];
		if (($mod_strings[$fieldlabel] == 'Preferred Language') && (!isset($value)))
		{
	...
	elseif($uitype == 62)
	{
		...
		if($value != '')
		{
			...
			elseif($parent_module == "Accounts")
			{
				...
				$sql = "select * from custbranch where accountid=".$value;
				$result = $adb->query($sql);
				$account_name = $adb->query_result($result,0,"brname");
			...
			elseif($parent_module == "Products")
			{
				...
				$sql = "select * from  stockmaster where productid=".$value;
				$result = $adb->query($sql);
				$productname= $adb->query_result($result,0,"description");
	...
	elseif($uitype == 27)
        {
                $custfld .= '<td width="20%" class="dataLabel">'.$mod_strings[$fieldlabel].':</td>';
                $debtor_no = $col_fields[$fieldname];
                if($debtor_no != '')
                {
                        $debtor_name = getDebtorName($debtor_no);
                }
				if ($debtor_name == $col_fields[brname])
				{$debtor_name = "--same--";}
                $custfld .= '<td width="30%" valign="top" class="dataField">'.$debtor_name.'</td>';
        }
	elseif($uitype == 76)
        {
                $custfld .= '<td width="20%" class="dataLabel">'.$mod_strings[$fieldlabel].':</td>';
...
$vtlog->logthis("in getAccountName ".$account_id,'info');  
	...
	if($account_id != '')
	{
		$sql = "select brname from custbranch where accountid=".$account_id;
        	$result = $adb->query($sql);
		$accountname = $adb->query_result($result,0,"brname");
	}
	return $accountname;
}
...
{
global $vtlog;
$vtlog->logthis("in getAccountType ".$account_id,'info');  

	global $adb;
	if($account_id != '')
	{
		$sql = "select account_type from custbranch where accountid=".$account_id;
        	$result = $adb->query($sql);
		$accounttype = $adb->query_result($result,0,"account_type");
	}
	return $accounttype;
}
...
$vtlog->logthis("in getproductname ".$product_id,'info');  
	...
	global $adb;
	$sql = "select description from stockmaster where productid=".$product_id;
        $result = $adb->query($sql);
	$productname = $adb->query_result($result,0,"description");
	return $productname;
}
...
$vtlog->logthis("in getSoName ".$so_id,'info');  
        global $adb;
        $sql = "select * from salesorders where salesorderid=".$so_id;
        $result = $adb->query($sql);
...
global $vtlog;
$vtlog->logthis("in getNav ".$start . " noofrows " . $noofrows . " list max entries " . $list_max_entries_per_page . " accountid " . $accountid,'info');  
...
function getRelatedToEntity($module,$list_result,$rset)
{
	...
	if(isset($seid) && $seid != '')
	{
		...
		{
			$parent_query = "SELECT brname FROM custbranch WHERE accountid=".$seid;
			$parent_result = $adb->query($parent_query);
			$parent_name = $adb->query_result($parent_result,0,"brname");
		}
		if($parent_module == 'Leads')
		{
		...
		{
			$parent_query = "SELECT description FROM stockmaster WHERE productid=".$seid;
			$parent_result = $adb->query($parent_query);
			$parent_name = $adb->query_result($parent_result,0,"description");
		}
		if($parent_module == 'Orders')
		{
		...
		if($parent_module == 'SalesOrder')
		{
			$parent_query = "SELECT subject FROM salesorders WHERE salesorderid=".$seid;
			$parent_result = $adb->query($parent_query);
			$parent_name = $adb->query_result($parent_result,0,"subject");
...
function getRelatedTo($module,$list_result,$rset)
{
	...
	$action = "DetailView";
        if($parent_module == 'Accounts')
        {
                $parent_query = "SELECT brname FROM custbranch WHERE accountid=".$parent_id;
                $parent_result = $adb->query($parent_query);
                $parent_name = $adb->query_result($parent_result,0,"brname");
        }
        if($parent_module == 'Leads')
        ...
        if($parent_module == 'Products')
        {
                $parent_query = "SELECT description FROM stockmaster WHERE productid=".$parent_id;
                $parent_result = $adb->query($parent_query);
                $parent_name = $adb->query_result($parent_result,0,"description");
        }
	...
	if($parent_module == 'Invoice')
        ...
        if($parent_module == 'SalesOrder')
        {
                $parent_query = "SELECT subject FROM salesorders WHERE salesorderid=".$parent_id;
                $parent_result = $adb->query($parent_query);
                $parent_name = $adb->query_result($parent_result,0,"subject");
		$action = "SalesOrderDetailView";
		$parent_module = "Orders";
...
{
	global $vtlog;
$vtlog->logthis("in sales order list getListViewEntries ".$module,'info');  
$vtlog->logthis("in sales order list getListViewEntries2 ".$list_result,'info');  
$vtlog->logthis("in sales order list getListViewEntries2 ".$navigation_array,'info');  
	...
	$orderno = $adb->query_result($list_result,$i-1,"orderno");
	$vtlog->logthis("in sales order list function ".$entity_id,'info');  
    $vtlog->logthis("in sales order list function2 ".$owner_id,'info');  
	$vtlog->logthis("in sales order list function3 ".$orderno,'info');  
		if($relatedlist == '')
		{
			$list_header .= '<td WIDTH="1" class="blackLine"><IMG SRC="'.$image_path.'blank.gif"></td>';
		...
		{
			$fieldname = $focus->list_fields_name[$name];
			$vtlog->logthis("in sales order list function4 field name  ".$fieldname . " and table " . $name,'info');  
...
function getValue($field_result, $list_result,$fieldname,$focus,$module,$entity_id,$list_result_count,$mode,$popuptype,$returnset='')
{
	...
	elseif($uitype == 71 || $uitype == 72)
	{
		...
		$temp21 = $_REQUEST['accountid'];
					
					$vtlog->logthis("account id is ".$accountid,'info');  
					$vtlog->logthis("request account id is ".$temp21,'info');  
				$accountid = $_REQUEST['accountid'];
					$productid=$adb->query_result($list_result,$list_result_count,'crmid');
					$taxcatid=$adb->query_result($list_result,$list_result_count,'taxcatid');
					$stockno = getStockNo($productid);
					$branchcode = getCustBranch($accountid);
					$debtorno = getDebtorNo($branchcode);
					$unitprice1=$adb->query_result($list_result,$list_result_count,'unit_price');
					$unitprice=GetPrice ($stockno, $debtorno, $branchcode);
					if ($unitprice == '0')
					{$unitprice = $unitprice1;
					 $priceinput = $symbol.' '.$unitprice;}
					 else {$priceinput = '<b><i>' . $unitprice . '</i></b>';}
		$vtlog->logthis("productid is ".$productid,'info');  
					$vtlog->logthis("stockno is ".$stockno,'info');  
				$vtlog->logthis("branchcode id is ".$branchcode,'info');  
					$vtlog->logthis("debtorno is ".$debtorno,'info');  
				
		
			$symbol = getCurrencySymbol();
		//	$value = $symbol.' '.$unitprice . ' (Wholesale:  '. $symbol.' '.$unitprice1 . ' )'; 
			$value = $priceinput; 
	...
	elseif($uitype == 62)
	{
		...
		{
			$tablename = "custbranch";		$fieldname = "brname";     $idname="accountid";
		}
		if($parenttype == "Products")	
		{
			$tablename = "stockmaster";	$fieldname = "description";     $idname="productid";
		}
	...
	elseif($uitype == 27)
		...
		if($module == 'Accounts')
			{
			if($popuptype == 'specific1')
			{
			if($temp_val != '')
                {
			
                        $debtor_name = getDebtorName($temp_val);
						$debtor_no = getDebtorNo($temp_val);
						$value= '<a href="a" LANGUAGE=javascript onclick=\'set_return_specific1("'.$debtor_name.'","'.$debtor_no.'"); window.close()\'>'.$debtor_name.'</a>';
				}
			...
			elseif($popuptype == "specific_account_address")
				{
					...
					$cust_name = getCustName ($temp_val);
                     $account_id = getAcctId ($temp_val);
					$branchcode = getCustBranch($account_id);
					$debtor_no = getDebtorNo($branchcode);
					$holdreason = getHoldReason($debtor_no);
					$CreditAvailable = getCreditAvailable($debtor_no);
...
//					$orderno = getOrderNo();
					...
					$acct_focus->retrieve_entity_info($account_id,"Accounts");
					
					
					$value = '<a href="a" LANGUAGE=javascript onclick=\'set_return_address("'.$cust_name.'","'.$account_id.'", "'.br2nl($acct_focus->column_fields['braddress1']).'", "'.br2nl($acct_focus->column_fields['brpostaddr1']).'", "'.br2nl($acct_focus->column_fields['braddress2']).'", "'.br2nl($acct_focus->column_fields['brpostaddr2']).'", "'.br2nl($acct_focus->column_fields['braddress3']).'", "'.br2nl($acct_focus->column_fields['brpostaddr3']).'", "'.br2nl($acct_focus->column_fields['braddress4']).'", "'.br2nl($acct_focus->column_fields['brpostaddr4']).'", "'.br2nl($acct_focus->column_fields['braddress6']).'", "'.br2nl($acct_focus->column_fields['brpostaddr6']).'", "'.br2nl($acct_focus->column_fields['braddress5']).'", "'.br2nl($acct_focus->column_fields['brpostaddr5']).'", "'.$branchcode.'", "'.$debtor_no.'", "'.$holdreason.'", "'.$CreditAvailable.'"); window.close()\'>'.$cust_name.'</a>';
			...
			elseif($temp_val != '')
                {
					$cust_name = getCustName ($temp_val);
                    $acct_id = getAcctId ($temp_val);
                    $value= '<a href="a" LANGUAGE=javascript onclick=\'set_return_specific("'.$cust_name.'","'.$acct_id.'"); window.close()\'>XZXZ'.$cust_name.'</a>';
				}
		...
		if($fieldname == $focus->list_link_field)
		{
			if($mode == "search")
			{
				...
				elseif($popuptype == "inventory_prod")
				{
					if($accountid != "")
				{
					$vtlog->logthis("SUCCESS account id is ".$accountid,'info');  
				}
					...
					$vtlog->logthis("account id is ".$accountid,'info');  
					$vtlog->logthis("request account id is ".$temp21,'info');  
				If (($_REQUEST['accountid'] != '') && (isset($_REQUEST['accountid'])))
					{$accountid = $_REQUEST['accountid'];}
					$taxgroupid = getTaxGroup($accountid);
					
					if (($_REQUEST['rowid'] != '') && (isset($_REQUEST['rowid'])))
				{$row_id = $_REQUEST['rowid'];}
				elseif (($_REQUEST['curr_row'] != '') && (isset($_REQUEST['curr_row'])))
			...
			//		$row_id = $_REQUEST['curr_row'];
			//		elseif ($_REQUEST['rowid'] != "" || isset($_REQUEST['rowid']))					
			//		{$row_id = $_REQUEST['rowid'];}
					
					
					$vtlog->logthis("rowid id is ".$row_id,'info');  
					$vtlog->logthis("rowid request id is ".$_REQUEST['rowid'],'info');  
				$vtlog->logthis("curr row id is ".$_REQUEST['curr_row'],'info');  
				
				
				$productid=$adb->query_result($list_result,$list_result_count,'crmid');
					$taxcatid=$adb->query_result($list_result,$list_result_count,'taxcatid');
					$stockno = getStockNo($productid);
					$qtyinstock = getStock($stockno);
					$qtyindemand = getDemand($stockno);
					$qtyavailable = $qtyinstock - $qtyindemand;
					$taxes = getItemTaxes2($taxgroupid,$taxcatid);
					$branchcode = getCustBranch($accountid);
					$debtorno = getDebtorNo($branchcode);
...
//					//$qty_stock=$adb->query_result($list_result,$list_result_count,'qtyinstock');
					$value = '<a href="a" LANGUAGE=javascript onclick=\'set_return_inventory("'.$entity_id.'", "'.br2nl($temp_val).'", "'.$unitprice.'", "'.$qtyavailable.'", "'.$qtyindemand.'",  "'.$taxes.'",  "'.$row_id.'"); window.close()\'>'.$taxes.' -- '.$temp_val.'</a>';
				}
				...
				elseif($popuptype == "inventory_pb1")
				{

$rowtotal = $_REQUEST['rowtotal'];
$tmp1=$_REQUEST['allproducts'];
$pieces = explode(",", $tmp1);
$PBPrice = "";
$PBPrice = strval($PBPrice);
	

					foreach($pieces as $piece)
		{
				$PBPrice1=getPBPrice($piece,$entity_id);
				if (($PBPrice1=="") || (!isset($PBPrice1)))
				{$PBPrice1=0;}
				if ($PBPrice == "")
				{$PBPrice = $PBPrice . $PBPrice1;}
				else {$PBPrice=$PBPrice . "|" . $PBPrice1;}

			
			
	}
					
					$value = '<a href="a" LANGUAGE=javascript onclick=\'set_return_inventory_pb1("'.$rowtotal.'", "'.$PBPrice.'"); window.close()\'>'.$temp_val.'</a>';
				}
				elseif($popuptype == "specific_account_address")
				{
					require_once('modules/Accounts/Account.php');
					...
					$braddress2 = getAddress2($entity_id);
					$brpostaddr2 = getPostAddress2($entity_id);
					$branchcode = getCustBranch($entity_id);
					$debtor_no = getDebtorNo($branchcode);
					$holdreason = getHoldReason($debtor_no);
					$CreditAvailable = getCreditAvailable($debtor_no);
					
					$value = '<a href="a" LANGUAGE=javascript onclick=\'set_return_address("'.$entity_id.'", "'.br2nl($temp_val).'", "'.br2nl($acct_focus->column_fields['bill_street']).'", "'.br2nl($acct_focus->column_fields['ship_street']).'", "'.br2nl($braddress2).'", "'.br2nl($brpostaddr2).'", "'.br2nl($acct_focus->column_fields['bill_city']).'", "'.br2nl($acct_focus->column_fields['ship_city']).'", "'.br2nl($acct_focus->column_fields['bill_state']).'", "'.br2nl($acct_focus->column_fields['ship_state']).'", "'.br2nl($acct_focus->column_fields['bill_code']).'", "'.br2nl($acct_focus->column_fields['ship_code']).'", "'.br2nl($acct_focus->column_fields['bill_country']).'", "'.br2nl($acct_focus->column_fields['ship_country']).'", "'.$branchcode.'", "'.$debtor_no.'", "'.$holdreason.'", "'.$CreditAvailable.'"); window.close()\'>'.$temp_val.'</a>';
				...
				elseif($popuptype=="specific1")
				{
					...
					$acntid = $adb->query_result($list_result,$list_result_count,"accountid");
                                        ...
                                        $acct_focus->retrieve_entity_info($acntid,"Accounts");
                                        $account_name = getAccountName($acntid);
										$branchcode = getCustBranch($acntid);
										//$debtor_no = getDebtorNo($branchcode);
										
										$value= '<a href="a" LANGUAGE=javascript onclick=\'set_return_specific1("'.$account_name.'","'.$branchcode.'"); window.close()\'>'.$account_name.'</a>';
			...
			else
			{
				...
				elseif($module == "SalesOrder")
				{
					...
					if($colname == 'Order Id')
						{
 				           $orderno=$adb->query_result($list_result,$list_result_count,'orderno');
			                $temp_val =$orderno;
							$value = $temp_val;
						}
				}
...
function getListQuery($module,$where='')
{
	if($module == "HelpDesk")
	{
		$query = "select crmentity.crmid,troubletickets.title,troubletickets.status,troubletickets.priority,crmentity.smownerid, contactdetails.contactid, troubletickets.parent_id, contactdetails.firstname, contactdetails.lastname, custbranch.accountid, custbranch.brname, ticketcf.* from troubletickets inner join ticketcf on ticketcf.ticketid = troubletickets.ticketid inner join crmentity on crmentity.crmid=troubletickets.ticketid left join contactdetails on troubletickets.parent_id=contactdetails.contactid left join custbranch on custbranch.accountid=troubletickets.parent_id left join users on crmentity.smownerid=users.id and troubletickets.ticketid = ticketcf.ticketid where crmentity.deleted=0";
	...
	if($module == "Accounts")
	{
		...
		$query = "select crmentity.crmid, custbranch.brname,custbranch.braddress3,custbranch.website,custbranch.phoneno,crmentity.smownerid, accountscf.*  from custbranch inner join crmentity on crmentity.crmid=custbranch.accountid inner join accountscf on custbranch.accountid = accountscf.accountid where crmentity.deleted=0";
		
	}
	if($module == "Accounts1")
	{
	 	if($query == "true" || $action=="Popup1")
		{
		$query = "select crmentity.crmid, crmentity.deleted, custbranch.brname, custbranch.branchcode, custbranch.debtorno, debtorsmaster.name, debtorsmaster.address3, crmentity.smownerid, accountscf.*  from custbranch left join crmentity on crmentity.crmid=custbranch.accountid left join debtorsmaster on debtorsmaster.debtorno = custbranch.debtorno left join accountscf on custbranch.accountid = accountscf.accountid group by custbranch.debtorno where crmentity.deleted=0";
	...
	{
		 //$query = "select crmentity.crmid, crmentity.smownerid,custbranch.brname, potential.*, potentialscf.* from potential , custbranch, potentialscf inner join crmentity on crmentity.crmid=potential.potentialid and potential.accountid = custbranch.accountid and potentialscf.potentialid = potential.potentialid where crmentity.deleted=0 ".$where;
		 $query = "select crmentity.crmid, crmentity.smownerid,custbranch.brname, potential.accountid,potential.potentialname,potential.sales_stage,potential.amount,potential.currency,potential.closingdate,potential.typeofrevenue, potentialscf.* from potential inner join crmentity on crmentity.crmid=potential.potentialid inner join custbranch on potential.accountid = custbranch.accountid inner join potentialscf on potentialscf.potentialid = potential.potentialid where crmentity.deleted=0 ".$where;
	...
	if($module == "Products")
	{
		$query = "select crmentity.crmid, stockmaster.*, productcf.* from stockmaster inner join crmentity on crmentity.crmid=stockmaster.productid left join productcf on stockmaster.productid = productcf.productid left join seproductsrel on seproductsrel.productid = stockmaster.productid where crmentity.deleted=0 and stockmaster.special=1";
	}
        if($module == "Notes")
        {
	...
	if($module == "Contacts")
        {
                $query = "select crmentity.crmid, crmentity.smownerid, contactdetails.*, contactaddress.*, contactsubdetails.*, contactscf.*, custbranch.brname from contactdetails, contactaddress, contactsubdetails, contactscf inner join crmentity on crmentity.crmid=contactdetails.contactid and contactdetails.contactid=contactaddress.contactaddressid and contactdetails.contactid = contactscf.contactid and contactaddress.contactaddressid=contactsubdetails.contactsubscriptionid left join custbranch on custbranch.accountid = contactdetails.accountid where crmentity.deleted=0";
		//$query = "select crmentity.crmid, crmentity.smownerid, contactdetails.*, contactaddress.*, contactsubdetails.*, contactscf.*, custbranch.brname from contactdetails, contactaddress, contactsubdetails, contactscf,crmentity,custbranch where crmentity.crmid=contactdetails.contactid and contactdetails.contactid=contactaddress.contactaddressid and contactdetails.contactid = contactscf.contactid and contactaddress.contactaddressid=contactsubdetails.contactsubscriptionid and custbranch.accountid = contactdetails.accountid and crmentity.deleted=0";
        }
	if($module == "Meetings")
        {
	...
	if($module == "Activities")
        {
		$query = " select crmentity.crmid,crmentity.smownerid,crmentity.setype, activity.*, contactdetails.lastname, contactdetails.firstname, contactdetails.contactid, custbranch.accountid, custbranch.brname, recurringevents.recurringtype from activity inner join crmentity on crmentity.crmid=activity.activityid left join cntactivityrel on cntactivityrel.activityid= activity.activityid left join contactdetails on contactdetails.contactid= cntactivityrel.contactid left join seactivityrel on seactivityrel.activityid = activity.activityid left outer join custbranch on custbranch.accountid = contactdetails.accountid left outer join recurringevents on recurringevents.activityid=activity.activityid WHERE crmentity.deleted=0 and (activity.activitytype = 'Meeting' or activity.activitytype='Call' or activity.activitytype='Task') ".$where  ;
		//included by Jaguar
        }
	if($module == "Emails")
        {
	...
	if($module == "Faq")
	{
		$query = "select crmentity.crmid, faq.*, crmentity.createdtime, crmentity.modifiedtime from faq inner join crmentity on crmentity.crmid=faq.id left join stockmaster on faq.product_id=stockmaster.productid where crmentity.deleted=0";
	}
	if($module == "Vendor")
	{
	...
	if($module == "Quotes")
	{
		$query = "select crmentity.*, quotes.*, quotesbillads.*, quotesshipads.*,potential.potentialname,custbranch.brname from quotes inner join crmentity on crmentity.crmid=quotes.quoteid inner join quotesbillads on quotes.quoteid=quotesbillads.quotebilladdressid inner join quotesshipads on quotes.quoteid=quotesshipads.quoteshipaddressid left outer join custbranch on custbranch.accountid=quotes.accountid left outer join potential on potential.potentialid=quotes.potentialid where crmentity.deleted=0".$where;
	}
	if($module == "Orders")
	{
	...
	if($module == "SalesOrder")
	{
		$query = "select crmentity.*, salesorders.*, sobillads.*, quotes.subject as quotename, custbranch.brname from salesorders inner join crmentity on crmentity.crmid=salesorders.salesorderid inner join sobillads on salesorders.salesorderid=sobillads.sobilladdressid left outer join quotes on quotes.quoteid=salesorders.quoteid left outer join custbranch on custbranch.accountid=salesorders.accountid where crmentity.deleted=0".$where;
	}
	if($module == "Invoice")
	{
		$query = "select crmentity.*, invoice.*, invoicebillads.*, invoiceshipads.*,salesorders.subject as salessubject from invoice inner join crmentity on crmentity.crmid=invoice.invoiceid inner join invoicebillads on invoice.invoiceid=invoicebillads.invoicebilladdressid inner join invoiceshipads on invoice.invoiceid=invoiceshipads.invoiceshipaddressid left outer join salesorders on salesorders.salesorderid=invoice.salesorderid where crmentity.deleted=0".$where;
	}
...
{
global $vtlog;
	$vtlog->logthis("in viewname2 ".$_REQUEST['viewname'],'info');  
	$vtlog->logthis("in viewid2 ".$viewid,'info');  
	...
	$vtlog->logthis("in viewname ".$viewid,'info');  
	$vtlog->logthis("in append_url ".$append_url,'info');  

	return $list;
}

function AlphabeticalSearch1($module,$action,$fieldname,$query,$type,$accountid,$accountid2,$rowid,$rowid2,$popuptype='',$recordid='',$return_module='',$append_url='',$viewid='')
{
global $vtlog;
$vtlog->logthis("AlphabeticalSearch1 Account ID".$rowid,"info");
	$vtlog->logthis("AlphabeticalSearch1 Row ID".$rowid2,"info");
	...
	for($var='A',$i =1;$i<=26;$i++,$var++)
		$list .= '<td class="alphaBg"><a href="index.php?module='.$module.'&action='.$action.'&accountid='.$accountid.'&rowid='.$rowid.'&viewname='.$viewid.'&query='.$query.'&'.$fieldname.'='.$var.$flag.$popuptypevalue.$returnvalue.$append_url.'">'.$var.'</a></td>';
	$vtlog->logthis("in viewname ".$viewid,'info');  
	$vtlog->logthis("in append_url ".$append_url,'info');  
...
function getRelatedLists($module,$focus)
{
	...
	global $profile_id;
global $vtlog;
$vtlog->logthis("in getRelatedLists processing" . $module .  " -- " . $focus,'info');  

	$mod_dir_name=getModuleDirName($module);
	$vtlog->logthis("in getRelatedLists modirname" . $module_dir_name,'info');  

	$tab_per_Data = getAllTabsPermission($profile_id);
	$vtlog->logthis("in getRelatedLists getAllTabsPermission" . $tab_per_Data,'info');  
...
$vtlog->logthis("in getRelatedLists processing"  . $function_name,'info');  
...
$vtlog->logthis("in getRelatedLists1 processing" . $focus->id . " -- " . $function_name,'info');  
		...
		{
			$focus_list = & $focus->$funtion_name($focus->id);
$vtlog->logthis("in getRelatedLists2 processing" . $focus->id . " -- " . $function_name,'info');  
...
function getAssociatedProducts($module,$focus,$seid='')
{
	...
	if($module == 'Quotes')
	{
		$query="select stockmaster.description,stockmaster.unit_price,stockmaster.taxcatid,stockmaster.qtyinstock,quotes.accountid,quotes.fromstkloc,locations.taxprovinceid,custbranch.taxgroupid,quotesproductrel.* from quotesproductrel inner join stockmaster on stockmaster.productid=quotesproductrel.productid inner join quotes on quotes.quoteid=quotesproductrel.quoteid INNER JOIN locations ON locations.loccode=quotes.fromstkloc INNER JOIN custbranch ON quotes.accountid = custbranch.accountid where quotesproductrel.quoteid=".$focus->id;
	}
	elseif($module == 'Orders')
	{
		$query="select stockmaster.description,stockmaster.unit_price,stockmaster.qtyinstock,poproductrel.* from poproductrel inner join stockmaster on stockmaster.productid=poproductrel.productid where purchaseorderid=".$focus->id;
	}
	elseif($module == 'SalesOrder')
	{
		$query="select stockmaster.description,stockmaster.unit_price,stockmaster.taxcatid,stockmaster.qtyinstock,salesorders.accountid,salesorders.fromstkloc,locations.taxprovinceid,custbranch.taxgroupid,salesorderdetails.* from salesorderdetails inner join stockmaster on stockmaster.productid=salesorderdetails.productid INNER JOIN salesorders ON salesorders.salesorderid = salesorderdetails.salesorderid INNER JOIN locations ON locations.loccode=salesorders.fromstkloc INNER JOIN custbranch ON salesorders.accountid = custbranch.accountid where salesorderdetails.salesorderid=".$focus->id;
	}
	elseif($module == 'Invoice')
	{
		$query="select stockmaster.description,stockmaster.unit_price,stockmaster.qtyinstock,invoiceproductrel.* from invoiceproductrel inner join stockmaster on stockmaster.productid=invoiceproductrel.productid where invoiceid=".$focus->id;
	}
	elseif($module == 'Potentials')
	{
		$query="select stockmaster.stockid,stockmaster.description,stockmaster.unit_price,stockmaster.qtyinstock,seproductsrel.* from stockmaster inner join seproductsrel on seproductsrel.productid=stockmaster.productid where crmid=".$seid;
	}
	elseif($module == 'Products')
	{
		$query="select stockmaster.productid,stockmaster.stockid,stockmaster.description,stockmaster.unit_price,salesorderdetails.discountpercent,crmentity.* from stockmaster left join salesorderdetails on salesorderdetails.stkcode=stockmaster.stockid  inner join crmentity on crmentity.crmid=stockmaster.productid where crmentity.deleted=0 and stockmaster.productid=".$seid;
	}
	...
	{
		$productname=$adb->query_result($result,$i-1,'description');
		$stockid=$adb->query_result($result,$i-1,'stockid');
		$unitprice=$adb->query_result($result,$i-1,'unit_price');
		$salesorder=$adb->query_result($result,$i-1,'salesorderid');
		$taxgroupid=$adb->query_result($result,$i-1,'taxgroupid');
		$dispatchprovince=$adb->query_result($result,$i-1,'taxprovinceid');
		$taxcatid=$adb->query_result($result,$i-1,'taxcatid');
		...
		$taxes=getItemTaxes1($taxgroupid,$dispatchprovince,$taxcatid);


		if($module == 'SalesOrder' || $module == 'SalesOrders' || $module == 'Orders')
	{
		$listprice=$adb->query_result($result,$i-1,'unitprice');
		$discount2=$adb->query_result($result,$i-1,'discountpercent');
		$discount = number_format(($discount2 * 100),2);

	}
		else
	{
		$listprice=$adb->query_result($result,$i-1,'listprice');
		$discount2=$adb->query_result($result,$i-1,'discount');
		$discount = number_format(($discount2 * 100),2);
		...
		if($discount !='')
		$discount3 = $qty*$listprice*$discount/100;
		$total = $qty*$listprice-($qty*$listprice*$discount/100);
		$orderlineno = $i-1;
		...
		$output .= '<td height="25" style="padding:3px;" nowrap><input id="txtProduct'.$i.'" name="txtProduct'.$i.'" type="text" size="30" readonly value="'.$productname.'"> <img src="'.$image_path.'search.gif" onClick=\'productPickList(this)\' align="absmiddle" style=\'cursor:hand;cursor:pointer\'></td>';
		$output .= '<td WIDTH="1" class="blackLine"><IMG SRC="'.$image_path.'blank.gif"></td>';
		if($module != 'Orders' && $focus->object_name != 'Order')
		{
			$output .= '<td style="padding:3px;"><div id="qtyInStock'.$i.'">'.$qtyavailable.'</div>&nbsp;</td>';
			$output .= '<td WIDTH="1" class="blackLine"><IMG SRC="'.$image_path.'blank.gif"></td>';
			$output .= '<td style="padding:3px;"><div id="qtyInDemand'.$i.'">'.$qtyindemand.'</div>&nbsp;</td>';
			$output .= '<td WIDTH="1" class="blackLine"><IMG SRC="'.$image_path.'blank.gif"></td>';
		}	
		...
		$output .= '<td WIDTH="1" class="blackLine"><IMG SRC="'.$image_path.'blank.gif"></td>';
		$output .= '<td style="padding:3px;"><input type=text id="txtListPrice'.$i.'" name="txtListPrice'.$i.'" value="'.$listprice.'" size="12" onselect="calcTotal(this)" onChange="calcTotal(this)" onBlur="calcTotal(this)"> <img src="'.$image_path.'pricebook.gif" onClick=\'priceBookPickList(this)\' align="absmiddle" style="cursor:hand;cursor:pointer" title="Price Book"></td>';
		$output .= '<td WIDTH="1" class="blackLine"><IMG SRC="'.$image_path.'blank.gif"></td>';
		$output .= '<td style="padding:3px;"><input type=text id="txtDiscount'.$i.'" name="txtDiscount'.$i.'" value="'.$discount.'" size="8" onBlur="calcTotal2(this)">%</td>';
		$output .= '<td WIDTH="1" class="blackLine"><IMG SRC="'.$image_path.'blank.gif"></td>';
		...
		$output .= '<input type="hidden" id="hdnOrderlineno'.$i.'" name="hdnOrderlineno'.$i.'" value="'.$orderlineno.'">';
		$output .= '<input type="hidden" id="hdnSOId'.$i.'" name="hdnSOId'.$i.'" value="'.$focus->id.'">';
		$output .= '<input type="hidden" id="hdnStockId'.$i.'" name="hdnStockId'.$i.'" value="'.$stockno.'">';
		$output .= '<input type="hidden" id="hdnItemTax'.$i.'" name="hdnItemTax'.$i.'" value="'.$taxes.'">';
		$output .= '<input type="hidden" id="hdnItemTaxTotal'.$i.'" name="hdnItemTaxTotal'.$i.'" value="'.$taxestotal.'">';
		$output .= '<input type="hidden" id="hdnProductId'.$i.'" name="hdnProductId'.$i.'" value="'.$productid.'">';
		$output .= '<input type="hidden" id="hdnDiscount'.$i.'" name="hdnDiscount'.$i.'" value="'.$discount3.'">';
...
function getAssociatedAccounts($module,$focus,$seid='')
{
	global $adb;
	$output = '';
	global $theme;
	...
	$image_path=$theme_path."images/";
		...
		$query="select contactaccountrel.contactid, contactaccountrel.accountid, account.accountname from contactaccountrel left join account on account.accountid=contactaccountrel.accountid inner join crmentity on crmentity.crmid=account.accountid where crmentity.deleted=0 and account.accountid=".$seid;
	...
	for($i=1;$i<=$num_rows;$i++)
	{
		...
		$output .= '<tr id="row'.$i.'" class="'.$row_class.'">';
		$output .= '<td height="25" style="padding:3px;" nowrap><input id="txtProduct'.$i.'" name="txtProduct'.$i.'" type="text" size="30" readonly value="'.$accountname.'"> <img src="'.$image_path.'search.gif" onClick=\'productPickList(this)\' align="absmiddle" style=\'cursor:hand;cursor:pointer\'></td>';
		$output .= '<td WIDTH="1" class="blackLine"><IMG SRC="'.$image_path.'blank.gif"></td>';
		$output .= '<td style="padding:3px;"><input type=text id="txtQty'.$i.'" name="txtQty'.$i.'" size="7" value="'.$qty.'" onBlur=\'calcTotal(this)\'></td>';
		$output .='<td WIDTH="1" class="blackLine"><IMG SRC="'.$image_path.'blank.gif"></td>';
		$output .= '<td style="padding:3px;"><div id="unitPrice'.$i.'">'.$unitprice.'</div>&nbsp;</td>';
...
function getNoOfAssocProducts($module,$focus,$seid='')
{
	...
	if($module == 'Quotes')
	{
		$query="select stockmaster.description,stockmaster.unit_price,quotesproductrel.* from quotesproductrel inner join stockmaster on stockmaster.productid=quotesproductrel.productid where quoteid=".$focus->id;
	}
	elseif($module == 'Orders')
	{
		$query="select stockmaster.description,stockmaster.unit_price,poproductrel.* from poproductrel inner join stockmaster on stockmaster.productid=poproductrel.productid where purchaseorderid=".$focus->id;
	}
	elseif($module == 'SalesOrder')
	{
		$query="select stockmaster.description,stockmaster.unit_price,salesorderdetails.* from salesorderdetails inner join stockmaster on stockmaster.productid=salesorderdetails.productid where salesorderid=".$focus->id;
	}
	elseif($module == 'Invoice')
	{
		$query="select stockmaster.description,stockmaster.unit_price,invoiceproductrel.* from invoiceproductrel inner join stockmaster on stockmaster.productid=invoiceproductrel.productid where invoiceid=".$focus->id;
	}
	elseif($module == 'Potentials')
	{
		$query="select stockmaster.description,stockmaster.unit_price,seproductsrel.* from stockmaster inner join seproductsrel on seproductsrel.productid=stockmaster.productid where crmid=".$seid;
	}	
	elseif($module == 'Products')
	{
		$query="select stockmaster.stockid,stockmaster.description,stockmaster.unit_price, crmentity.* from stockmaster inner join crmentity on crmentity.crmid=stockmaster.productid where crmentity.deleted=0 and productid=".$seid;
	}
...
function getNoOfAssocAccounts($module,$focus,$seid='')
{
	global $adb;
	$output = '';
	
		$query="select contactaccountrel.contactid, contactaccountrel.accountid, account.accountname from contactaccountrel inner join account on account.accountid=contactaccountrel.accountid where contactid=".$focus->id;
...
function getListPrice($productid,$pbid)
{
	...
	global $adb;
	$query = "select price from prices where pricebookid=".$pbid." and productid=".$productid;
	$result = $adb->query($query);
	$lp = $adb->query_result($result,0,'price');
	return $lp;
}
...
function getUnitPrice($productid)
{
        ...
        global $adb;
        $query = "select stockmaster.unit_price from stockmaster where productid=".$productid;
...
function getDetailAssociatedProducts($module,$focus)
{
    ...
    $output .= '<td width="33%" style="padding:3px;">Product</td>';
    $output .= '<td WIDTH="1" class="blackLine"><IMG SRC="'.$image_path.'blank.gif"></td>';
    if($module != 'Orders')
    {
    	$output .= '<td width="8%" style="padding:3px;">Qty In Stock</td>';
    	$output .= '<td WIDTH="1" class="blackLine"><IMG SRC="'.$image_path.'blank.gif"></td>';
    }	
    $output .= '<td width="8%" style="padding:3px;">Qty Ordered</td>';
    $output .= '<td WIDTH="1" class="blackLine"><IMG SRC="'.$image_path.'blank.gif"></td>';
    $output .= '<td width="10%" style="padding:3px;">Unit Price</td>';
    $output .= '<td WIDTH="1" class="blackLine"><IMG SRC="'.$image_path.'blank.gif"></td>';
    $output .= '<td width="13%" style="padding:3px;">List Price</td>';
    $output .= '<td WIDTH="1" class="blackLine"><IMG SRC="'.$image_path.'blank.gif"></td>';
    $output .= '<td width="10%" style="padding:3px;">Discount</td>';
    $output .= '<td WIDTH="1" class="blackLine"><IMG SRC="'.$image_path.'blank.gif"></td>';
	...
	if($module == 'Quotes')
	{
	//	$query="select stockmaster.description,stockmaster.unit_price,stockmaster.qtyinstock,quotesproductrel.* from quotesproductrel inner join stockmaster on stockmaster.productid=quotesproductrel.productid where quoteid=".$focus->id;
		$query="select 	stockmaster.description,stockmaster.unit_price,stockmaster.taxcatid,stockmaster.qtyinstock,quotes.accountid,quotes.fromstkloc,locations.taxprovinceid,custbranch.taxgroupid,quotesproductrel.* from quotesproductrel inner join stockmaster on stockmaster.productid=quotesproductrel.productid INNER JOIN quotes ON quotes.quoteid = quotesproductrel.quoteid INNER JOIN locations ON locations.loccode=quotes.fromstkloc INNER JOIN custbranch ON quotes.accountid = custbranch.accountid where quotesproductrel.quoteid=".$focus->id;
			$vtlog->logthis("in getDetailAssociatedProducts 5228 - ".$focus->id,'debug');

	}
	elseif($module == 'Orders')
	{
		$query="select stockmaster.description,stockmaster.unit_price,stockmaster.unitprice,stockmaster.qtyinstock,poproductrel.* from poproductrel inner join stockmaster on stockmaster.productid=poproductrel.productid where purchaseorderid=".$focus->id;
...
//		$stock_id=getStockNo($productid);
//		$qtyinstock=getStock($stock_id);
		//$qtyindemand = getDemand($stock_id);
...
//		$discount = number_format(($discount2 * 100),2);
		...
		if($module != 'Orders')
		{	
                	$output .= '<td style="padding:3px;">'.$qtyavailable.'</td>';
	        	$output .= '<td WIDTH="1" class="blackLine"><IMG SRC="'.$image_path.'blank.gif"></td>';
		}
        	...
        	$output .= '<td WIDTH="1" class="blackLine"><IMG SRC="'.$image_path.'blank.gif"></td>';
        	$output .= '<td style="padding:3px;">'.$discount.' %</td>';
        	$output .= '<td WIDTH="1" class="blackLine"><IMG SRC="'.$image_path.'blank.gif"></td>';
        	$output .= '<td style="padding:3px;"><div id="total'.$i.'" align="right">'.$total.'</div></td>';
                $output .= '</tr>';
...
/*
		...
		$output .= '<td width="13%">'.$productname.'</td>';
		if($module != 'Orders')
		{	
                	$output .= '<td  width="7%">'.$qtyavailable.'</td>';
	        	$output .= '<td WIDTH="1" class="blackLine"><IMG SRC="'.$image_path.'blank.gif"></td>';
		}
	     $output .='<td width="7%">'.$qty.'</td><td width="10%" >'.$unitprice.'</td><td width="10%">'.$listprice.'</td><td width="10%">'.$discount.'</td><td width="12%">'.$total.'</td></tr>';
...
*/		
	...
	$output .= '<td width="150"></td>';
      	$output .= '<td>';
		$output .= '<table border="0" cellpadding="0" cellspacing="0" align="right"><tr><td><div align="right"><b>';
		$output .= 'Discount Total:</b></div></td><td><div align="right"><div id="txtDiscountTotal" align="right">';
		$output .= '&nbsp;'.$focus->column_fields['hdnDiscountTotal'].'</div></div></td><td><div align="right"><b>';
		$output .= '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Sub Total:</b></div></td></tr></table>';
		$output .= '</td>';
        ...
        $output .= '<tr>'; 
	$output .=  '<td>&nbsp;</td>';
        $output .= '<td><div align="right"><b>GST:</b></div></td>';
        $output .= '<td width="150"><div align="right" style="border:1px solid #000;padding:2px">&nbsp;'.$focus->column_fields['txtTax'].'</div></td>';
      $output .= '</tr>';

      $output .= '<tr>'; 
      $output .= '<td>&nbsp;</td>';			
      $output .= '<td><div align="right"><b>Freight Charge:</b></div></td>';
      $output .= '<td width="150"><div align="right"><div align="right" style="border:1px solid #000;padding:2px">&nbsp;'.$focus->column_fields['freightcost'].'</div></td>';
      $output .= '</tr>';
      $output .= '<tr>'; 
      $output .= '<td>&nbsp;</td>';
      $output .= '<td><div align="right"><b>Freight Tax:</b></div></td>';
      $output .= '<td width="150"><div align="right"><div align="right" style="border:1px solid #000;padding:2px">&nbsp;'.$focus->column_fields['freighttax'].'</div></td>';
      $output .= '</tr>';
      $output .= '<tr>'; 
      $output .= '<td>&nbsp;</td>';
...
/*	
		...
		$output .= '<tr><td width="15%" class="dataLabel" colspan="4">Freight Charge:</td><td width="15%" class="dataLabel">'.$focus->column_fields['freightcost'].'</td></tr>';
...
function getConvertQuoteToSoObject($focus,$quote_focus,$quoteid)
{
	...
	$vtlog->logthis("in getConvertQuoteToSoObject ".$quoteid,'info');  
        ...
        $focus->column_fields['bill_street2'] = $quote_focus->column_fields['bill_street2'];
        $focus->column_fields['ship_street2'] = $quote_focus->column_fields['ship_street2'];
...
function getProductImageName($id)
{
	global $adb;
	global $vtlog;
	$query = "select imagename from stockmaster where productid=".$id;
	$result = $adb->query($query);
...
function logQuery($del_query)
{
global $vtlog;
$vtlog->logthis("in logQUERY ".$del_query,'info'); 
}
...
{global $vtlog;
 ...
 $vtlog->logthis("in updateSubTotal ".$query,'info');  
...
function getInventoryTotal($return_module,$id)
{
	...
	{
		$query ="select stockmaster.description,stockmaster.unit_price,stockmaster.qtyinstock,seproductsrel.* from stockmaster inner join seproductsrel on seproductsrel.productid=stockmaster.productid where crmid=".$id;
	}
	elseif($return_module == "Products")
	{
		$query="select stockmaster.productid,stockmaster.description,stockmaster.unit_price,stockmaster.qtyinstock,crmentity.* from stockmaster inner join crmentity on crmentity.crmid=stockmaster.productid where crmentity.deleted=0 and productid=".$id;
	}
	$result = $adb->query($query);
	$num_rows=$adb->num_rows($result);
	$total=0;
...
{
	global $adb;
	$query= "update stockmaster set qtyinstock=".$upd_qty." where productid=".$product_id;
        $adb->query($query);
...
function getDebtorName($debtor_no)
{
global $vtlog;
$vtlog->logthis("in getDebtorName ".$debtor_no,'info');  
        global $adb;
        $sql = "select debtorsmaster.name, debtorsmaster.debtorno, custbranch.debtorno, custbranch.branchcode from custbranch inner join debtorsmaster on custbranch.debtorno=debtorsmaster.debtorno where custbranch.debtorno='".$debtor_no."'";
		$result = $adb->query($sql);
        $debtor_name = $adb->query_result($result,0,"name");
        return $debtor_name;
}
...
global $vtlog;
        global $adb;
        $sql = "select id from crmentity_seq";
		$result = $adb->query($sql);
        $crmseqid = $adb->query_result($result,0,"id");
        return $crmseqid;
}

function getSearchListViewEntries1($focus, $module,$list_result,$navigation_array)
{
	global $adb;
	$noofrows = $adb->num_rows($list_result);
	$list_header = '';
	...
	for ($i=$navigation_array['start']; $i<=$navigation_array['end_val']; $i++)
	{
		...
		$list_header .= '<td WIDTH="1" class="blackLine"><IMG SRC="'.$image_path.'blank.gif"></td>';
		foreach($focus->search_fields1 as $name=>$tableinfo)
		{
			$fieldname = $focus->search_fields_name1[$name];
			global $profile_id;
			$query = "select profile2field.* from field inner join profile2field on field.fieldid=profile2field.fieldid where profile2field.tabid=".$tabid." and profile2field.profileid=".$profile_id." and field.fieldname='".$fieldname."'";
			$result = $adb->query($query);
			...
			if($adb->query_result($result,0,"visible") == 0 && $adb->query_result($result_def,0,"visible") == 0)
			{
				...
				else
				{
					...
					elseif($name=='Account Name' && ($module == 'Potentials' || $module == 'SalesOrder' || $module == 'Quotes'))
                                        {
                                                $account_id = $adb->query_result($list_result,$i-1,"accountid");
                                                $account_name = getDebtorName($account_id);
                                                $value = $account_name;
                                        }
					elseif($name=='Quote Name' && $module == 'SalesOrder')
                                        {
...
function getSearchListViewHeader1($focus, $module,$sort_qry='',$sorder='',$order_by='')
{
	global $adb;
	global $theme;
	global $app_strings;
...
function getCustBranch($value)
{
global $vtlog;
			$vtlog->logthis("value2 selected is  ".$value2 ,'info');  

$vtlog->logthis("requested accountid is  ".$_REQUEST['account_id'],'info');  

$vtlog->logthis("record_id selected is  ".$col_fields["record_id"] ,'info');  

$vtlog->logthis("account_id selected is  ".$col_fields["accountid"] ,'info');  

$vtlog->logthis("branchcode selected is  ".$col_fields["branchcode"] ,'info');  

$vtlog->logthis("branchcode selected is  ".$_REQUEST["branchcode"] ,'info');  


$vtlog->logthis("in getCustBranch ".$value,'info');  
        global $adb;
        $sql = "select branchcode from custbranch where accountid='".$value."'";
		$result = $adb->query($sql);
        $branchcode = $adb->query_result($result,0,"branchcode");
        return $branchcode;
}
...
global $vtlog;
$vtlog->logthis("in getOrderNo ",'info');  
        global $adb;
        $sql = "select soid from so_seq";
		$result = $adb->query($sql);
        $temp_id = $adb->query_result($result,0,"soid");
 $vtlog->logthis("in getDebtorNo ".$temp_id,'info');  
       
$orderno = $temp_id + 1;
$vtlog->logthis("in getOrderno ".$orderno,'info');  
 
$sql1 = "update so_seq set soid='".$orderno."'";
		$result1 = $adb->query($sql1);
 $vtlog->logthis("in getOrderno ".$sql1,'info');  
...
function getOrderNoSpec ($record1){
...
$vtlog->logthis("in getOrderNo ",'info');  
        global $adb;
        $sql = "select orderno from salesorders where salesorderid='".$record1."'";
		$result = $adb->query($sql);
        $order_no_spec = $adb->query_result($result,0,"orderno");
 $vtlog->logthis("in getOrderNoSpec ".$order_no_spec,'info');  
...
return $order_no_spec;
}
function getDebug ($debug1,$debug2,$debug3){

/*Gets the Product ID for VTiger */

global $vtlog;
 
 $vtlog->logthis("in getDebug1 ".$debug1,'info');  
$vtlog->logthis("in getDebug2 ".$debug2,'info');  
$vtlog->logthis("in getDebug3 ".$debug3,'info');  

$debug4='done';
return $debug4;
}

function getDebtorNo($branch_code)
{
global $vtlog;
$vtlog->logthis("in getDebtorNo ".$branch_code,'info');  
        global $adb;
        $sql = "select debtorsmaster.name, debtorsmaster.debtorno, custbranch.debtorno, custbranch.branchcode from custbranch inner join debtorsmaster on custbranch.debtorno=debtorsmaster.debtorno where custbranch.branchcode='".$branch_code."'";
		$result = $adb->query($sql);
        $debtor_no = $adb->query_result($result,0,"debtorno");
        return $debtor_no;
}
...
{
global $vtlog;
$vtlog->logthis("in getStock ".$stock_id,'info');  
        global $adb;
		$sqlflag = "SELECT * from stockmaster where stockid = '". $stock_id ."' ";

		$resultflag = $adb->query($sqlflag);
        $mbflag = $adb->query_result($resultflag,0,"mbflag");
        $desc = $adb->query_result($resultflag,0,"categoryid");
 $vtlog->logthis("mbflag is  ".$mbflag,'info');  
 $vtlog->logthis("desc is  ".$desc,'info');  
      
	    if ($mbflag == 'A')
		{
		 $vtlog->logthis("MBFLAG ROUTINE  ",'info');  

		$sqlbom = "SELECT * from bom where parent='". $stock_id ."' and component like '". $desc ."%' ";
			$resultbom = $adb->query($sqlbom);
        	$bom = $adb->query_result($resultbom,0,"component");
       		$bomqty = $adb->query_result($resultbom,0,"quantity");
        	 $vtlog->logthis("bom is  ".$bom,'info');  
			 $vtlog->logthis("bomqty is  ".$bomqty,'info');  

			
			$sql = "SELECT SUM( quantity ) AS dem FROM locstock WHERE locstock.stockid = '". $bom ."' ";
			$result = $adb->query($sql);
        $stockValue1 = $adb->query_result($result,0,"dem");
		$stockValue2 = $stockValue1/$bomqty;
         $vtlog->logthis("stockValue1 is  ".$stockValue1,'info');  
			 $vtlog->logthis("stockValue2 is  ".$stockValue2,'info');  

		
		$stockValue = number_format($stockValue2, 0, '.', '');
        
		}
		else
		{
           	$sql = "SELECT SUM( quantity ) AS dem FROM locstock WHERE locstock.stockid = '". $stock_id ."' ";
			$result = $adb->query($sql);
        $stockValue = $adb->query_result($result,0,"dem");
		}
		return $stockValue;
}
...
{
global $vtlog;
$vtlog->logthis("in getCategory ".$value,'info');  
        global $adb;
        $sql = "SELECT categorydescription,categoryid from stockcategory where categoryid = '". $value ."' ";

		$result = $adb->query($sql);
        $category = $adb->query_result($result,0,"categorydescription");
        return $category;

}

function getSalesperson($userid1)
{
global $vtlog;
$vtlog->logthis("in getSalesperson ".$value,'info');  
        global $adb;
        $sql = "SELECT salesmanname from salesman where userid = '". $userid1 ."' ";

		$result = $adb->query($sql);
        $salesmanname = $adb->query_result($result,0,"salesmanname");
        $salesperson = $salesmanname;
        return $salesperson;

}

function getSalespersonCode($userid1)
{
global $vtlog;
$vtlog->logthis("in getSalesperson ".$value,'info');  
        global $adb;
        $sql = "SELECT salesmancode from salesman where userid = '". $userid1 ."' ";

		$result = $adb->query($sql);
        $salesmancode = $adb->query_result($result,0,"salesmancode");
		return $salesmancode;

}

function getStockNo($productid)
{
global $vtlog;
$vtlog->logthis("in getStockNo ".$stock_id,'info');  
        global $adb;
        $sql = "SELECT stockid,productid from stockmaster where productid = ". $productid ." ";

		$result = $adb->query($sql);
        $stockno = $adb->query_result($result,0,"stockid");
        return $stockno;
}


function getDemand($stock_id)
{
global $vtlog;
$vtlog->logthis("in getDemand ".$stock_id,'info');  
        global $adb;
        $sql = "SELECT SUM(salesorderdetails.quantity-salesorderdetails.qtyinvoiced) AS dem FROM salesorderdetails, salesorders WHERE salesorders.orderno = salesorderdetails.orderno AND salesorders.quotation=0 AND salesorderdetails.stkcode= '". $stock_id ."' ";

		$result = $adb->query($sql);
        $demandValue = $adb->query_result($result,0,"dem");
        return $demandValue;
}


function getTaxGroup($accountid)
{
global $vtlog;
$vtlog->logthis("in getTaxGroup ".$accountid,'info');  
        global $adb;
        $sql = "SELECT taxgroupid FROM custbranch WHERE accountid = '". $accountid ."' ";
		$result = $adb->query($sql);
        $taxgroupid = $adb->query_result($result,0,"taxgroupid");
        return $taxgroupid;
}

function getAddress2($entity_id)
{
global $vtlog;
$vtlog->logthis("in getAddress2 ".$entity_id,'info');  
        global $adb;
        $sql = "SELECT braddress2 FROM custbranch WHERE accountid = '". $entity_id ."' ";
		$result = $adb->query($sql);
        $braddress2 = $adb->query_result($result,0,"braddress2");
        return $braddress2;
}

function getPostAddress2($entity_id)
{
global $vtlog;
$vtlog->logthis("in getPostAddress2 ".$entity_id,'info');  
        global $adb;
        $sql = "SELECT brpostaddr2 FROM custbranch WHERE accountid = '". $entity_id ."' ";
		$result = $adb->query($sql);
        $brpostaddr2 = $adb->query_result($result,0,"brpostaddr2");
        return $brpostaddr2;
}



function getItemTaxes1($taxgroupid,$dispatchprovince,$taxcatid)
{
global $vtlog;
$vtlog->logthis("in getItemTaxes ".$taxgroupid."-".$dispatchprovince."-".$taxcatid,'info');  
        global $adb;
...
$result1 = $adb->query($sql);

$taxrate1 = $adb->query_result($result1,0,"taxrate");
$taxrate2 = $adb->query_result($result1,1,"taxrate");
$taxontax1 = $adb->query_result($result1,0,"taxontax");
$taxontax2 = $adb->query_result($result1,1,"taxontax");

$vtlog->logthis("in getItemTaxes rates ".$taxrate1."-".$taxrate2."-".$taxontax1."-".$taxontax2,'info');  

if ($taxontax1 == 1 && $taxontax2 == 1)
	{ $taxes=1*$taxrate1+1*$taxrate2-1; }	
elseif ($taxontax1 == 1 && $taxontax2 == 0)
	{ $taxes=1*$taxrate2+1*$taxrate1-1; }	
elseif ($taxontax1 == 0 && $taxontax2 == 1)
	{ $taxes=(1+$taxrate1)*(1+$taxrate2)-1; }	
else
	{ $taxes=$taxrate1+$taxrate2; }	
	return $taxes;

$vtlog->logthis("in getItemTaxes taxes ".$taxes,'info');  
...
function getItemTaxes2($taxgroupid,$taxcatid)
{
global $vtlog;
$vtlog->logthis("in getItemTaxes ".$taxcatid,'info');  
global $adb;
...
if ($taxontax1 == 1 && $taxontax2 == 1)
	{ $taxes1=1*$taxrate1+1*$taxrate2-1; }	
elseif ($taxontax1 == 1 && $taxontax2 == 0)
	{ $taxes1=1*$taxrate2+1*$taxrate1-1; }	
elseif ($taxontax1 == 0 && $taxontax2 == 1)
	{ $taxes1=(1+$taxrate1)*(1+$taxrate2)-1; }	
else
	{ $taxes1=$taxrate1+$taxrate2; }	
	return $taxes1;
}


Function GetHoldReason($DebtorNo) {
global $adb,$noof_group_rows;
	...
	$result = $adb->query($sql);
	$noof_group_rows=$adb->num_rows($result);
	$holdreason=$adb->query_result($result,0,"holdreason");
	return $holdreason;
	

}	

Function GetPBPrice($piece,$entity_id) {
	global $vtlog;
	$vtlog->logthis("in getBPPrice productid ".$piece,'info');  

	global $adb;
	$sql = "select pricebook.*,prices.productid, prices.price, crmentity.crmid, crmentity.smownerid, crmentity.modifiedtime from pricebook inner join prices on prices.pricebookid = pricebook.pricebookid inner join crmentity on crmentity.crmid = pricebook.pricebookid where prices.productid=".$piece." and crmentity.deleted=0 and pricebook.pricebookid='".$entity_id."'";
	$result = $adb->query($sql);
	$PBPrice=$adb->query_result($result,0,"price");
	$vtlog->logthis("in getBPPrice price ".$PBPrice,'info');  
return $PBPrice;
...
Function GetCreditAvailable($DebtorNo) {
	...
	global $adb,$noof_group_rows;
	global $vtlog;
$vtlog->logthis("in GetCreditAvailable ".$DebtorNo,'info');  
	...
	$result = $adb->query($sql);
	$noof_group_rows=$adb->num_rows($result);
$vtlog->logthis("in noof_group_rows ".$noof_group_rows,'info');  
$vtlog->logthis("in sql ".$sql,'info');  

if ($noof_group_rows==1){
		$creditlimit=$adb->query_result($result,0,"creditlimit");
		$balance=$adb->query_result($result,0,"balance");
		$CreditAvailable = $creditlimit-$balance;
$vtlog->logthis("in creditlimit ".$creditlimit,'info');  
$vtlog->logthis("in balance ".$balance,'info');  

	} 
else {
	

		$sql2 = "SELECT creditlimit
				FROM debtorsmaster 
				WHERE debtorno='" . $DebtorNo . "'";
				
		
	$result2 = $adb->query($sql2);
	$noof_group_rows2=$adb->num_rows($result2);
	$CreditAvailable=$adb->query_result($result2,0,"creditlimit");
	$vtlog->logthis("in sql2 ".$sql2,'info');  
$vtlog->logthis("in CreditAvailable ".$CreditAvailable,'info');  
	
	}
	
	$sql3 = "SELECT SUM(salesorderdetails.unitprice *
	...
	$noof_group_rows3=$adb->num_rows($result3);
	$CreditAvailable -= $adb->query_result($result3,0,"ordervalue");
	
	return $CreditAvailable;
	
}

function GetPrice ($StockID, $DebtorNo, $BranchCode){
global $vtlog;
$vtlog->logthis("in getPrice stock id ".$StockID,'info');  
$vtlog->logthis("in getPrice debtorno ".$DebtorNo,'info');  
$vtlog->logthis("in getPrice branchcode ".$BranchCode,'info');  
global $adb;
	$unitprice = 0;
...
$sql="SELECT prices.price,
	...
	$vtlog->logthis("in getPrice rowcount1 ".$rowcount,'info');  
	...
	if ($rowcount==0){
		...
		$result = $adb->query($sql);
		$rowcount=$adb->num_rows($result);
		$vtlog->logthis("in getPrice rowcount2 ".$rowcount,'info');  
	...
	$vtlog->logthis("in getPrice rowcount_final ".$rowcount,'info');  

	if ($rowcount!=0){
		/*There is a price from one of the above so return that */
		$unitprice = $adb->query_result($result,0,"price");
		$vtlog->logthis("in getPrice SUCCESS ".$price,'info');  
