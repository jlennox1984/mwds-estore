...
for($i = 0; $i < $company_name_count; $i++)
{
	...
	$whitespace = array(" ", ".", "&", "\/");
	$website = str_replace($whitespace, "", strtolower($account->column_fields["brname"]));
...
for($i=0; $i<10; $i++)
{
	...
	else
	{
        ...
        $product->column_fields["description"] 	= 	$product_name_array[$i];
        $product->column_fields["stockid"] 	= 	$product_code_array[$i];
//        $product->column_fields["manufacturer"]	= 	$manufacturer;

	$product->column_fields["categoryid"] = 	$category;
...
//        $product->column_fields["productsheet"] =	"";
	...
	$contact_key = array_rand($contact_ids);
        ...
        $product->column_fields["taxcatid"] 	= 	'SalesTax';
