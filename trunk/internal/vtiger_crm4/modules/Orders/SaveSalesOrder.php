...
$local_log =& LoggerManager::getLogger('index');
        ...
        $bufferclearquery1 = "delete from salesorders where orderno=0";
        $adb->query($bufferclearquery1);
...
$order_type = $focus->column_fields["ordertype"];
$subject = $focus->column_fields["subject"];
$acct_name = getAccountName($acct_id);
$custbranch = getCustBranch($acct_id);
$debtorno = getDebtorNo($custbranch);  
//$deliverydate = date('d-m-Y');
$query1a = "select orddate from salesorders where salesorderid=".$focus->id;
$result1a = $adb->query($query1a);
$orddate1 = $adb->query_result($result1a,0,"orddate");
if ($orddate1 = '0000-00-00' || $orddate1 < '2004-01-01')		
{ $orddate = date('Y-m-d'); }
else { $orddate = $orddate1; }
if($focus->column_fields["orderno"] == '')
{
        $orderno = getOrderNo();
}
...
{
$orderno = $focus->column_fields["orderno"];

}
$vtlog->logthis("orddate is ".$orddate,'info'); 
$vtlog->logthis("debtorno is ".$debtorno,'info'); 
$vtlog->logthis("acct_name is ".$acct_name,'info'); 
 $vtlog->logthis("orderno is ".$orderno,'info'); 
 $vtlog->logthis("ordernospec is ".$ordernospec,'info'); 
...
$query2 = 'update salesorders set orderno="'.$orderno.'", customerref="' . $subject . '", debtorno="'.$debtorno.'", branchcode="'.$custbranch.'", orddate="'.$orddate.'", deliverydate="'.$orddate.'", ordertype="'.$order_type.'", deliverto="'.$acct_name.'" where salesorderid='.$focus->id;
