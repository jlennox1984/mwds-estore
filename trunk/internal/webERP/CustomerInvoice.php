...
/*The customer transaction uses the CustTrans class to hold the information about the invoice
the CustTrans class contains an array of GRNs objects - containing details of GRNs for invoicing 
...
$title = _('Enter Customer Invoice');

include('includes/header.inc');
include('includes/SQL_CommonFunctions.inc');

echo "<A HREF='" . $rootpath . '/SelectCustomer2.php?' . SID . "'>" . _('Back to Customers') . '</A><BR>';

if (isset($_GET['CustomerID'])){

 /*It must be a new invoice entry - clear any existing invoice details from the CustTrans object and initiate a newy*/
	if (isset( $_SESSION['CustTrans'])){
		unset ( $_SESSION['CustTrans']->GRNs);
		unset ( $_SESSION['CustTrans']->GLCodes);
		unset ( $_SESSION['CustTrans']);
	}

	 if (isset( $_SESSION['CustTransTmp'])){
		unset ( $_SESSION['CustTransTmp']->GRNs);
		unset ( $_SESSION['CustTransTmp']->GLCodes);
		unset ( $_SESSION['CustTransTmp']);
	}
	 Session_register('SuppInv');
	 $_SESSION['CustTrans'] = new CustTrans;

/*Now retrieve customer information - name, currency, default ex rate, terms, tax rate etc */
	 ...
	 $sql = "SELECT custbranch.brname,
	 	...
	 	FROM custbranch,
			...
			currencies,
			taxauthorities
	 	WHERE custbranch.taxgroupid=taxgroups.taxgroupid
		AND custbranch.currcode=currencies.currabrev
	 	AND custbranch.debtorno=debtorsmaster.debtorno
	 	AND custbranch.branchcode = '" . $_GET['CustomerID'] . "'";

	$ErrMsg = _('The customer record selected') . ': ' . $_GET['CustomerID'] . ' ' ._('cannot be retrieved because');
	...
	$_SESSION['CustTrans']->CustomerName = $myrow['brname'];
	$_SESSION['CustTrans']->DebtorNo = $myrow['debtorno'];
	$_SESSION['CustTrans']->CurrCode = $myrow['currcode'];
	$_SESSION['CustTrans']->ExRate = $myrow['exrate'];
	$_SESSION['CustTrans']->TaxGroup = $myrow['taxgroupid'];
	$_SESSION['CustTrans']->TaxGroupDescription = $myrow['taxgroupdescription'];
	
	
	$_SESSION['CustTrans']->CustomerID = $_GET['CustomerID'];
	...
	if(DB_num_rows($LocalTaxProvinceResult)==0){
		prnMsg(_('The tax province associated with your user account has not been set up in this database. Tax calculations are based on the tax group of the customer and the tax province of the user entering the invoice. The system administrator should redefine your account with a valid default stocking location and this location should refer to a valid tax provincce'),'error');
		include('includes/footer.inc');
		exit;
	}
	
	$LocalTaxProvinceRow = DB_fetch_row($LocalTaxProvinceResult);
	$_SESSION['CustTrans']->LocalTaxProvince = $LocalTaxProvinceRow[0];
	
	$_SESSION['CustTrans']->GetTaxes();
	
	
	$_SESSION['CustTrans']->GLLink_Creditors = $_SESSION['CompanyRecord']['gllink_creditors'];
	$_SESSION['CustTrans']->GRNAct = $_SESSION['CompanyRecord']['grnact'];
	$_SESSION['CustTrans']->CreditorsAct = $_SESSION['CompanyRecord']['debtorsact'];

	$_SESSION['CustTrans']->InvoiceOrCredit = 'Invoice';

} elseif (!isset( $_SESSION['CustTrans'])){

	prnMsg( _('To enter a customer invoice the customer must first be selected from the customer selection screen'),'warn');
	echo "<BR><A HREF='$rootpath/SelectCustomer2.php?" . SID ."'>" . _('Select A Customer to Enter an Invoice For') . '</A>';
	include('includes/footer.inc');
	exit;

	/*It all stops here if there ain't no customer selected */
}

/* Set the session variables to the posted data from the form if the page has called itself */
if (isset($_POST['ExRate'])){
	$_SESSION['CustTrans']->ExRate = $_POST['ExRate'];
	$_SESSION['CustTrans']->Comments = $_POST['Comments'];
	$_SESSION['CustTrans']->TranDate = $_POST['TranDate'];

	if (substr( $_SESSION['CustTrans']->Terms,0,1)=='1') { /*Its a day in the following month when due */
		$_SESSION['CustTrans']->DueDate = Date($_SESSION['DefaultDateFormat'], Mktime(0,0,0,Date('m')+1, substr( $_SESSION['CustTrans']->Terms,1),Date('y')));
	} else { /*Use the Days Before Due to add to the invoice date */
		$_SESSION['CustTrans']->DueDate = Date($_SESSION['DefaultDateFormat'], Mktime(0,0,0,Date('m'),Date('d') + (int) substr( $_SESSION['CustTrans']->Terms,1),Date('y')));
	}

	$_SESSION['CustTrans']->SuppReference = $_POST['SuppReference'];

	if ( $_SESSION['CustTrans']->GLLink_Creditors == 1){
...
if the link is not active then OvAmount must be entered manually. */
		...
		$_SESSION['CustTrans']->OvAmount = 0; /* for starters */
		if (count($_SESSION['CustTrans']->GRNs) > 0){
			foreach ( $_SESSION['CustTrans']->GRNs as $GRN){
				$_SESSION['CustTrans']->OvAmount = $_SESSION['CustTrans']->OvAmount + ($GRN->This_QuantityInv * $GRN->ChgPrice);
			}
		}
		if (count($_SESSION['CustTrans']->GLCodes) > 0){
			foreach ( $_SESSION['CustTrans']->GLCodes as $GLLine){
				$_SESSION['CustTrans']->OvAmount = $_SESSION['CustTrans']->OvAmount + $GLLine->Amount;
			}
		}
		if (count($_SESSION['CustTrans']->Shipts) > 0){
			foreach ( $_SESSION['CustTrans']->Shipts as $ShiptLine){
				$_SESSION['CustTrans']->OvAmount = $_SESSION['CustTrans']->OvAmount + $ShiptLine->Amount;
			}
		}
		if (count($_SESSION['CustTrans']->ProdRuns) > 0){
			foreach ( $_SESSION['CustTrans']->ProdRuns as $ProdRunsLine){
				$_SESSION['CustTrans']->OvAmount = $_SESSION['CustTrans']->OvAmount + $ProdRunsLine->Amount;
			}
		}
		$_SESSION['CustTrans']->OvAmount = round($_SESSION['CustTrans']->OvAmount,2);
	}else {
/*OvAmount must be entered manually */
		 $_SESSION['CustTrans']->OvAmount = round($_POST['OvAmount'],2);
	}
}
...
if (!isset($_POST['PostInvoice'])){
	...
	if ($_POST['ProdRuns'] == _('Enter Against Production Run')){
		/*This ensures that any changes in the page are stored in the session before calling the shipments page */
		echo "<META HTTP-EQUIV='Refresh' CONTENT='0; URL=" . $rootpath . "/SuppPRChgs.php?" . SID . "'>";
		echo '<P>' . _('You should automatically be forwarded to the entry of invoices against goods received page') .
			'. ' . _('If this does not happen') . ' (' . _('if the browser does not support META Refresh'). ') ' .
			"<A HREF='" . $rootpath . "/SuppPRChgs.php?" . SID . "'>" . _('click here') . '</a> ' . _('to continue') . '.<BR>';
		exit;
	}
	if ($_POST['GL'] == _('Enter General Ledger Analysis')){
		/*This ensures that any changes in the page are stored in the session before calling the shipments page */
		echo "<META HTTP-EQUIV='Refresh' CONTENT='0; URL=" . $rootpath . "/CustTransGLAnalysis.php?" . SID . "'>";
		echo '<P>' . _('You should automatically be forwarded to the entry of invoices against the general ledger page') .
			'. ' . _('If this does not happen') . ' (' . _('if the browser does not support META Refresh'). ') ' .
			"<A HREF='" . $rootpath . "/CustTransGLAnalysis.php?" . SID . "'>" . _('click here') . '</a> ' . _('to continue') . '.<BR>';
		exit;
	}
	
	/* everything below here only do if a customer is selected
	fisrt add a header to show who we are making an invoice for */
	echo "<CENTER><TABLE BORDER=2 COLSPAN=4><TR><TD CLASS='tableheader'>" . _('Customer') .
		"</TD><TD CLASS='tableheader'>" . _('Currency') .
		"</TD><TD CLASS='tableheader'>" . _('Tax Authority') . '</TD></TR>';
	
	echo '<TR><TD><FONT COLOR=blue><B>' . $_SESSION['CustTrans']->CustomerID . ' - ' .
		$_SESSION['CustTrans']->CustomerName . '</B></FONT></TD>
		<TD ALIGN=CENTER><FONT COLOR=blue><B>' .  $_SESSION['CustTrans']->CurrCode . '</B></FONT></TD>
		<TD><FONT COLOR=blue><B>' . $_SESSION['CustTrans']->TaxGroupDescription . '</B></FONT></TD>
		</TR>
		</TABLE>';
	...
	echo '<TR><TD>' . _('Customer Invoice Reference') . ":</TD>
		<TD><FONT SIZE=2><INPUT TYPE=TEXT SIZE=20 MAXLENGTH=20 NAME=SuppReference VALUE='" .
		$_SESSION['CustTrans']->SuppReference . "'></TD>";
	
	if (!isset($_SESSION['CustTrans']->TranDate)){
		$_SESSION['CustTrans']->TranDate= Date($_SESSION['DefaultDateFormat'], Mktime(0,0,0,Date('m'),Date('d')-1,Date('y')));
	}
	echo '<TD>' . _('Invoice Date') . ' (' . _('in format') . ' ' . $_SESSION['DefaultDateFormat'] . ") :</TD>
			<TD><INPUT TYPE=TEXT SIZE=11 MAXLENGTH=10 NAME='TranDate' VALUE=" . $_SESSION['CustTrans']->TranDate . '></TD>';
	
	echo '<TD>' . _('Exchange Rate') . ":</TD>
			<TD><INPUT TYPE=TEXT SIZE=11 MAXLENGTH=10 NAME='ExRate' VALUE=" . $_SESSION['CustTrans']->ExRate . '></TD></TR>';
	echo '</TABLE>';
	...
	//echo "<INPUT TYPE=SUBMIT NAME='ProdRuns' VALUE='" . _('Enter Against Production Run') . "'> ";
	}
	...
	if (count( $_SESSION['CustTrans']->GRNs)>0){   /*if there are any GRNs selected for invoicing then */
		...
		$tableheader = "<TR BGCOLOR=#800000><TD CLASS='tableheader'>" . _('Seq') . " #</TD>
				...
				<TD CLASS='tableheader'>" . _('Price in') . ' ' . $_SESSION['CustTrans']->CurrCode . "</TD>
				<TD CLASS='tableheader'>" . _('Line Total') . ' ' . $_SESSION['CustTrans']->CurrCode . '</TD></TR>';
		echo $tableheader;
	
		$TotalGRNValue = 0;
	
		foreach ($_SESSION['CustTrans']->GRNs as $EnteredGRN){
	...
	if (count( $_SESSION['CustTrans']->Shipts) > 0){   /*if there are any Shipment charges on the invoice*/
		...
		foreach ($_SESSION['CustTrans']->Shipts as $EnteredShiptRef){
	...
	if (count( $_SESSION['CustTrans']->ProdRuns) > 0){   /*if there are any Shipment charges on the invoice*/
		...
		$TotalProdRunsValue = 0;
	
		foreach ($_SESSION['CustTrans']->ProdRuns as $EnteredProdRunsRef){
	
			echo '<TR><TD>' . $EnteredProdRunsRef->ProdRunsRef . '</TD><TD ALIGN=RIGHT>' .
				number_format($EnteredProdRunsRef->Amount,2) . '</TD></TR>';
	
			$TotalProdRunsValue = $TotalProdRunsValue + $EnteredProdRunsRef->Amount;
		...
		echo '<TR><TD COLSPAN=2 ALIGN=RIGHT><FONT SIZE=4 COLOR=blue>' . _('Total') . ':</FONT></TD>
			<TD ALIGN=RIGHT><FONT SIZE=4 COLOR=BLUE><U>' .  number_format($TotalProdRunsValue,2) . '</U></FONT></TD></TR>';
	}
	...
	if ( $_SESSION['CustTrans']->GLLink_Creditors == 1){
		...
		if (count($_SESSION['CustTrans']->GLCodes) > 0){
			...
			$TableHeader = "<TR><TD CLASS='tableheader'>" . _('Account') .
					...
					"</TD><TD CLASS='tableheader'>" . _('Amount') . '<BR>' . _('in') . ' ' . $_SESSION['CustTrans']->CurrCode . "</TD>
		...
		if (!isset($TotalProdRunsValue)){
			$TotalProdRunsValue = 0;
		}
		
		$_SESSION['CustTrans']->OvAmount = $TotalGRNValue + $TotalGLValue + $TotalShiptValue + $TotalProdRunsValue;
		
		echo '<TABLE><TR><TD>' . _('Amount in customer currency') . ':</TD><TD COLSPAN=2 ALIGN=RIGHT>' .
			number_format( $_SESSION['CustTrans']->OvAmount,2) . '</TD></TR>';
	} else {
		echo '<TABLE><TR><TD>' . _('Amount in customer currency') .
	...
	foreach ($_SESSION['CustTrans']->Taxes as $Tax) {
		...
		if (isset($_POST['TaxRate'  . $Tax->TaxCalculationOrder])){
			$_SESSION['CustTrans']->Taxes[$Tax->TaxCalculationOrder]->TaxRate = $_POST['TaxRate'  . $Tax->TaxCalculationOrder]/100;
		}
		...
		if ($_POST['OverRideTax']=='Auto' OR !isset($_POST['OverRideTax'])){
			...
			echo  ' <INPUT TYPE=TEXT NAME=TaxRate' . $Tax->TaxCalculationOrder . ' MAXLENGTH=4 SIZE=4 VALUE=' . $_SESSION['CustTrans']->Taxes[$Tax->TaxCalculationOrder]->TaxRate * 100 . '>%';
			
			/*Now recaluclate the tax depending on the method */
			if ($Tax->TaxOnTax ==1){
				
				$_SESSION['CustTrans']->Taxes[$Tax->TaxCalculationOrder]->TaxOvAmount = $_SESSION['CustTrans']->Taxes[$Tax->TaxCalculationOrder]->TaxRate * ($_SESSION['CustTrans']->OvAmount + $TaxTotal);
			
			} else { /*Calculate tax without the tax on tax */
				
				$_SESSION['CustTrans']->Taxes[$Tax->TaxCalculationOrder]->TaxOvAmount = $_SESSION['CustTrans']->Taxes[$Tax->TaxCalculationOrder]->TaxRate * $_SESSION['CustTrans']->OvAmount;
			
			}
			
			
			echo '<INPUT TYPE=HIDDEN NAME="TaxAmount'  . $Tax->TaxCalculationOrder . '"  VALUE=' . round($_SESSION['CustTrans']->Taxes[$Tax->TaxCalculationOrder]->TaxOvAmount,2) . '>';
			
			echo '</TD><TD ALIGN=RIGHT>' . number_format($_SESSION['CustTrans']->Taxes[$Tax->TaxCalculationOrder]->TaxOvAmount,2);
			
		} else { /*Tax being entered manually accept the taxamount entered as is*/
			
			$_SESSION['CustTrans']->Taxes[$Tax->TaxCalculationOrder]->TaxOvAmount = $_POST['TaxAmount'  . $Tax->TaxCalculationOrder];
			
			echo  ' <INPUT TYPE=HIDDEN NAME=TaxRate' . $Tax->TaxCalculationOrder . ' VALUE=' . $_SESSION['CustTrans']->Taxes[$Tax->TaxCalculationOrder]->TaxRate * 100 . '>';

			echo '</TD><TD><INPUT TYPE=TEXT SIZE=12 MAXLENGTH=12 NAME="TaxAmount'  . $Tax->TaxCalculationOrder . '"  VALUE=' . round($_SESSION['CustTrans']->Taxes[$Tax->TaxCalculationOrder]->TaxOvAmount,2) . '>';
		}
		
		$TaxTotal += $_SESSION['CustTrans']->Taxes[$Tax->TaxCalculationOrder]->TaxOvAmount;
		echo '</TD></TR>';	
	}
	
	$_SESSION['CustTrans']->OvAmount = round($_SESSION['CustTrans']->OvAmount,2);
	
	$DisplayTotal = number_format(( $_SESSION['CustTrans']->OvAmount + $TaxTotal), 2);
	...
	echo '<TABLE><TR><TD>' . _('Comments') . '</TD><TD><TEXTAREA NAME=Comments COLS=40 ROWS=2>' .
		$_SESSION['CustTrans']->Comments . '</TEXTAREA></TD></TR></TABLE>';
...
/*Need to recalc the taxtotal */
	...
	$InputError = False;
	if /*( $TaxTotal + $_SESSION['CustTrans']->OvAmount <= 0){
		$InputError = True;
	...
	} elseif */(strlen( $_SESSION['CustTrans']->SuppReference)<1){
		$InputError = True;
		prnMsg(_('The invoice as entered cannot be processed because the there is no customers invoice number or reference entered') . '. ' . _('The customer invoice number must be entered'),'error');

	} elseif (!is_date( $_SESSION['CustTrans']->TranDate)){
		$InputError = True;
	...
	} elseif (DateDiff(Date($_SESSION['DefaultDateFormat']), $_SESSION['CustTrans']->TranDate, "d") < 0){
		$InputError = True;
	...
	}elseif ( $_SESSION['CustTrans']->ExRate <= 0){
		$InputError = True;
		prnMsg( _('The invoice as entered cannot be processed because the exchange rate for the invoice has been entered as a negative or zero number') . '. ' . _('The exchange rate is expected to show how many of the customers currency there are in 1 of the local currency'),'error');

	}elseif ( $_SESSION['CustTrans']->OvAmount < $$TotalGLValue){
		...
		$sql = "SELECT count(*) 
			FROM debtortrans 
			WHERE debtorno='" . $_SESSION['CustTrans']->DebtorNo . "' 
			AND debtortrans.reference='" . $_POST['SuppReference'] . "'";
	...
	/*Start an SQL transaction */
		...
		$PeriodNo = GetPeriod( $_SESSION['CustTrans']->TranDate, $db);
		$SQLInvoiceDate = FormatDateForSQL( $_SESSION['CustTrans']->TranDate);
			...
			the credit is to creditors control act  done later for the total invoice value + tax*/
				...
				$SQL = 'INSERT INTO gltrans (type, 
						...
						VALUES (10, ' .
							...
							'" . $_SESSION['CustTrans']->CustomerID . ' ' . $EnteredGLCode->Narrative . "', 
							" . round($EnteredGLCode->Amount/ $_SESSION['CustTrans']->ExRate*-1,2) . ", 
				...
				$LocalTotal += round($EnteredGLCode->Amount/ $_SESSION['CustTrans']->ExRate,2);
			}

			foreach ($_SESSION['CustTrans']->Shipts as $ShiptChg){
			...
			these entries are reversed from the GRN suspense when the shipment is closed*/
				...
				$SQL = 'INSERT INTO gltrans (type, 
							...
							VALUES (10, ' .
								...
								'" . $SQLInvoiceDate . "', 
								" . $PeriodNo . ', 
								' . $_SESSION['CustTrans']->GRNAct . ", 
								'" . $_SESSION['CustTrans']->CustomerID . ' ' . _('Shipment charge against') . ' ' . $ShiptChg->ShiptRef . "', 
								" . $ShiptChg->Amount/ $_SESSION['CustTrans']->ExRate . ')';
				...
				$LocalTotal += $ShiptChg->Amount/ $_SESSION['CustTrans']->ExRate;

			}

			foreach ($_SESSION['CustTrans']->ProdRuns as $ProdRunsChg){
			...
			these entries are reversed from the GRN suspense when the shipment is closed*/
				...
				$SQL = 'INSERT INTO gltrans (type, 
							...
							VALUES (10, ' .
								...
								" . $PeriodNo . ', 
								' . $_SESSION['CustTrans']->GRNAct . ", 
								'" . $_SESSION['CustTrans']->CustomerID . ' ' . _('Production Run charge against') . ' ' . $ProdRunsChg->ProdRunsRef . "', 
								" . $ProdRunsChg->Amount/ $_SESSION['CustTrans']->ExRate . ')';

				$ErrMsg = _('CRITICAL ERROR') . '! ' . _('NOTE DOWN THIS ERROR AND SEEK ASSISTANCE') . ': ' . _('The general ledger transaction for the shipment') .
							' ' . $ProdRunsChg->ProdRunsRef . ' ' . _('could not be added because');

				$DbgMsg = _('The following SQL to insert the GL transaction was used');

				$Result = DB_query($SQL, $db, $ErrMsg, $DbgMsg, True);

				$LocalTotal += $ProdRunsChg->Amount/ $_SESSION['CustTrans']->ExRate;
//prnMsg(_('ShiptRef ') . ' ' . $_SESSION['CustTrans']->ShiptRef . ' ' . _(' STAGE 1'),'success');
//prnMsg(_('ProdRunsRef ') . ' ' . $_SESSION['CustTrans']->ProdRunsRef . ' ' . _(' STAGE 1'),'success');
			...
			foreach ($_SESSION['CustTrans']->GRNs as $EnteredGRN){
				...
				if (strlen($EnteredGRN->ShiptRef) == 0 OR $EnteredGRN->ShiptRef == 0 OR strlen($EnteredGRN->ProdRunsRef) == 0 OR $EnteredGRN->ProdRunsRef == 0){ 
				/*so its not a shipment item 
				  enter the GL entry to reverse the GRN suspense entry created on delivery at standard cost used on delivery */
					...
					if ($EnteredGRN->StdCostUnit * $EnteredGRN->This_QuantityInv != 0) {
						$SQL = 'INSERT INTO gltrans (type, 
								...
								VALUES (10, ' . $InvoiceNo . ", 
									...
									" . $PeriodNo . ', 
									' . $_SESSION['CustTrans']->GRNAct . ", 
									'" . $_SESSION['CustTrans']->CustomerID . ' - ' . _('GRN') . ' ' . $EnteredGRN->GRNNo . ' - ' . $EnteredGRN->ItemCode . ' x ' . $EnteredGRN->This_QuantityInv . ' @  ' .
					...
					$PurchPriceVar = round($EnteredGRN->This_QuantityInv * (($EnteredGRN->ChgPrice  / $_SESSION['CustTrans']->ExRate) - $EnteredGRN->StdCostUnit),2);
					...
					if ($PurchPriceVar !=0){ /* don't bother with this lot if there is no difference ! */
						if (strlen($EnteredGRN->ItemCode)>0 OR $EnteredGRN->ItemCode != ''){ /*so it is a stock item */
							...
							if ($_SESSION['WeightedAverageCosting']==1){ /*Weighted Average costing */
								...
								The cost of these items - $EnteredGRN->ChgPrice  / $_SESSION['CustTrans']->ExRate
								*/
								...
								if ($EnteredGRN->This_QuantityInv > $TotalQuantityOnHand){
									...
									$WriteOffToVariances =  ($EnteredGRN->This_QuantityInv
										- $TotalQuantityOnHand)
									* (($EnteredGRN->ChgPrice  / $_SESSION['CustTrans']->ExRate) - $EnteredGRN->StdCostUnit);
									...
									VALUES (10, ' .
									 ...
									 ", '" . $_SESSION['CustTrans']->CustomerID . ' - ' . _('GRN') . ' ' . $EnteredGRN->GRNNo .
									 ' - ' . $EnteredGRN->ItemCode . ' x ' . ($EnteredGRN->This_QuantityInv-$TotalQuantityOnHand) . ' x  ' . _('price var of') . ' ' .
									 number_format(($EnteredGRN->ChgPrice  / $_SESSION['CustTrans']->ExRate) - $EnteredGRN->StdCostUnit,2)  .
									 "', " . $WriteOffToVariances . ')';
								...
								$SQL = 'INSERT INTO gltrans (type, 
									...
									VALUES (10, ' .
									 ...
									 ", '" . $_SESSION['CustTrans']->CustomerID . ' - ' . ('Average Cost Adj') .
								...
								if ($TotalQuantityOnHand>0) {
									...
									$sql = 'UPDATE stockmaster SET lastcost=materialcost+overheadcost, materialcost=materialcost+' . $CostIncrement . ' WHERE stockid="' . $EnteredGRN->ItemCode . '"';
									$Result = DB_query($sql, $db, $ErrMsg, $DbgMsg, True);
								} else {
									$sql = 'UPDATE stockmaster SET lastcost=materialcost+overheadcost,
									materialcost=' . ($EnteredGRN->ChgPrice  / $_SESSION['CustTrans']->ExRate) . ' WHERE stockid="' . $EnteredGRN->ItemCode . '"';
									$Result = DB_query($sql, $db, $ErrMsg, $DbgMsg, True);
								}
					...
					$SQL = 'INSERT INTO gltrans (type, 
									...
									amount) 
							VALUES (10, ' .
							 $InvoiceNo . ", '" . $SQLInvoiceDate . "', " . $PeriodNo . ', ' . $_SESSION['CustTrans']->GRNAct .
							 ", '" . $_SESSION['CustTrans']->CustomerID . ' - ' . _('GRN') . ' ' . $EnteredGRN->GRNNo . ' - ' .
							 $EnteredGRN->ItemCode . ' x ' . $EnteredGRN->This_QuantityInv . ' @ ' .
							 $_SESSION['CustTrans']->CurrCode . $EnteredGRN->ChgPrice . ' @ ' . _('a rate of') . ' ' .
							 $_SESSION['CustTrans']->ExRate . "', " .
							 round(($EnteredGRN->ChgPrice * $EnteredGRN->This_QuantityInv) / $_SESSION['CustTrans']->ExRate,2) . ')';
				...
				$LocalTotal += round(($EnteredGRN->ChgPrice * $EnteredGRN->This_QuantityInv) / $_SESSION['CustTrans']->ExRate,2);
			} /* end of GRN postings */

			if ($debug == 1 AND ( abs($_SESSION['CustTrans']->OvAmount/ $_SESSION['CustTrans']->ExRate) - $LocalTotal) >0.009999){

				echo '<P>' . _('The total posted to the debit accounts is') . ' ' .
						$LocalTotal . ' ' . _('but the sum of OvAmount converted at ExRate') . ' = ' .
						( $_SESSION['CustTrans']->OvAmount / $_SESSION['CustTrans']->ExRate);
			}
			...
			foreach ($_SESSION['CustTrans']->Taxes as $Tax){
				/* Now the TAX account */
						...
						VALUES (10, ' .
							...
							" . $PeriodNo . ',
							' . $Tax->TaxGLCode . ",
						 	'" . $_SESSION['CustTrans']->CustomerID . ' - ' . _('Inv') . ' ' .
						 $_SESSION['CustTrans']->SuppReference . ' ' . $Tax->TaxAuthDescription . ' ' . number_format($Tax->TaxRate*100,2) . '% ' . $_SESSION['CustTrans']->CurrCode .
						 $Tax->TaxOvAmount  . ' @ ' . _('exch rate') . ' ' . $_SESSION['CustTrans']->ExRate .
						 "',
						 	" . round( $Tax->TaxOvAmount/ $_SESSION['CustTrans']->ExRate,2) . ')';
			...
			$SQL = 'INSERT INTO gltrans (type,
							...
							amount)
					VALUES (10, ' .
					 $InvoiceNo . ", '" . $SQLInvoiceDate . "', " . $PeriodNo . ', ' . $_SESSION['CustTrans']->CreditorsAct .
					 ", '" . $_SESSION['CustTrans']->CustomerID . ' - ' . _('Inv') . ' ' .
					 $_SESSION['CustTrans']->SuppReference . ' ' . $_SESSION['CustTrans']->CurrCode .
					 number_format( $_SESSION['CustTrans']->OvAmount + $TaxTotal,2)  .
					 ' @ ' . _('a rate of') . ' ' . $_SESSION['CustTrans']->ExRate . "', " .
					 round(($LocalTotal + ( $TaxTotal / $_SESSION['CustTrans']->ExRate)),2) . ')';
	...
	$SQL = "INSERT INTO debtortrans (
		...
		VALUES (
			". $InvoiceNo . ",
			10,
			'" . $_SESSION['CustTrans']->DebtorNo . "',
			'" . $_SESSION['CustTrans']->CustomerID . "',
			'" . $SQLInvoiceDate . "',
			" . $PeriodNo . ",
			'" . $_SESSION['CustTrans']->SuppReference . "',
			'DE',
			" . round($_SESSION['CustTrans']->OvAmount,2) . ', 
				' . round($TaxTotal,2) . ', 
				
				' .  $_SESSION['CustTrans']->ExRate . ", 
				'" . $_SESSION['CustTrans']->Comments . "'
		)";




prnMsg( _('POST TO DEBTORSTRANS') . '. ' . $SQL,'warn');


		
		$ErrMsg = _('CRITICAL ERROR') . '! ' . _('NOTE DOWN THIS ERROR AND SEEK ASSISTANCE') . ': ' . _('The customer invoice transaction could not be added to the database because');

		$DbgMsg = _('The following SQL to insert the customer invoice was used');

		$Result = DB_query($SQL, $db, $ErrMsg, $DbgMsg, True);
		
		$CustTransID = DB_Last_Insert_ID($db,'debtortrans','id');
		
		/* Insert the tax totals for each tax authority where tax was charged on the invoice */
		foreach ($_SESSION['CustTrans']->Taxes AS $TaxTotals) {
...
prnMsg( _('POST TO DEBTORSTRANSTAXES') . '. ' . $SQL,'warn');

			$ErrMsg =_('CRITICAL ERROR') . '! ' . _('NOTE DOWN THIS ERROR AND SEEK ASSISTANCE') . ': ' . _('The customer transaction taxes records could not be inserted because');
			$DbgMsg = _('The following SQL to insert the customer transaction taxes record was used:');
 			$Result = DB_query($SQL,$db,$ErrMsg,$DbgMsg,true);
		}
		...
		foreach ($_SESSION['CustTrans']->GRNs as $EnteredGRN){
			...
			if (strlen($EnteredGRN->ShiptRef)>0 AND $EnteredGRN->ShiptRef != '0'){
				...
				$SQL = 'INSERT INTO shipmentcharges (shiptref, 
						...
						VALUES (' . $EnteredGRN->ShiptRef . ', 
							...
							' . $InvoiceNo . ", 
							'" . $EnteredGRN->ItemCode . "', 
							" . ($EnteredGRN->This_QuantityInv * $EnteredGRN->ChgPrice) / $_SESSION['CustTrans']->ExRate . ')';
			...
			//		prnMsg( _('PRRef ') . $EnteredGRN->PRRef,'info');
		//prnMsg( _('ProdRunsRef ') . $EnteredGRN->ProdRunsRef,'info');
		//prnMsg( _('Prno ') . $EnteredGRN->PRNo,'info');
	//	prnMsg( _('Prno ') . $EnteredGRN->ProdRunsRef,'info');
		 $sqlspec3="SELECT prref from productionruns where prno=" . $EnteredGRN->PRNo;
			//prnMsg( _('sql ') . $sqlspec3,'info');
	 
	$resultspec3 = DB_query($sqlspec3,$db);
	$myrowspec3 = DB_fetch_row($resultspec3);
	$prnospec3 = $myrowspec3[0];
	//prnMsg( _('prnospec3 ') . $prnospec3,'warn');
	
	if ($prnospec3 == '' || (!isset($prnospec3)))
	{$prnospec3 = 0;}
	//prnMsg( _('prnospec3a ') . $prnospec3,'info');
	
	//prnMsg( _('PRRef2 ') . $prnospec3,'info');
		
		$EnteredGRN->ProdRunsRef=$prnospec3;
		
if (strlen(($EnteredGRN->ProdRunsRef)>0 AND $EnteredGRN->ProdRunsRef != '0')  || (strlen($EnteredGRN->PRNo)>0 AND $EnteredGRN->PRNo != '0')){

		

				/* insert the shipment charge records */

if (($EnteredGRN->PRNo=='') || (!isset($EnteredGRN->PRNo)))
{$EnteredGRN->PRNo=0;}
	
				$SQL = 'INSERT INTO productionruncostingcharges (prref, 
									transtype, 
									transno, 
									stockid, 
									value,
									prno) 
						VALUES (' . $EnteredGRN->ProdRunsRef . ', 
							20, 
							' . $InvoiceNo . ", 
							'" . $EnteredGRN->ItemCode . "', 
							" . ($EnteredGRN->This_QuantityInv * $EnteredGRN->ChgPrice) / $_SESSION['CustTrans']->ExRate . ',
				...
				$ErrMsg = _('CRITICAL ERROR') . '! ' . _('NOTE DOWN THIS ERROR AND SEEK ASSISTANCE') . ': ' . _('The shipment charge record for the production run ') .
							 ' ' . $EnteredGRN->ProdRunsRef . ' ' . _('could not be added because');

				$DbgMsg = _('The following SQL to insert the Shipment charge record was used');
// prnMsg( _('ProdRunRef sql ') . $SQL,'info');
		prnMsg(_('Prod Run ') . ' ' . $SQL . ' ' . _('has been processed'),'success');

				$Result = DB_query($SQL, $db, $ErrMsg, $DbgMsg, True);

			}

		} /* end of the loop to do the updates for the quantity of order items the customer has invoiced */
		...
		foreach ($_SESSION['CustTrans']->Shipts as $ShiptChg){
			...
			$DbgMsg = _('The following SQL to insert the Shipment charge record was used');
// prnMsg(_('Shipment ') . ' ' . $_SESSION['CustTrans']->Shipts . ' ' . _('has been processed'),'success');
		prnMsg(_('Prod Run ') . ' ' . $SQL . ' ' . _('has been processed'),'success');
		...
		//   prnMsg(_('test 4'),'warn');

	// print_r($_SESSION['CustTrans']->ProdRuns);
...
{$EnteredGRN->PRNo=0;}
			...
			$SQL = 'INSERT INTO productionruncostingcharges (prref, 
								...
								value,
								prno) 
					VALUES (' . $ProdRunsChg->ProdRunsRef . ', 
						20, 
						' . $InvoiceNo . ', 
						' . $ProdRunsChg->Amount/ $_SESSION['CustTrans']->ExRate . ',
						' . $EnteredGRN->PRNo . ')';
			...
			$DbgMsg = _('The following SQL to insert the Shipment charge record was used');
prnMsg(_('Prod Run2 ') . ' ' . $_SESSION['CustTrans']->ProdRuns . ' ' . _('has been processed'),'success');
		prnMsg(_('Prod Run2 ') . ' ' . $SQL . ' ' . _('has been processed'),'success');
		...
		unset( $_SESSION['CustTrans']->GRNs);
		unset( $_SESSION['CustTrans']->Shipts);
		unset( $_SESSION['CustTrans']->ProdRuns);
		unset( $_SESSION['CustTrans']->GLCodes);
		unset( $_SESSION['CustTrans']);

		prnMsg(_('Customer invoice number') . ' ' . $InvoiceNo . ' ' . _('has been processed'),'success');
		echo "<P><A HREF='$rootpath/SelectCustomer2.php'>" . _('Select a customer') . '</A>';
	}
