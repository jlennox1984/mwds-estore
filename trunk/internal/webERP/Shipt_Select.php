...
//end if stock search results to show
	...
	if (isset($ShiptRef) && $ShiptRef !="") {
		...
		$SQL = "SELECT DISTINCT shipments.shiptref, shipments.allowprint, vessel, voyageref, suppliers.suppname, shipments.eta, shipments.closed
			FROM shipments INNER JOIN suppliers
				ON shipments.supplierid = suppliers.supplierid
			INNER JOIN purchorderdetails
	...
	if (DB_num_rows($ShipmentsResult)>0){
		...
		while ($myrow=DB_fetch_array($ShipmentsResult)) {
			...
			//  $URL__Shipment_Request = $rootpath . '/Ship_PDFShipment.php?' . SID . 'ShiptRef=' . $myrow['shiptref'];
...
$URL_Shipment_Request = '<a href="' . $rootpath . '/Ship_PDFShipment.php?' . SID . 'ShiptRef=' . $myrow['shiptref'] . '">Print Now</a>';

			
			
		} else {
			$URL_Shipment_Request = '<a href="' . $rootpath . '/Ship_PDFShipment.php?' . SID . 'ShiptRef=' . $myrow['shiptref'] . '">Reprint</a>';
