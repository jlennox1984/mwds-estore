...
if (isset($_GET['SelectedCategory'])){
	...
	if ($SelectedCategory AND $InputError !=1) {
		...
		$msg = _('The stock category record has been updated only CMS Module ');
         //CHECK FOR DULPCACTE ITEMS
       
                 $sql2="select count(*) as checkrec  from jos_vm_category where  ptcode='$ptcode'";
                $result2=DB_query($sql2,$db);
                $myrow2=DB_fetch_array($result2);
                //SAFETY FUNCTION FOR CATEGORY REFENECE TABEL//
         $REC_COUNT=$myrow2['checkrec'];	
	prnMsg( _('Record Count ' .$REC_COUNT));
	} elseif ($InputError !=1 AND  $REC_COUNT  <=0) {
...
// 	$Catid= $_POST['CategoryID'];
	...
	$sql = "INSERT INTO jos_vm_category (
		...
		'" . DB_escape_string($_POST['CategoryName']) . "', 
		 ...
		 $sql2="select count(*) as checkno  from jos_vm_category_xref where  category_child_id='$ptcode'";
		$result2=DB_query($sql2,$db);
		$myrow2=DB_fetch_array($result2);
		//SAFETY FUNCTION FOR CATEGORY REFENECE TABEL//
	 $checkno=$myrow2['checkno'];
		prnMsg( _('Cross Refeence  check ' .$checkno));
		
			 prnMsg( _('Cross Refeence check sql ' .$sql2));
 
			if($checkno <=1) {

                     $sql="INSERT INTO jos_vm_category_xref (category_parent_id,
 						        category_child_id)Values(
							 '0',
						". $ptcode .")";
     $result= DB_query($sql,$db);
}
	...
	if ($myrow[0]>0) {
		...
		if ($myrow[0]>0) {
			...
			if ($myrow[0]>0) {
				...
				$result = DB_query($sql,$db);
                               $result = DB_query($sql,$db);
//_____________________VirturMart Delete Routine___________________________________________________________//

                        $sql="SELECT category_id FROM jos_vm_category where ptcode='$SelectedCategory'";
                        $result=DB_query($sql,$db);
                        $myrow_ref=DB_fetch_row($result);
                        $vmref=$myrow_ref[0];
                          prnMsg( _('Refeence check sql ' .$vmref));
...
//_______________END Virturmart delete Routine JD.Moncrieff 10/29/2008_____________________________________ //
