<?php
/* $Revision: 1.9 $ */
/* Definition of the Customer Transactions class to hold all the information for an accounts payable invoice or credit note
*/
...
Class CustTrans {
	...
	function CustTrans(){
	/*Constructor function initialises a new Customer Transaction object */
		$this->GRNs = array();
		$this->GLCodes = array();
		$this->Shipts = array();
		$this->ProdRuns = array();
	...
	function GetTaxes () {
		...
		/*Gets the Taxes and rates applicable to the tax group of the customer 
...
function Add_ProdRuns_To_Trans($ProdRunsRef, $Amount){
		if ($Amount!=0){
...
Class ProductionRunCosting {
	...
	function ProductionRunCosting ($Counter, $ProdRunsRef, $Amount){
		$this->Counter = $Counter;
		$this->ProdRunsRef = $ProdRunsRef;
		$this->Amount = $Amount;
	}
