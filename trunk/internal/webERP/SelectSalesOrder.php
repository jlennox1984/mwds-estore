...
If (isset($_REQUEST['OrderNumber']) AND $_REQUEST['OrderNumber']!='') {
	...
	if (!is_numeric($_REQUEST['OrderNumber'])){
		...
		echo _('Order Number') . ' - SO' . $_REQUEST['OrderNumber'];
	}
} else {
...
if (!isset($StockID)) {
	...
	if ($_REQUEST['OrderNumber']=='' OR !$_REQUEST['OrderNumber']){
		...
		echo _('Order number') . ": SO<INPUT type=text name='OrderNumber' MAXLENGTH =8 SIZE=9>&nbsp " . _('From Stock Location') . ":<SELECT name='StockLocation'> ";
...
//end if stock search results to show
	...
	while ($myrow=DB_fetch_array($SalesOrdersResult)) {
		...
		$FormatedDelDate = ConvertSQLDate($myrow['duedate']);
