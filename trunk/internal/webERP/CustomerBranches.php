...
//get virtualmart user info
$sqlpre='SELECT userid from user_rel';
$result = DB_query($sqlpre,$db);
	$uid = DB_fetch_row($result);
...
if (isset($_POST['submit'])) {
	...
	if (isset($SelectedBranch) AND $InputError !=1) {
		...
		$sql = "UPDATE custbranch SET brname = '" . DB_escape_string($_POST['BrName']) . "',
                  ...
                  $sql="UPDATE jos_vm_user_info SET  address1= '".DB_escape_string($_POST['BrAddress1']) . "
                             address2 = '" . DB_escape_string($_POST['BrAddress2']) . "',
                             city = '" . DB_escape_string($_POST['BrAddress3']) . "',
                             state = '" . DB_escape_string($_POST['BrAddress4']) . "',
                             zip = '" . DB_escape_string($_POST['BrAddress5']) . "',
                             country ='" . DB_escape_string($_POST['BrAddress5']) . "',
                             WHERE user_id ='$uid'";
...
$sql = "insert into crmentity (crmid,smcreatorid,smownerid,setype,description,createdtime,modifiedtime) values('".$crm_id."','1','1','Accounts','','".$today."','".$today."')";	
...
if (isset($SelectedBranch) AND $InputError !=1) {
					$account_id = getAccountID($SelectedBranch);
//		echo '<BR>'.$account_id._(' RESULTS ').$SelectedBranch;
	
					$today = date("YmdHis");
					
					$sql = "update crmentity set modifiedtime ='".$today."' WHERE crmid = '".$account_id."'";
...
//	echo '<BR>'._('Success')._('no action for accountscf.');


	} elseif ($InputError !=1) {
					$sql = "insert into accountscf (accountid) values (".$crm_id.")";
...
Function GetAccountID ($SelectedBranch){

/*Gets the Product ID for VTiger */
	$Aresult = mysql_query('SELECT accountid FROM custbranch WHERE branchcode = "'.$SelectedBranch.'" ');
if (!$Aresult) {
   die('Could not query:' . mysql_error());
}

$account_id = mysql_result($Aresult, 0); // outputs current crmid
...
Function getCurrCode ($debtorno){

/*Gets the Product ID for VTiger */
	$result = mysql_query('SELECT currcode FROM debtorsmaster where debtorno="' . $debtorno . '"');
if (!$result) {
   die('Could not query:' . mysql_error());
}
$currcode = mysql_result($result, 0); // outputs current crmid
