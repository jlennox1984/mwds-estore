...
if(isset($_REQUEST['query']) && $_REQUEST['query'] != '' && $_REQUEST['query'] == 'true')
{
	...
	{
		array_push($where_clauses, "stockmaster.description like ".PearDatabase::quote($productname.'%'));
		//$search_query .= " and stockmaster.description like '".$productname."%'";
		$url_string .= "&description=".$productname;
	}
	
	if (isset($productcode) && $productcode !='')
	{
		array_push($where_clauses, "stockid like ".PearDatabase::quote($productcode.'%'));
		//$search_query .= " and stockid like '".$stockid."%'";
		$url_string .= "&stockid=".$productcode;
	}
...
//Combo Fields for Manufacturer and Category are moved from advanced to Basic Search
        ...
        if (isset($_REQUEST['advanced']) && $_REQUEST['advanced'] == 'true') 
	{
		...
		$search_form->assign("ALPHABETICAL",AlphabeticalSearch('Products','AddProductsToPriceBook','description','true','advanced','','','','&pricebook_id='.$pricebook_id));
	...
	{        
		$search_form->assign("ALPHABETICAL",AlphabeticalSearch('Products','AddProductsToPriceBook','description','true','basic','','','','&pricebook_id='.$pricebook_id));
...
$sql1 = "select productid from prices where pricebookid=".$pricebook_id;
$res1 = $adb->query($sql1);
...
for($i=0; $i<$num_rows; $i++)
{
	...
	if(! array_key_exists($entity_id, $prod_array))
	{
		...
		$list_body .= '<td WIDTH="1" class="blackLine"><IMG SRC="'.$image_path.'blank.gif"></td>';
		$list_body .= '<td height="21" style="padding:0px 3px 0px 3px;">'.$adb->query_result($list_result,$i,"description").'</td>';
		$list_body .='<td WIDTH="1" class="blackLine" NOWRAP><IMG SRC="'.$image_path.'blank.gif"></td>';
		$list_body .= '<td height="21" style="padding:0px 3px 0px 3px;">'.$adb->query_result($list_result,$i,"stockid").'</td>';
		$list_body .='<td WIDTH="1" class="blackLine" NOWRAP><IMG SRC="'.$image_path.'blank.gif"></td>';
