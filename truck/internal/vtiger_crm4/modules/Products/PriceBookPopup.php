...
if(isset($_REQUEST['query']) && $_REQUEST['query'] != '' && $_REQUEST['query'] == 'true')
{
	...
	if (isset($_REQUEST['description'])) $productname = $_REQUEST['description'];
        if (isset($_REQUEST['stockid'])) $productcode = $_REQUEST['stockid'];
	...
	if (isset($productname) && $productname !='')
	{
		...
		//$search_query .= " and stockmaster.description like '".$productname."%'";
		$url_string .= "&productname=".$productname;
	}
	
	if (isset($productcode) && $productcode !='')
	{
		array_push($where_clauses, "stockid like ".PearDatabase::quote($stockid.'%'));
		//$search_query .= " and stockid like '".$stockid."%'";
		$url_string .= "&stockid=".$stockid;
	}
 ...
 /*       if (isset($_REQUEST['advanced']) && $_REQUEST['advanced'] == 'true') 
	{
		...
		$search_form->assign("ALPHABETICAL",AlphabeticalSearch('Products','index','description','true','advanced'));
	...
	{        
		$search_form->assign("ALPHABETICAL",AlphabeticalSearch('Products','index','description','true','basic'));
...
$pp=$_REQUEST['popuptype'];
$rowtotal = $_REQUEST['rowtotal'];
$allproducts = $_REQUEST['allproducts'];
$pieces = explode(",", $allproducts);
...
$list_query='select pricebook.*,crmentity.crmid, crmentity.smownerid, crmentity.modifiedtime from pricebook  inner join crmentity on crmentity.crmid = pricebook.pricebookid where crmentity.deleted=0';}
else {
$list_query= 'select pricebook.*,prices.productid,crmentity.crmid, crmentity.smownerid, crmentity.modifiedtime from pricebook inner join prices on prices.pricebookid = pricebook.pricebookid inner join crmentity on crmentity.crmid = pricebook.pricebookid where prices.productid='.$productid.' and crmentity.deleted=0';}
/*
...
// $rowtotal = $_REQUEST['rowtotal'];
$xtpl->assign("ROWTOTAL", $rowtotal);

// $allproducts = $_REQUEST['allproducts'];
$xtpl->assign("ALLPRODUCTS", $allproducts);
