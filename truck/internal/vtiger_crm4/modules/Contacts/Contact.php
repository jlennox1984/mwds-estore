...
// account query goes for ecery single account row
	...
	*/
	...
	function get_tickets($id)
	{
	  ...
	  {
		$query = "select crmentity.*, salesorders.*, quotes.subject as quotename, custbranch.brname, contactdetails.lastname from salesorders  inner join crmentity on crmentity.crmid=salesorders.salesorderid left outer join quotes on quotes.quoteid=salesorders.quoteid left outer join custbranch on custbranch.accountid=salesorders.accountid left outer join contactdetails on contactdetails.contactid=salesorders.contactid where crmentity.deleted=0 and salesorders.contactid = ".$id;
		renderRelatedSalesOrders($query,$id);	
	  }
	  function get_products($id)
	  {
		$query = 'select stockmaster.productid, stockmaster.description, stockmaster.stockid, stockmaster.commissionrate, stockmaster.qty_per_unit, stockmaster.unit_price, crmentity.crmid, crmentity.smownerid,contactdetails.lastname from stockmaster inner join crmentity on crmentity.crmid = stockmaster.productid left outer join contactdetails on contactdetails.contactid = stockmaster.contactid where contactdetails.contactid = '.$id.' and crmentity.deleted = 0';
...
return $exists;
	   ...
	   $query =  $this->constructCustomQueryAddendum() .",
				...
				inner join crmentity on crmentity.crmid=contactdetails.contactid
                                ...
                                LEFT JOIN custbranch on contactdetails.accountid=custbranch.accountid
	...
	function fill_in_additional_detail_fields()
	{
		...
		$query = "SELECT acc.accountid, acc.brname from custbranch acc, contactdetails  a_c where acc.accountid = a_c.contactid and a_c.contactid = '$this->id' and a_c.deleted=0";
