	...
	function create_list_query(&$order_by, &$where)
	{
		...
		if ($this->bean_type == 'Contacts')
		{
			/*$query = "SELECT distinct
				custbranch.brname as account_name,
				custbranch.accountid  as account_id,
				...
				LEFT JOIN custbranch  ON custbranch.accountid=contactdetails.accountid 
		...
		{
			/*$query = "SELECT distinct custbranch.*,
                                users.user_name assigned_user_name
				FROM accounts, users_last_import
                                LEFT JOIN users
                                ON custbranch.assigned_user_id=users.id
				WHERE 
				...
				$query = "SELECT distinct custbranch.*, custbranch.braddress1,
                                users.user_name assigned_user_name,
				crmid, smownerid 
				FROM custbranch
				inner join crmentity on crmentity.crmid=custbranch.accountid
		...
		else if ($this->bean_type == 'Potentials')
		{
			...
			$query = "SELECT distinct
                                custbranch.accountid account_id,
                                custbranch.brname account_name,
                                users.user_name assigned_user_name,
				crmentity.crmid, smownerid,
				potential.*
                               FROM potential 
			       inner join custbranch on custbranch.accountid=potential.accountid 
...
/*	function create_list_query(&$order_by, &$where)
	{
		...
		else if ($this->bean_type == 'Accounts')
		{
			/*$query = "SELECT distinct custbranch.*,
                                users.user_name assigned_user_name
				FROM custbranch, users_last_import
                                LEFT JOIN users
                                ON custbranch.assigned_user_id=users.id
				WHERE 
				users_last_import.assigned_user_id=
				...
				$query = "SELECT distinct custbranch.*,
                                users.user_name assigned_user_name
				FROM custbranch, users_last_import inner join crmentity on crmentity.crmid=custbranch.accountid left join users
