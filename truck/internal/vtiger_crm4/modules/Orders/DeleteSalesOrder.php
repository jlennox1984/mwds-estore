...
$sql_erp ="UPDATE salesorders set fromstkloc = '', debtorno='', branchcode=''  where salesorderid=".$_REQUEST['record'];
$adb->query($sql_erp);
...
{
	$relation_query = "UPDATE salesorders set quoteid='' where salesorderid=".$_REQUEST['record'];
	$adb->query($relation_query);
}
elseif($_REQUEST['return_module'] == "Potentials")
{
	$relation_query = "UPDATE salesorders set potentialid='' where salesorderid=".$_REQUEST['record'];
	$adb->query($relation_query);
}
elseif($_REQUEST['return_module'] == "Contacts")
{
	$relation_query = "UPDATE salesorders set contactid='' where salesorderid=".$_REQUEST['record'];
	$adb->query($relation_query);
}
elseif($_REQUEST['return_module'] == "Products")
{
	...
	$so_query = "select * from salesorderdetails where productid=".$_REQUEST['return_id'];
	...
	for($i=0; $i< $num_rows; $i++)
	{
	        ...
	        updateSubTotal("SalesOrder","salesorders","subtotal","total","salesorderid",$so_id,$prod_total);
	}
	//delete the relation from so product rel
	$vtlog->logthis("Delete query".$_REQUEST['potential_id'],'debug');
	
	$del_query = "delete from salesorderdetails where productid=".$_REQUEST['return_id']." and salesorderid=".$_REQUEST['record'];
	logQuery($del_query);
	$adb->query($del_query);
