...
//outlook security
function getNavigationValues($start, $noofrows, $list_max_entries_per_page, $accountid)
{
...
//					$orderno = getOrderNo();
					...
					$value = '<a href="a" LANGUAGE=javascript onclick=\'set_return_address("'.$cust_name.'","'.$account_id.'", "'.br2nl($acct_focus->column_fields['braddress1']).'", "'.br2nl($acct_focus->column_fields['brpostaddr1']).'", "'.br2nl($acct_focus->column_fields['braddress2']).'", "'.br2nl($acct_focus->column_fields['brpostaddr2']).'", "'.br2nl($acct_focus->column_fields['braddress3']).'", "'.br2nl($acct_focus->column_fields['brpostaddr3']).'", "'.br2nl($acct_focus->column_fields['braddress4']).'", "'.br2nl($acct_focus->column_fields['brpostaddr4']).'", "'.br2nl($acct_focus->column_fields['braddress6']).'", "'.br2nl($acct_focus->column_fields['brpostaddr6']).'", "'.br2nl($acct_focus->column_fields['braddress5']).'", "'.br2nl($acct_focus->column_fields['brpostaddr5']).'", "'.br2nl($holdreason).'", "'.br2nl($CreditAvailable).'"); window.close()\'>'.$cust_name.'</a>';
		...
		if($fieldname == $focus->list_link_field)
		{
			if($mode == "search")
			{
				...
				If (($_REQUEST['accountid'] != '') && (isset($_REQUEST['accountid'])))
					...
					if ($_REQUEST['curr_row'] != "" || isset($_REQUEST['curr_row']))
					{$row_id = $_REQUEST['curr_row'];}
					elseif ($_REQUEST['rowid'] != "" || isset($_REQUEST['rowid']))					
					{$row_id = $_REQUEST['rowid'];}
					$vtlog->logthis("rowid id is ".$rowid,'info');  
					$vtlog->logthis("rowid request account id is ".$_REQUEST['rowid'],'info');  
				$vtlog->logthis("curr row id is ".$_REQUEST['curr_row'],'info');  
				$productid=$adb->query_result($list_result,$list_result_count,'productid');
...
function getListQuery($module,$where='')
{
	...
	if($module == "Products")
	{
		$query = "select crmentity.crmid, stockmaster.*, productcf.* from stockmaster inner join crmentity on crmentity.crmid=stockmaster.productid left join productcf on stockmaster.productid = productcf.productid left join seproductsrel on seproductsrel.productid = stockmaster.productid where crmentity.deleted=0";
	}
        if($module == "Notes")
        {
...
function getNoOfAssocProducts($module,$focus,$seid='')
{
	...
	elseif($module == 'SalesOrder')
	{
		$query="select stockmaster.description,stockmaster.unit_price,salesorderdetails.* from salesorderdetails inner join stockmaster on stockmaster.productid=salesorderdetails.productid where salesorderdetails.completed=0 AND salesorderid=".$focus->id;
	}
	elseif($module == 'Invoice')
	{
...
$vtlog->logthis("in getDemand ".$stock_id,'info');  
        global $adb;
        $sql = "SELECT SUM(salesorderdetails.quantity-salesorderdetails.qtyinvoiced) AS dem FROM salesorderdetails, salesorders WHERE salesorders.orderno = salesorderdetails.orderno AND salesorderdetails.completed=0 AND salesorders.quotation=0 AND salesorderdetails.stkcode= '". $stock_id ."' ";
...
//	$ErrMsg =  _('The current account balance of the customer could not be retrieved because');
	...
	$nameArray2 = $adb->fetch_array($result2);
	$CreditAvailable = $nameArray2['creditlimit'];
	}
	...
	$result32 = $adb->query($sql3);
	$noof_group_rows3=$adb->num_rows($result3);
	$nameArray3 = $adb->fetch_array($result3);
	
	$CreditAvailable -= $nameArray3['ordervalue'];
