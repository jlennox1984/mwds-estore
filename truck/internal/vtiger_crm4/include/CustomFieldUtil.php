	...
	if($adb->num_rows($result) != 0)
	{
		...
		for($i=0; $i<$noofrows; $i++)
		{
			...
			if($uitype == 5)
			{
				...
				$custfld .= '<td width="30%"><input name="'.$setName.'" id="jscal_field_'.$setName.'" type="text" tabindex="2" size="11" maxlength="10" value="'.$value.'"> <img src="themes/'.$theme.'/images/calendar.gif" id="jscal_trigger_'.$setName.'"> ';
				$custfld .= '<input type="hidden" name="deliverydate" value="'.$value.'">';
				$custfld .= '<font size=1><em old="(yyyy-mm-dd)">(yyyy-mm-dd)</em></font></td>';
			...
			if($uitype == 999)
			{
				$custfld .= '<input type="hidden" name="'.$setName.'" value="'.$value.'">';
