...
function title ($label, $ext, $position)
{
    ...
    $this->RoundedRect($r1-16, $y1-1, 72, $y2+1, 2.5, 'DF');
 ...
 *
 */
function addDescBlock2( $text, $title, $position, $type='D',$curve='' )
{
    global $max_desc_size;
    $lengthtitle = strlen($title);
    $lengthdata= $position[3];
    ...
    if ((strlen($text) > $max_desc_size+140) && ($max_desc_size != 0))
    {
	$text = substr($text,0,$max_desc_size+140);
    }
    ...
    $this->SetXY( $position[0]+2 , $y1 + 1 );
    $this->SetFont( "Helvetica", "B", 10);
    $this->Cell(140,4, $title);
    $this->SetXY( $position[0]+2 , $y1 + 6 );
    $this->SetFont( "Helvetica", "", 10);
    $this->MultiCell(140,4,$text);
}
