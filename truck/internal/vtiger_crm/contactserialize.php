...
$server->wsdl->addComplexType(
    ...
    array(
        ...
        'brname' => array('name'=>'brname','type'=>'xsd:string'),
        ...
        'faxno' => array('name'=>'faxno','type'=>'xsd:string'),
...
function get_combo_values($id)
{
	global $adb;
	$output = Array();
	$sql = "select * from stockmaster inner join crmentity on crmentity.crmid=stockmaster.productid where crmentity.deleted=0";
...
function get_KBase_details($id='')
{
	...
	$product_query = "select * from stockmaster inner join crmentity on crmentity.crmid=stockmaster.productid where crmentity.deleted=0";
...
function retrieve_account_id($account_name,$user_id)
{
  if($account_name=="")
    ...
    $query = "select custbranch.brname accountname,custbranch.accountid accountid from custbranch inner join crmentity on crmentity.crmid=custbranch.accountid where crmentity.deleted=0 and custbranch.brname='" .$account_name."'";
    ...
    if($rows_count==0)
    {
    	require_once('modules/Accounts/Account.php');
	    $account = new Account();
    	$account->column_fields[brname] = $account_name;
