...
if(isset($_REQUEST['query']))
{
	...
	if(isset($last_name) && $last_name != "") array_push($where_clauses, "last_name like ".$adb->quote($last_name.'%')."");
	if(isset($first_name) && $first_name != "") array_push($where_clauses, "first_name like ".$adb->quote($first_name.'%')."");
	if(isset($user_name) && $user_name != "") array_push($where_clauses, "user_name like ".$adb->quote($user_name.'%')."");
	...
	if(isset($phone) && $phone != "") array_push($where_clauses, "(phone_home like ".$adb->quote('%'.$phone.'%')." OR phone_mobile like ".$adb->quote('%'.$phone.'%')." OR phone_work like ".$adb->quote('%'.$phone.'%')." OR phone_other like ".$adb->quote('%'.$phone.'%')." OR phone_fax like ".$adb->quote('%'.$phone.'%').")");
	if(isset($email) && $email != "") array_push($where_clauses, "(users.email1 like ".$adb->quote($email.'%')." OR users.email2 like ".$adb->quote($email.'%').")");
	if(isset($yahoo_id) && $yahoo_id != "") array_push($where_clauses, "yahoo_id like ".$adb->quote($yahoo_id.'%')."");
	if(isset($department) && $department != "") array_push($where_clauses, "department like ".$adb->quote($department.'%')."");
	if(isset($title) && $title != "") array_push($where_clauses, "title like ".$adb->quote($title.'%')."");
	if(isset($address_street) && $address_street != "") array_push($where_clauses, "address_street like ".$adb->quote($address_street.'%')."");
	if(isset($address_city) && $address_city != "") array_push($where_clauses, "address_city like ".$adb->quote($address_city.'%')."");
	if(isset($address_state) && $address_state != "") array_push($where_clauses, "address_state like ".$adb->quote($address_state.'%')."");
	if(isset($address_postalcode) && $address_postalcode != "") array_push($where_clauses, "address_postalcode like ".$adb->quote($address_postalcode.'%')."");
	if(isset($address_country) && $address_country != "") array_push($where_clauses, "address_country like ".$adb->quote($address_country.'%')."");
