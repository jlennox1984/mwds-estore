...
elseif($fld_module == 'Products')
{
	$custFldArray = Array($mod_strings['LBL_MANUFACTURER']=>'manufacturer'
			,$mod_strings['LBL_PRODUCT_CATEGORY']=>'categoryid'
...
function getStdOutput($custFldArray, $mod_strings)
{
	...
	foreach($custFldArray as $custFldName => $custFldTableName)
	{
		...
		if ($custFldTableName=="categoryid")
		{
		$standCustFld .= '<td width="12%" height="21" style="padding:0px 3px 0px 3px;"><div><a href="index.php?module=Settings&action=EditComboField&fld_module='.$_REQUEST["fld_module"].'&fld_name='.$custFldName.'&table_name=stockcategory&column_name='.$custFldTableName.'">'.$mod_strings['Edit'].'</a></div></td>';
