...
$tmp1 = strtoupper($accountname);
$tmp2 = substr($tmp1, 0, 3);
$tmp3 = $crmid;
$vtlog->logthis("crmseq is ".$crmid,'info'); 
$vtlog->logthis("crmseq is ".$_REQUEST['id'],'info'); 
$vtlog->logthis("crmseq is ".$_REQUEST['record'],'info'); 

$tmp4 = "00000" . $tmp3;
$tmp5 = substr($tmp4,-4,4);
$tmp6 = $tmp2 . $tmp5;
$debtorcode = $tmp6;
$vtlog->logthis("debtorcode is ".$debtorcode,'info'); 
...
$currcode = 'CDN';
        ...
        $sql_debtor = "INSERT INTO debtorsmaster (debtorno, name, address1, address2, address3, address4, address5, address6, currcode, clientsince, holdreason, paymentterms, discount, discountcode, pymtdiscount, creditlimit, salestype, invaddrbranch, taxref) VALUES 
...
$adb->query($sql_insert_contactsubdetails);
 ...
 $sql_insert_contactaddress = "INSERT INTO contactaddress (contactaddressid,mailingcity,mailingstreet,mailingstreet2,mailingstate,mailingcountry,mailingzip) VALUES (".$contact_id.",'".$row["city"] ."','" .$row["lane"] ."','" .$row["lane2"] ."','".$row['state']."','" .$row["country"] ."','".$row['code']."')";
