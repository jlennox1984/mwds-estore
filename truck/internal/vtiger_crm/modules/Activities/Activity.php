...
//Function Call for Related List -- Start
	...
	{
		$query = 'select activity.activityid, stockmaster.productid, stockmaster.description, stockmaster.stockid, stockmaster.commissionrate, stockmaster.qty_per_unit, stockmaster.unit_price, crmentity.crmid, crmentity.smownerid from activity inner join seactivityrel on activity.activityid = seactivityrel.activityid inner join stockmaster on seactivityrel.crmid = stockmaster.productid inner join crmentity on crmentity.crmid = stockmaster.productid where activity.activityid = '.$id.' and crmentity.deleted = 0';
