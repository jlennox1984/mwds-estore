...
if (isset($_POST['Show'])){
	...
	$LastPeriodSelected = max($SelectedPeriod);
 	...
 	$sql= "SELECT gltrans.type,
			...
			gltrans.periodno,
			lastdate_in_period,
			banktrans.amount as usamount,
			banktrans.exrate
		FROM gltrans, systypes
		JOIN periods on periods.periodno=gltrans.periodno
		LEFT JOIN banktrans on (banktrans.transno=gltrans.typeno AND banktrans.type=gltrans.type)
		WHERE gltrans.account = $SelectedAccount
		AND systypes.typeid=gltrans.type
		AND posted=1
		AND gltrans.periodno>=$FirstPeriodSelected
		AND gltrans.periodno<=$LastPeriodSelected
		ORDER BY gltrans.periodno, gltrans.trandate, counterindex";
	...
	$TableHeader = "<TR>
			...
			<TD class='tableheader'>" . _('Cheque Number') . "</TD>
			<TD class='tableheader'>" . _('Date') . "</TD>
			<TD class='tableheader'>" . _('Debit') . "</TD>
			<TD class='tableheader'>" . _('Credit') . "</TD>
			<TD class='tableheader'>" . _('Amount in US') . "</TD>
			<TD class='tableheader'>" . _('Exchange Rate') . "</TD>
	...
	while ($myrow=DB_fetch_array($TransResult)) {
		...
		if ($myrow['periodno']!=$PeriodNo){
			if ($PeriodNo!=-9999){ //ie its not the first time around
				...
				$PeriodNo2 = $PeriodNo - 1;
				$sql2 = "SELECT lastdate_in_period
					FROM periods
					WHERE periods.periodno=" . $PeriodNo;

$ErrMsg2 = _('The chart details for account') . ' ' . $SelectedAccount . ' ' . _('could not be retrieved');
				$ChartDetailsResult2 = DB_query($sql2,$db,$ErrMsg);
				$ChartDetailRow2 = DB_fetch_array($ChartDetailsResult2);
				$testing2=$ChartDetailRow2['lastdate_in_period'];


				echo "<TR bgcolor='#FDFEEF'> 
					<TD COLSPAN=4><B>" . _('Total for period') . ' ' . _(MonthAndYearFromSQLDate($testing2)) . '</B></TD>';
...
//					<TD COLSPAN=3><B>" . _('Total for period') . ' ' . $PeriodNo . '</B></TD>';
				if ($PeriodTotal < 0 ){ //its a credit balance b/fwd
					echo '<TD></TD>
						...
						<TD ALIGN=RIGHT><B>' . number_format($PeriodTotal2,2) . '</B></TD>
						<TD COLSPAN=2></TD>
						</TR>';
				} else { //its a debit balance b/fwd
					echo '<TD ALIGN=RIGHT><B>' . number_format($PeriodTotal,2) . '</B></TD>
						<TD></TD><TD ALIGN=RIGHT><B>' . number_format($PeriodTotal2,2) . '</B></TD>
						<TD COLSPAN=2></TD>
						</TR>';
				}
		...
		$RunningTotal2 += $myrow['amount'];
		$PeriodTotal += $myrow['amount'];
		$PeriodTotal2 += $myrow['usamount'];
		...
		if($myrow['usamount']>=0){
			$USAmount = number_format($myrow['usamount'],2);
...
if($myrow['chequeno']==0 || !isset($myrow['chequeno']) || $myrow['chequeno']==""){
			$Cheque = "-";
