...
If (isset($_POST['PrintPDF'])
	...
	if ($_POST['Location']=='All'){
		$SQL = "SELECT stockmaster.categoryid,
				...
				stockmaster.materialcost + stockmaster.overheadcost AS unitcost,
				SUM(locstock.quantity) *(stockmaster.materialcost + stockmaster.overheadcost) AS itemtotal
			FROM stockmaster,
				stockcategory,
				locstock
		...
		$SQL = "SELECT stockmaster.categoryid,
				...
				stockmaster.materialcost + stockmaster.overheadcost AS unitcost,
				locstock.quantity *(stockmaster.materialcost + stockmaster.overheadcost) AS itemtotal
			FROM stockmaster,
				stockcategory,
				locstock
			WHERE stockmaster.stockid=locstock.stockid
	...
	While ($InventoryValn = DB_fetch_array($InventoryResult,$db)){
		...
		if ($_POST['DetailedReport']=='Yes'){
			...
			$DisplayUnitCost = number_format($InventoryValn['unitcost'],4);
		...
		$Tot_Val += ROUND($InventoryValn['itemtotal'],2);
		$CatTot_Val += ROUND($InventoryValn['itemtotal'],2);
