...
if (!isset($_POST['PostInvoice'])){
	...
	if (count( $_SESSION['SuppTrans']->Shipts) > 0){   /*if there are any Shipment charges on the invoice*/
		...
		$TableHeader = "<TR><TD CLASS='tableheader'>" . _('Shipment') . "</TD>
				<TD CLASS='tableheader'>" . _('Amount') . '</TD><TD CLASS="tableheader">Type</TD></TR>';
		echo $TableHeader;
		...
		foreach ($_SESSION['SuppTrans']->Shipts as $EnteredShiptRef){
			...
			echo '<TR><TD>' . $EnteredShiptRef->ShiptRef . '</TD><TD ALIGN=RIGHT>' .
				number_format($EnteredShiptRef->Amount,2) . '</TD><TD>' . $EnteredShiptRef->InvoiceType . '</TD></TR>';
	...
	if (count( $_SESSION['SuppTrans']->ProdRuns) > 0){   /*if there are any Shipment charges on the invoice*/
		...
		foreach ($_SESSION['SuppTrans']->ProdRuns as $EnteredProdRunsRef){
			...
			echo '<TR><TD>' . $EnteredProdRunsRef->ProdRunsRef . '</TD><TD ALIGN=RIGHT>' .
				number_format($EnteredProdRunsRef->Amount,2) . '</TD><TD>' . $EnteredProdRunsRef->InvoiceType . '</TD></TR>';
			...
			these entries are reversed from the GRN suspense when the shipment is closed*/
				...
				$SQL = 'INSERT INTO gltrans (type, 
							...
							VALUES (20, ' .
						 		$InvoiceNo . ", 
								'" . $SQLInvoiceDate . "', 
								'" . $ShiptChg->InvoiceType . "', 
								" . $PeriodNo . ', 
								' . $_SESSION['SuppTrans']->GRNAct . ", 
								'" . $_SESSION['SuppTrans']->SupplierID . ' ' . _('Shipment charge against') . ' ' . $ShiptChg->ShiptRef . ' - ' . $ShiptChg->InvoiceType . "', 
				...
				$Result = DB_query($SQL, $db, $ErrMsg, $DbgMsg, True);
prnMsg(_('ShiptRef SQL ') . ' ' . $SQL . ' ' . _(' STAGE 1'),'success');
			...
			these entries are reversed from the GRN suspense when the shipment is closed*/
				...
				$SQL = 'INSERT INTO gltrans (type, 
							...
							VALUES (20, ' .
						 		$InvoiceNo . ", 
								'" . $SQLInvoiceDate . "', 
								'" . $ProdRunsChg->InvoiceType . "', 
								" . $PeriodNo . ', 
								' . $_SESSION['SuppTrans']->GRNAct . ", 
								'" . $_SESSION['SuppTrans']->SupplierID . ' ' . _('Production Run charge against') . ' ' . $ProdRunsChg->ProdRunsRef . ' - ' . $ProdRunsChg->InvoiceType . "', 
...
prnMsg(_('ProdRunsRef SQL ') . ' ' . $SQL . ' ' . _(' STAGE 1'),'success');
//prnMsg(_('$ProdRunsChg->InvoiceType   ') . ' ' . $ProdRunsChg->InvoiceType . ' ' . _(' STAGE 1'),'success');
...
//prnMsg(_('Prod Run TYPE ') . ' ' . $ProdRunChg->InvoiceType . ' ' . _('has been processed'),'success');
		 prnMsg(_('Prod Run GRN ') . ' ' . $SQL . ' ' . _('has been processed'),'success');
		...
		foreach ($_SESSION['SuppTrans']->Shipts as $ShiptChg){
			...
			$SQL = 'INSERT INTO shipmentcharges (shiptref, 
					...
					VALUES (' . $ShiptChg->ShiptRef . ', 
						20, 
						' . $InvoiceNo . ', 
						' . $ShiptChg->Amount/ $_SESSION['SuppTrans']->ExRate . ', "' . $ShiptChg->InvoiceType . '")';
...
// prnMsg(_('Shipment ') . ' ' . $_SESSION['SuppTrans']->Shipts . ' ' . _('has been processed'),'success');
		prnMsg(_('Shipt ') . ' ' . $SQL . ' ' . _('has been processed'),'success');
...
{$EnteredGRN->PRNo=0;}
			...
			$DbgMsg = _('The following SQL to insert the Shipment charge record was used');
prnMsg(_('Prod Run TYPE ') . ' ' . $ProdRunChg->InvoiceType . ' ' . _('has been processed'),'success');
		 prnMsg(_('Prod Run PR ') . ' ' . $SQL . ' ' . _('has been processed'),'success');
