...
if (!isset($_SESSION['PO'])){
   header ('Location:' . $rootpath . '/PO_Header2.php?' . SID);
   exit;
}
...
echo "<A HREF='$rootpath/PO_Header2.php?" . SID . "'>" ._('Back To Purchase Order Header') . '</A><BR>';
	...
	if ($InputError!=1){
		 ...
		 if ($_SESSION['ExistingOrder']==0){ /*its a new order to be inserted */
		     ...
		     $sql = "INSERT INTO purchorders (supplierno,
				...
				'" . FormatDateForSQL($_SESSION['PO']->Orig_OrderDate) . "',
				'" . FormatDateForSQL($_SESSION['PO']->ReqDate) . "',
				'" . DB_escape_string($_SESSION['PO']->ShipVia) . "',
				" . $_SESSION['PO']->ExRate . ",
...
if (isset($POLine->PRNo) || $POLine->PRNo<>'')
{
			 
			 $sqlspec3="SELECT prref from productionruns where prno='" . $POLine->PRNo . "'";
			 
	$resultspec3 = DB_query($sqlspec3,$db);
	$myrowspec3 = DB_fetch_row($resultspec3);
	$prrefspec3 = $myrowspec3['prref'];
}
else {$prrefspec3 = 0;}

	if ($prrefspec3 == '' || (!isset($prrefspec3)))
	{$prrefspec3 = 0;}
			...
			$sql = "UPDATE purchorders SET
					...
					orddate='" . $_SESSION['PO']->Orig_OrderDate . "',
					reqdeldate='" . $_SESSION['PO']->ReqDate . "',
					shipvia='" . $_SESSION['PO']->ShipVia . "',
	...
	{$prrefspec3 = 0;}
		 ...
		 echo "<BR><BR><A HREF='$rootpath/PO_Header2.php?" . SID . "&NewOrder=Yes'>" . _('Enter A New Purchase Order') . '</A>';
		 echo "<BR><A HREF='$rootpath/PO_SelectOSPurchOrder2.php?" . SID . "'>" . _('Select An Outstanding Purchase Order') . '</A>';
		 exit;
...
/*	if ($_SESSION['PO']->LineItems[$_POST['LineNo']]->QtyInv > $_POST['Qty'] OR $_SESSION['PO']->LineItems[$_POST['LineNo']]->QtyReceived > $_POST['Qty']){*/
		if ($_SESSION['PO']->LineItems[$_POST['LineNo']]->QtyReceived > $_POST['Qty']){
		$AllowUpdate = False;
...
echo '<TR><TD>' . _('Item Assembly Ref') . ': <FONT SIZE=1>(' . _('Leave blank if N/A') . ")</FONT></TD>
		<TD><input type='Text' SIZE=10 MAXLENGTH=9 name='PRNo' value=" . $_SESSION['PO']->LineItems[$_GET['Edit']]->PRNo . "><a target='_blank' href='$rootpath/PrsList.php?" . SID . "SupplierID=" . $_SESSION['PO']->SupplierID . "&SupplierName=" . $_SESSION['PO']->SupplierName . "'>" . _('Show Open Item Assemblies') . '</a></TD></TR>';
	/*
...
*/
 ...
 /*show a form for putting in a new line item with or without a stock entry */
	...
	echo '<TR><TD>' . _('Item Assembly Ref') . ': <FONT SIZE=1>' . _('(Leave blank if N/A)') . "</FONT></TD>
		<TD><input type='Text' SIZE=10 MAXLENGTH=9 name='PRNo' value=" . $_POST['PRNo'] . " > <a target='_blank' href='$rootpath/PrsList.php?" . SID . "&SupplierID=" . $_SESSION['PO']->SupplierID . "&SupplierName=" . $_SESSION['PO']->SupplierName . "'>" . _('Show Open Item Assemblies') . '</a></TD></TR>';
/*
echo '<TR><TD>' . _('Contract Ref') . ': <FONT SIZE=1>' . _('(Leave blank if N/A)') . "</FONT></TD>
...
echo "<BR><A HREF='$rootpath/PO_Header2.php?" . SID . "'>" ._('Back To Purchase Order Header') . '</A>';
