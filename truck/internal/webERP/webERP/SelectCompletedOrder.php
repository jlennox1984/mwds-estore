...
If (isset($OrderNumber)) {
	echo _('Order Number') . ' - SO' . $OrderNumber;
...
if ($OrderNumber=='' OR !isset($OrderNumber)){
	echo '<TR><TD>' . _('Order Number') . ':</TD><TD>SO' . "<INPUT type=text name='OrderNumber' MAXLENGTH =8 SIZE=9></TD><TD rowspan=2>" . _('for all orders placed after') .
...
/*show a table of the orders returned by the SQL */
	...
	while ($myrow=DB_fetch_array($SalesOrdersResult)) {
		...
		printf("<td><A target='_blank' HREF='%s'>SO%s</A></td>
			<td>%s</td>
			<td>%s</td>
			<td>%s</td>
			<td>%s</td>
