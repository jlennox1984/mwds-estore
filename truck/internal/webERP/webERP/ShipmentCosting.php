...
$sql = "SELECT SUM( purchorderdetails.qtyinvoiced ) AS totalqty FROM purchorderdetails LEFT JOIN shipmentcharges ON purchorderdetails.itemcode = shipmentcharges.stockid AND purchorderdetails.shiptref = shipmentcharges.shiptref WHERE purchorderdetails.shiptref =" . $_GET['SelectedShipment'];

$ErrMsg = _('Shipment') . ' ' . $_GET['SelectedShipment'] . ' ' . _('total quantities cannot be retrieved from the database');
$GetShiptQtyResult = DB_query($sql,$db, $ErrMsg);
if (DB_num_rows($GetShiptQtyResult)==0) {
	echo '<BR>';
	prnMsg ( _('No Invoiced Quantities exist for Shipment') . ' ' . $_GET['SelectedShipment'] . ' ' . _('in the database'), 'error');
	include ('includes/footer.inc');
	exit;
}
...
$LineItemsSQL = "SELECT purchorderdetails.orderno,
			...
			stockmaster.materialcost as stdcostunit,
...
if (db_num_rows($LineItemsResult) > 0) {
	...
	//		$PortionOfCharges = $TotalCostsToApportion *($myrow['invoicedcharges']/$TotalInvoiceValueOfShipment);
			$PortionOfCharges = $TotalCostsToApportion *($myrow['qtyinvoiced']/$TotalQty);
		} else {
			$PortionOfCharges = 0;
		}
		...
		if ( isset($_POST['Close']) AND $Variance !=0){
				...
				*/
				...
				if ($TotalQuantityOnHand>0) {
                                	...
                                	$CostIncrement2 = ($myrow['qtyinvoiced'] *($ItemShipmentCost - $myrow['stdcostunit']) - $WriteOffToVariances) / $TotalQuantityOnHand;
									$CostIncrement = $CostIncrement2 + $myrow['stdcostunit'];
									$CostIncrement3 = $PortionOfCharges/$TotalQuantityOnHand;
									
									
									
		//									prnMsg ( _('Shipment calulation, cost increment2 ') . $CostIncrement2 . 'cost increment '. $CostIncrement . ' ' . ' = qty invoiced ' . $myrow['qtyinvoiced'] . ' * (shipment cost)' . $ItemShipmentCost . ' - stdcost ' . $myrow['stdcostunit'] . ' - write of var  ' .  $WriteOffToVariances . ' ) / qty on hand ' . $TotalQuantityOnHand .  _('in the database'), 'warn');
											prnMsg ( _('additional value to add to inventory ') . $PortionOfCharges . '  Cost Increment ' . $CostIncrement3);
									
                                	$sql = 'UPDATE stockmaster SET lastcost=materialcost+overheadcost,
                                                                   materialcost=materialcost + ' . $CostIncrement3 . ' WHERE stockid="' . $myrow['itemcode'] . '"';
					$Result = DB_query($sql, $db, $ErrMsg, $DbgMsg,'',TRUE);
				} else {
				
									
											prnMsg ( _('Shipment calulation, item shipment cost') . ' '. $ItemShipmentCost .   _('in the database'), 'warn');
					$sql = 'UPDATE stockmaster SET lastcost=materialcost+overheadcost,
