       ...
       if (DB_num_rows($GetShiptHdrResult)==1) {
	      ...
	      if ($myrow['closed']==1){
			...
			exit;
			  ...
			  $_SESSION['Shipment']->InitDate = $myrow['initdate'];
		$_SESSION['Shipment']->ReqDate = $myrow['reqdate'];
		$_SESSION['Shipment']->Initiator = $myrow['initiator'];
		$_SESSION['Shipment']->Comments = $myrow['comments'];
		$_SESSION['Shipment']->RequisitionNo = $myrow['requisitionno'];
		$_SESSION['Shipment']->DatePrinted = $myrow['dateprinted'];
		$_SESSION['Shipment']->AllowPrint = $myrow['allowprint'];
...
$_SESSION['Shipment']->Initiator = $_POST['Initiator'];
$_SESSION['Shipment']->Comments = $_POST['Comments'];
$_SESSION['Shipment']->RequisitionNo = $_POST['RequisitionNo'];
$_SESSION['Shipment']->AllowPrint = $_POST['AllowPrint'];
$_SESSION['Shipment']->InitDate = FormatDateForSQL($_POST['InitDate']);
	$_SESSION['Shipment']->ReqDate = FormatDateForSQL($_POST['ReqDate']);
//	$_SESSION['Shipment']->DatePrinted = FormatDateForSQL($_POST['DatePrinted']);
	...
	if ($InputError ==0 AND count($_SESSION['Shipment']->LineItems)>0){
		...
		if (DB_num_rows($result)==1){
			...
			$sql = "UPDATE shipments SET vessel='" . DB_escape_string($_SESSION['Shipment']->Vessel) . "',
							...
							initdate='" .  $_SESSION['Shipment']->InitDate . "',
							reqdate='" .  $_SESSION['Shipment']->ReqDate . "',
							allowprint='" .  $_SESSION['Shipment']->AllowPrint . "',
							requisitionno='" .  $_SESSION['Shipment']->RequisitionNo . "',
							initiator='" .  $_SESSION['Shipment']->Initiator . "',
							comments='" . DB_escape_string($_SESSION['Shipment']->Comments) . "'
			...
			$sql = "INSERT INTO shipments (shiptref,
					...
					VALUES (" . $_SESSION['Shipment']->ShiptRef . ",
						...
						'" . $_SESSION['Shipment']->InitDate . "',
						'" . $_SESSION['Shipment']->ReqDate . "',
						'" . $_SESSION['Shipment']->Initiator . "',
						'" . DB_escape_string($_SESSION['Shipment']->Comments) . "',
						'" . $_SESSION['Shipment']->RequisitionNo . "',
						'" . $_SESSION['Shipment']->AllowPrint . "')"  ;
...
echo '<TR><TD VALIGN="TOP">'. _('Expected Arrival Date (ETA)'). ': </TD>
	<TD VALIGN="TOP"><INPUT TYPE=Text NAME="ETA" MAXLENGTH=10 SIZE=10 VALUE="' . $ETA . '"></TD>
	<TD>'. _('Into').' ';
...
if (isset($_SESSION['Shipment']->InitDate)){
	$InitDate = ConvertSQLDate($_SESSION['Shipment']->InitDate);
} else {
	$InitDate =Date($_SESSION['DefaultDateFormat']);
}
	  
	//  if ($_SESSION['ExistingShipment']!=0){ 
	//	$date1 = $_SESSION['Shipment']->InitDate;
		//echo "<INPUT TYPE=TEXT NAME='InitDate' SIZE=11 MAXLENGTH=10 VALUE=" . $date1 . ">";
	 echo "<INPUT TYPE=TEXT NAME='InitDate' SIZE=11 MAXLENGTH=10 VALUE=" . $InitDate . ">";
	 // } else {
...
if (isset($_SESSION['Shipment']->ReqDate)){
	$ReqDate = ConvertSQLDate($_SESSION['Shipment']->ReqDate);
} else {
	$ReqDate =Date($_SESSION['DefaultDateFormat']);
}
	 // if ($_SESSION['Shipment']!=0){ 
	//	$date1a = $_SESSION['Shipment']->ReqDate;
	//	echo "<INPUT TYPE=TEXT NAME='ReqDate' SIZE=11 MAXLENGTH=10 VALUE=" . $date1a . ">";
	 echo "<INPUT TYPE=TEXT NAME='ReqDate' SIZE=11 MAXLENGTH=10 VALUE=" . $ReqDate . ">";
	 // } else {
	  	/* DefaultDateFormat defined in config.php */
	...
	//	 echo "<INPUT TYPE=TEXT NAME='ReqDate' SIZE=11 MAXLENGTH=10 VALUE=" . Date($_SESSION['DefaultDateFormat']) . ">";
	 // }
	  ...
	  echo '<TR><TD>' . _('Initiated By') . ":</TD>
	  		<TD><INPUT TYPE=TEXT NAME='Initiator' SIZE=11 MAXLENGTH=10 VALUE=" . $_SESSION['Shipment']->Initiator . "></TD>
		</TR>";
	  echo '<TR><TD>' . _('Requistion Ref') . ":</TD><TD><INPUT TYPE=TEXT NAME='RequisitionNo' SIZE=16 MAXLENGTH=15 VALUE=" . $_SESSION['Shipment']->RequisitionNo . '></TD>
	  	</TR>';

	  echo '<TR><TD>' . _('Date Printed') . ':</TD><TD>';

	  if (isset($_SESSION['Shipment']->DatePrinted) AND strlen($_SESSION['Shipment']->DatePrinted)>6){
	  	$DatePrint=ConvertSQLDate($_SESSION['Shipment']->DatePrinted);
		$Printed = True;
		//if ($DatePrint=="00/00/0000")
		//{$DatePrint="Not yet printed.";
		//$Printed = False;}
	     echo $DatePrint;
	  ...
	  if ($_SESSION['Shipment']->AllowPrint==0 AND $_POST['RePrint']!=1){
	     echo "</TD></TR><TR><TD>" . _('Allow Reprint') . ":</TD><TD><SELECT NAME='AllowPrint'><OPTION SELECTED VALUE=0>" . _('No') . "<OPTION VALUE=1>" . _('Yes') . "</SELECT></TD></TR>";
	  } elseif ($Printed) {
	     echo "</TD></TR><TR><TD COLSPAN=2 ALIGN=CENTER><A target='_blank'  HREF='$rootpath/Ship_PDFShipment.php?" . SID . "ShiptRef=" . $_SESSION['ExistingShipment'] . "'>" . _('Reprint Now') . "</A></TD></TR>";
	  }

echo '</TABLE>';

echo  _('Comments');
echo ":<textarea name='Comments' cols=70 rows=2>" . $_SESSION['Shipment']->Comments . '</textarea><br><br>';
