...
$so_id = $focus->column_fields["orderno"];
...
$query="select stockmaster.description,stockmaster.stockid,stockmaster.unit_price,stockmaster.longdescription,salesorderdetails.* from salesorderdetails inner join stockmaster on stockmaster.productid=salesorderdetails.productid where salesorderid=".$id;
...
for($i=0;$i<$num_products;$i++) {
			...
			$disc[$i]=$adb->query_result($result,$i,'discountpercent');
	//			$discountpercent[$i]="".$disc[$i]*100."%";
                $unit_price[$i]= $currency_symbol.number_format($adb->query_result($result,$i,'unit_price'),2,'.',',');
                $list_price[$i]= $currency_symbol.number_format($adb->query_result($result,$i,'unitprice'),2,'.',',');
                $list_pricet[$i]= $adb->query_result($result,$i,'unitprice');
//                $prod_totalt= $qty[$i]*$list_pricet[$i];
 $prod_totalt= $qty[$i]*$list_pricet[$i]-($qty[$i]*$list_pricet[$i]*$disc[$i]);
		...
		$prod_total[$i] = $currency_symbol.number_format($prod_totalt,2,'.',',');
                ...
                $product_line[] = array( "Product ID"    => $stock_id[$i],
