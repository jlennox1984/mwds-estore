...
if (DB_num_rows($result)==0){
        ...
        if ($myrow['printedpackingslip']==1 AND ($_GET['Reprint']!='OK' OR !isset($_GET['Reprint']))){
              ...
              echo '<P><A HREF="' . $rootpath . '/PrintCustOrder.php?' . SID . '&TransNo=' . $_GET['TransNo'] . '&Reprint=OK">'
                ...
                '<A HREF="' . $rootpath. '/PrintCustOrder_generic.php?' . SID . '&TransNo=' . $_GET['TransNo'] . '&Reprint=OK">'. _('Do a Re-Print') . ' (' . _('Plain paper') . ' - ' . _('Letter') . ' ' . _('portrait') . ') ' . _('Even Though Previously Printed'). '</A>';
...
$LeftOvers = $pdf->addTextWrap($Xpos+15,$YPos-75,170,$FontSize, _('Total Pallets: '),'left');
		...
		$pdf->line($Xpos+90,$YPos-55,$Xpos+250, $YPos-55);
