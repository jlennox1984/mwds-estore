...
If (isset($OrderNumber) && $OrderNumber!='') {
	if (!is_numeric($OrderNumber)){
		...
		echo _('Order Number') . ' - PO' . $OrderNumber;
	}
} else {
	If ($SelectedSupplier) {
...
if ($OrderNumber=='' OR !isset($OrderNumber)){
	...
	echo _('order number') . ': PO<INPUT type=text name="OrderNumber" MAXLENGTH =8 SIZE=9>  ' . _('Into Stock Location') . ':<SELECT name="StockLocation"> ';
...
//end if stock search results to show
	...
	while ($myrow=DB_fetch_array($PurchOrdersResult)) {
		...
		if ($myrow["allowprint"]==1){
			...
			$PrintPurchOrder = '<A target="_blank" HREF="' . $rootpath . '/PO_PDFPurchOrder.php?' . SID . 'OrderNo=' . $myrow['orderno'] . '">' . _('Printed') . '</a>';
		}
		$FormatedOrderDate = ConvertSQLDate($myrow['orddate']);
		$FormatedOrderValue = number_format($myrow['ordervalue'],2);

		printf("<td><A HREF='%s'>PO%s</A></FONT></td>
