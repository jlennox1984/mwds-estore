...
if (isset($_GET['ModifyOrderNumber'])) {
	$title = _('Modify Purchase Order') . ' PO' . $_GET['ModifyOrderNumber'];
} else {
...
//{$_POST['OrdDate'] = $_SESSION['PO']->OrdDate;}
	...
	$_SESSION['PO']->Orig_OrderDate = $_POST['OrdDate'];
	$_SESSION['PO']->ReqDate = $_POST['ReqDate'];
	$_SESSION['PO']->ShipVia = $_POST['ShipVia'];
	...
	echo "<META HTTP-EQUIV='Refresh' CONTENT='0; URL=" . $rootpath . '/PO_Items2.php?' . SID . "'>";
	echo '<P>';
	prnMsg(_('You should automatically be forwarded to the entry of the purchase order line items page') . '. ' . _('If this does not happen') . ' (' . _('if the browser does not support META Refresh') . ') ' . "<a href='$rootpath/PO_Items2.php?" . SID . "'>" . _('click here') . '</a> ' . _('to continue'),'info');
}
...
// everything below here only do if a supplier is selected
	...
	echo '<CENTER>' . _('Purchase Order') . ': <FONT COLOR=BLUE SIZE=4><B>PO' . $_SESSION['PO']->OrderNo . ' ' . $_SESSION['PO']->SupplierName . '</U> </B></FONT> - ' . _('All amounts stated in') . ' ' . $_SESSION['PO']->CurrCode . '<BR><BR>';
	...
	if(($_POST['StkLocation']=='' OR !isset($_POST['StkLocation'])) AND (isset($_SESSION['PO']->Location) AND $_SESSION['PO']->Location!='')){
	    ...
	    $_POST['OrdDate']=$_SESSION['PO']->Orig_OrderDate;
	    $_POST['ReqDate']=$_SESSION['PO']->ReqDate;
	    $_POST['ShipVia']=$_SESSION['PO']->ShipVia;
	...
	/*nested table level1 */
	 ...
	 if (!isset($_POST['StkLocation']) OR $_POST['StkLocation']==''){ /*If this is the first time the form loaded set up defaults */
	  ...
	  echo '<TABLE><TR><TD>' . _('Order Date (d-m-y)') . ':</TD><TD>';
	  if ($_SESSION['ExistingOrder']!=0){ 
//		echo ConvertSQLDate($_SESSION['PO']->Orig_OrderDate);
		$date1 = $_POST['OrdDate'];
		$date2 = date('$date1 dmY');
		echo "<INPUT TYPE=TEXT NAME='OrdDate' SIZE=11 MAXLENGTH=10 VALUE=" . $date1 . ">";
	  } else {
	  	/* DefaultDateFormat defined in config.php */
		// echo Date($_SESSION['DefaultDateFormat']);
		 echo "<INPUT TYPE=TEXT NAME='OrdDate' SIZE=11 MAXLENGTH=10 VALUE=" . Date($_SESSION['DefaultDateFormat']) . ">";
	  }
...
//		echo ConvertSQLDate($_SESSION['PO']->Orig_OrderDate);
	  ...
	  echo '<TR><TD>' . _('Ship Via') . ":</TD>
	  		<TD><INPUT TYPE=TEXT NAME='ShipVia' SIZE=16 MAXLENGTH=50 VALUE=" . $_POST['ShipVia'] . "></TD>
		</TR>";
