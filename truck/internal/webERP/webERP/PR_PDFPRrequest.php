...
$locationname = $LocnNameRow[0];
		...
		$pdf->addText(460,$YPos-8, 12, _('Order Total - excl tax'). ' ' . $POHeader['currcode']);
		$LeftOvers = $pdf->addTextWrap($Left_Margin+1+64+300+85+40+60+75,$YPos-11,95,14,$DisplayOrderTotal, 'right');

	} /*end if there are order details to show on the order*/
    //} /* end of check to see that there was an order selected to print */
$YPos = $Bottom_Margin + $line_height;
		
$pdf->addText(35,$YPos-9, 11, _('Signature'). ' :');
$pdf->line($Left_Margin, $YPos+ ($line_height*2)+10, ($Page_Width/2)+50, $YPos+ ($line_height*2)+10);
$pdf->line($Page_Width/2+50, $YPos+ ($line_height*2)+10, $Page_Width/2+50, $Bottom_Margin);
$pdf->line(($Page_Width/2)+50, $YPos+ $line_height, $Page_Width-$Right_Margin, $YPos+ $line_height);
