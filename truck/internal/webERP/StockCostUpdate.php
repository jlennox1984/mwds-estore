...
if (isset($_POST['UpdateData'])){
	...
	//    prnMsg (_('old SQL') . $sql,'error',_('Non-existent Item'));
    ...
    DB_free_result($oldresult);
 	...
 	$OldCost =$_POST['OldMaterialCost'];
   	$NewCost =$_POST['MaterialCost'];

	$OldLabourCost =$_POST['OldLabourCost'];
   	$NewLabourCost =$_POST['LabourCost'];

	$result = DB_query("SELECT * FROM stockmaster WHERE stockid='$StockID'",$db);
	$myrow = DB_fetch_row($result);
	if (DB_num_rows($result)==0) {
		prnMsg (_('The entered item code does not exist'),'error',_('Non-existent Item'));
	} elseif ($OldCost != $NewCost || $OldLabourCost != $NewLabourCost ){
	...
	//  prnMsg(_('UPDATE SQL  ') . $SQL . _(' '),'warn');
...
if (! in_array($UpdateSecurity,$_SESSION['AllowedPageSecurityTokens']) OR !isset($UpdateSecurity)){
	echo '<TR><TD>' . _('Cost') . ':</TD><TD ALIGN=RIGHT>' . number_format($myrow['materialcost']+$myrow['overheadcost'],2) . '</TD></TR></TABLE>';
} else {
	...
	if ($myrow['mbflag']=='M'){
		echo '<TR><TD>' . _('Standard Labour Cost Per Unit') . ':</TD><TD ALIGN=RIGHT><INPUT TYPE=TEXT NAME=LabourCost VALUE=' . $myrow['labourcost'] . '> * not included in last cost above</TD></TR>';
