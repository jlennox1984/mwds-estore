...
if (isset($_GET['ModifyOrderNumber'])) {
	$title = _('Modifying Order') . ' SO' . $_GET['ModifyOrderNumber'];
	$sotitle = ' SO' . $_GET['ModifyOrderNumber'];
...
/*read in all the guff from the selected order into the Items cart  */
	...
	$OrderHeaderSQL = 'SELECT salesorders.debtorno,
				...
				salesorders.freighttax,
				salesorders.discounttotal,
				salesorders.salestax,
...
/*CustomerID defined in header.inc */
		...
		$_SESSION['Items']->Subject = $myrow['subject'];
		...
		$_SESSION['Items']->DeliveryDate = ConvertSQLDate($myrow['duedate']);
		$_SESSION['Items']->DeliveryDate2 = ConvertSQLDate($myrow['deliverydate']);
		...
		$_SESSION['Items']->AccountId = $myrow['accountid'];
		$_SESSION['Items']->SalesOrderId = $myrow['salesorderid'];
		$_SESSION['Items']->Quotation = $myrow['quotation'];
		$_SESSION['Items']->FreightCost = $myrow['freightcost'];
		$_SESSION['Items']->FreightCostOriginal = $myrow['freightcost'];
		$_SESSION['Items']->FreightTax = $myrow['freighttax'];
		$_SESSION['Items']->DiscountTotal = $myrow['discounttotal'];
		$_SESSION['Items']->SalesTax = $myrow['salestax'];
		$_SESSION['Items']->SalesTaxOriginal = $myrow['salestax'];
...
# the branch was also selected from the customer selection so default the delivery details from the customer branches table CustBranch. The order process will ask for branch details later anyway
		...
		$_SESSION['Items']->AccountId = $myrow[12];
...
# defaulted from the entry of the userid and password.
	...
	// the order process will ask for branch details later anyway
		...
		$_SESSION['Items']->AccountId = $myrow[11];
	 ...
	 If (isset($_POST['QuickEntry'])){
	     ...
	     do {
			...
			if (DB_num_rows($KitResult)==0){
				prnMsg( _('The item code') . ' ' . $sql . ' ' . _('could not be retrieved from the database and has not been added to the order'),'warn');
				prnMsg(_('The bill of material for  part ') . ' - ' . _(' ' . ' 1 ' .  $sql  . ' 2 ' .  $aa2  . ' 3 ' .  $aa3 . ' 4 ' .  $aa4 ),'info');
