...
if (isset($_FILES['ItemPicture']) AND $_FILES['ItemPicture']['name'] !='') {
 require('estore.conf.php');
	...
	$filename2 = '../vtiger_crm/test/product/' . $StockID . '.jpg';
	 $filename_parts = '../../prod-picts/' . $StockID .'.jpg'; 
	...
	//$result =   move_uploaded_file($_FILES['ItemPicture']['tmp_name'], $filename_parts);
		...
		prnMsg( _('estore product thumbnal have been saved in ' .$filename_parts));
//		prnMsg( _('result2'). ' ' . $filename2. ' '. _(' '). 'warning');
//  prnMsg( _('result2'). ' ' . $_SESSION['part_pics_dir']. ' '. _(' '). 'warning');
	...
	if ($InputError !=1){
		...
		if (!isset($_POST['New'])) { /*so its an existing one */
			...
			*/
			...
			$sql = "SELECT mbflag, controlled, serialised, special FROM stockmaster WHERE stockid = '$StockID'";
			$MBFlagResult = DB_query($sql,$db);
			$myrow = DB_fetch_row($MBFlagResult);
			$OldMBFlag = $myrow[0];
			$OldControlled = $myrow[1];
			$OldSerialised = $myrow[2];
			$Special=$myrow['special'];
			$sql = "SELECT SUM(locstock.quantity) FROM locstock WHERE stockid='$StockID'";
			$result = DB_query($sql,$db);
			$stkqtychk = DB_fetch_row($result);
		$sql = "SELECT special FROM stockmaster WHERE stockid='$StockID'";
			$specresult = DB_query($sql,$db);
			$specresult1 = DB_fetch_row($specresult);
...
if ($specresult1[0]!=0) {
	...
	//					prnMsg( _('The make or buy flag cannot be changed from manufactured, kitset or assembly to'). ' ' . $_POST['MBFlag'] . ' '. _('where there is a bill of material set up for the item') . '. ' . _('Bills of material are not appropriate for purchased or dummy items'), 'error');
			...
			$itempicture = $_FILES['ItemPicture'];
			
$filename1 = '../webERP/' . $_SESSION['part_pics_dir'] . '/' . $StockID . '.jpg';
$picportal = $productpics . '/' . $StockID . '.jpg';	
			//FIX Sql errors on 09/05/08 
			if ($InputError == 0){
                              $timestamp= time();			
				$sql = "UPDATE stockmaster
						SET longdescription='" . DB_escape_string($_POST['LongDescription']) . "',
							...
							special='" . $_POST['VTiger'] . "'
					WHERE stockid='$StockID'";
				   //Update for Vituremart
                                      $sql1 = "UPDATE jos_vm_product 
						SET product_s_desc= '" . DB_escape_string($_POST['Description']) ."',
                                                	    product_desc='" . DB_escape_string($_POST['LongDescription']) . "',
							product_publish= '". $_POST['PUB'] . "',
							product_length='". $_POST['LENGHT'] ."',
							product_weight='".$_POST['KGS'] ."',
							mdate='". $timestamp ."',
                                                        product_tax_id='". $_POST['TaxCat'] ."',					   								product_availability= '" . $_POST['eta'] ."',
							product_weight_uom= '" . $_POST['Unit'] . "',
							product_height ='". $_POST['HEIGHT'] . "',
							product_width ='". $_POST['WIDTH'] . "', 
							product_name='" . DB_escape_string($_POST['Description']) . "',
							 product_thumb_image='$picportal',
							  product_full_image='$picportal'
							where product_sku='$StockID'";

                                                          
prnMsg( _('itempicture'). ' ' . $_FILES['ItemPicture']['name'] . ' '. _('   ') . '. ' . _('image name to be saved '). ' ' . $filename1 . ' '. _(' '). 'warning');
			if (DB_error_no($db) ==0) {
				$product_id = getProductID($StockID);
//		echo '<BR>'.$account_id._(' RESULTS ').$SelectedBranch;
	
					$today = date("YmdHis");
					
					$crmsql = "update crmentity set modifiedtime ='".$today."' WHERE crmid = '".$product_id."'";
	
					$ErrMsg =  _('Could not update CRM') . ' ' . $myrow[0] .  ' ' . _('could not be added because');
					$DbgMsg = _('Could not update CRM') . ' ' . _('The SQL that was used to update the CRM entity');
					$CRMResult = DB_query($crmsql,$db,$ErrMsg,$DbgMsg);
							}
				$ErrMsg = _('The stock item could not be updated because');
				$DbgMsg = _('The SQL that was used to update the stock item and failed was');
				$result = DB_query($sql,$db,$ErrMsg,$DbgMsg);
                                $result1 = DB_query($sql1,$db,$ErrMsg,$DbgMsg);  
				prnMsg( _('SQL ') . ' ' . $sql1 . ' ' . _('has been updated'), 'success');
				//prnMsg( _('CRM SQL ') . ' ' . $crmsql . ' ' . _('has been updated'), 'success');
			...
			$current_id = getUniqueID($db);
							
					$today = date("YmdHis");
					$sql = "insert into crmentity (crmid,smcreatorid,smownerid,setype,description,createdtime,modifiedtime) values('".$current_id."','1','1','Products','','".$today."','".$today."')";
					...
					$sql = "update crmentity_seq set id = ".$current_id."";
...
//					echo '<BR>'._('The new crmid is').' ' . $new_id. ' '._('added.');
				...
				$result = DB_query($sql,$db, $ErrMsg, $DbgMsg);
				$timestamp= time();
				$save_name = '$StockID.jpg';
                                $picportal = $productpics . '/' . $StockID . '.jpg';
				 $sql2= "INSERT INTO jos_vm_product	( vendor_id,
									product_sku,
									product_s_desc,		
									product_desc,	
									product_publish,
				...
				$result = DB_query($sql2,$db, $ErrMsg, $DbgMsg);
                              $sql =" select product_id from jos_vm_product  where product_sku='$StockID'";
                   $result1= DB_query($sql,$db); 
                   $myrow1=DB_fetch_array($result1);
                  $prodcode=$myrow1['product_id'];
                      
                     $sql =" select category_id from jos_vm_category where ptcode='" . $_POST['CategoryID'] ."' ";
                   $result1= DB_query($sql,$db); 
                   $myrow1=DB_fetch_array($result1);
                  $ptcode=$myrow1['category_id'];
                    
			$sql ="INSERT INTO jos_vm_product_category_xref( category_id,product_id)  Values (". $ptcode ."
													,". $prodcode .")";
				                             $ErrMsg =  _('The item could not be added because');
				$DbgMsg = _('The SQL that was used to add the item failed was');
				$result = DB_query($sql,$db, $ErrMsg, $DbgMsg);
...
//		echo '<BR>'.$account_id._(' RESULTS ').$SelectedBranch;
					...
					$sql = "insert into productcf (productid) values('".$current_id."')";

					$ErrMsg =  _('The locations for the item') . ' ' . $myrow[0] .  ' ' . _('could not be added because');
					$DbgMsg = _('Could not add productcf') . ' ' . _('The SQL that was used to add the productcf entity');
					$ProdCFResult = DB_query($sql,$db,$ErrMsg,$DbgMsg);
	...
	if ($CancelDelete==0) {
		$result = DB_query('BEGIN', $db);
			...
			$result=DB_query($sql,$db, _('Could not delete the item record'),'',true);
                        $sql="DELETE FROM jos_vm_product where product_sku='$StockID'";
		        $result=DB_query($sql,$db, _('Could not delete the item record'),'',true);
...
echo '<TR><TD>' . _('Part Description') . ' (' . _('long') . '):</TD><TD><textarea  ID="longdesc" name="LongDescription"style="height: 270px; width: 500px;">' . $_POST['LongDescription'] . ' </textarea>';
//old Editor
//echo '<script language="javascript1.2">';
 //echo "generate_wysiwyg('longdesc')";
//echo '</script> </TD></TR> ';
echo '</TD></TR>';
...
if ($_POST['VTiger']=='' OR !isset($_POST['VTiger'])){
	$_POST['VTiger']=0;
}
...
echo '<TR><TD>' . _('Lenght of Product') . ':</TD><TD><input type="Text" name="LENGHT" SIZE=12 MAXLENGTH=10 value="' . $_POST['LENGHT'] . '"></TD></TR>';


echo '<TR><TD>' . _('Width of Product') . ':</TD><TD><input type="Text" name="WIDTH" SIZE=12 MAXLENGTH=10 value="' . $_POST['WIDTH'] . '"></TD></TR>';

echo '<TR><TD>' . _('Height of Product') . ':</TD><TD><input type="Text" name="HEIGHT" SIZE=12 MAXLENGTH=10 value="' . $_POST['HEIGHT'] . '"></TD></TR>';
...
echo '<TR><TD>' . _('Available in VTiger') . ' ';
//echo  $_POST['VTIGER'] .'  aaa '. $myrow['special'] . ' qqq ' . $qqq . ' rrr ' . $rrr;
echo ':  </TD><TD><select name="VTiger">  ';
...
// Jeff edits
//------------------------------------------------
echo '<TR><TD>' . _('ETA Of Devlivery') . ':</TD><TD><SELECT name="eta">';
if ($_POST['eta']=='24'){
	echo '<OPTION SELECTED VALUE="24h.gif">' . _('24 Hours');
} else {
	echo '<OPTION VALUE="24h.gif">' . _('24 Hours');
}
if ($_POST['eta']=='48'){
	echo '<OPTION SELECTED VALUE="48h.gif">' . _('48 Hours');
} else {
	echo '<OPTION VALUE="48h.gif">' . _('48 Hours');
}
if ($_POST['eta']=='34')  {
	echo '<OPTION SELECTED VALUE="3-5d.gif">' . _('3 to 5 days');
} else {
	echo '<OPTION VALUE="3-5d.gif">' . _('3 to 5 days');
}

if ($_POST['eta']=='7') {
	echo '<OPTION SELECTED VALUE="7day.gif">' . _('7 days');
} else {
	echo '<OPTION VALUE="7d.gif">' . _('7 Days');
}

if ($_POST['eta']=='oo'){
	echo '<OPTION SELECTED VALUE="on-order.gif">' . _('On Order');
} else {
	echo '<OPTION VALUE="on-order.gif">' . _('On Order');
}
echo '</select> </TD></TR>';
//------------------------//


 echo '<TR> <TD>'  . _('Online sale') . ':</TD><TD><SELECT name="PUB">';
     if ($_POST['PUB'] == yes){
